...
thf(asprvalpw_ax,axiom,(! [XV:($i > $o)] : (! [XW:($i > $o)] : ((cwcel @ XV @ XW) => ((ccfv @ XV @ ccspr) = (ccrab @ (^ [Xp:$i] : (cwrex @ (^ [Xa:$i] : (cwrex @ (^ [Xb:$i] : ((ccv @ Xp) = (ccpr @ (ccv @ Xa) @ (ccv @ Xb)))) @ (^ [Xb:$i] : XV))) @ (^ [Xa:$i] : XV))) @ (^ [Xp:$i] : (ccpw @ XV)))))))).
