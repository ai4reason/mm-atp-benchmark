...
thf(cfnemeet1_conj,conjecture,(! [XA2:($i > $o)] : (! [XS:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [XX:($i > $o)] : (! [Xy1:$i] : ((cw3a @ (cwcel @ XX @ (XV @ Xy1)) @ (cwral @ (^ [Xy1:$i] : (XX = (ccuni @ (ccv @ Xy1)))) @ (^ [Xy1:$i] : XS)) @ (cwcel @ XA2 @ XS)) => (cwbr @ (ccin @ (ccpw @ XX) @ (cciin @ (^ [Xt:$i] : XS) @ (^ [Xt:$i] : (ccfv @ (ccv @ Xt) @ cctg)))) @ XA2 @ ccfne)))))))).
