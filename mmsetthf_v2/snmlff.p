...
thf(csnmlff_conj,conjecture,(! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : (! [XR:($i > ($i > $o))] : (! [XF:($i > ($i > ($i > $o)))] : ((! [Xk:$i] : (! [Xn:$i] : ((XF @ Xk @ Xn) = (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (cco @ (ccfv @ (ccrab @ (^ [Xk:$i] : ((ccfv @ (cco @ (cco @ (XA2 @ Xk) @ (cco @ (XR @ Xk) @ (ccv @ Xk) @ ccexp) @ ccmul) @ (XR @ Xk) @ ccmo) @ ccfl) = (XB2 @ Xk))) @ (^ [Xk:$i] : (cco @ cc1 @ (ccv @ Xn) @ ccfz))) @ cchash) @ (ccv @ Xn) @ ccdiv)))))) => (! [Xk:$i] : (! [Xn:$i] : (cwf @ ccn @ (cco @ ccc0 @ cc1 @ ccicc) @ (XF @ Xk @ Xn)))))))))).
