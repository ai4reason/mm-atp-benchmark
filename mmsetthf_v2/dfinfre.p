...
thf(cdfinfre_conj,conjecture,(! [XA2:($i > $o)] : ((cwss @ XA2 @ ccr) => ((ccinf @ XA2 @ ccr @ cclt) = (ccuni @ (ccrab @ (^ [Xx3:$i] : ((cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xx3) @ (ccv @ Xy1) @ ccle)) @ (^ [Xy1:$i] : XA2)) & (cwral @ (^ [Xy1:$i] : ((cwbr @ (ccv @ Xx3) @ (ccv @ Xy1) @ cclt) => (cwrex @ (^ [Xz:$i] : (cwbr @ (ccv @ Xz) @ (ccv @ Xy1) @ cclt)) @ (^ [Xz:$i] : XA2)))) @ (^ [Xy1:$i] : ccr)))) @ (^ [Xx3:$i] : ccr))))))).
