...
thf(cf1omptsnlem_conj,conjecture,(! [XA2:($i > $o)] : (! [XR:($i > $o)] : (! [XF:($i > ($i > $o))] : ((! [Xu:$i] : ((XF @ Xu) = (ccmpt @ (^ [Xx3:$i] : XA2) @ (^ [Xx3:$i] : (ccsn @ (ccv @ Xx3)))))) => ((XR = (ccab @ (^ [Xu:$i] : (cwrex @ (^ [Xx3:$i] : ((ccv @ Xu) = (ccsn @ (ccv @ Xx3)))) @ (^ [Xx3:$i] : XA2))))) => (! [Xu:$i] : (cwf1o @ XA2 @ XR @ (XF @ Xu))))))))).
