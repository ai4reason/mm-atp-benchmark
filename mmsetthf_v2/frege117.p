...
thf(afrege116_ax,axiom,(! [XR:($i > $o)] : (! [XU:($i > ($i > ($i > $o)))] : (! [XX:($i > $o)] : ((! [Xa:$i] : (! [Xb:$i] : (cwcel @ XX @ (XU @ Xa @ Xb)))) => ((cwfun @ (cccnv @ (cccnv @ XR))) => (! [Xb:$i] : ((cwbr @ (ccv @ Xb) @ XX @ XR) => (! [Xa:$i] : ((cwbr @ (ccv @ Xb) @ (ccv @ Xa) @ XR) => ((ccv @ Xa) = XX))))))))))).
