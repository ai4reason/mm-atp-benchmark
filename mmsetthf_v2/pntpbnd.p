...
thf(apntrsumbnd2_ax,axiom,(! [XR:($i > ($i > $o))] : ((! [Xa:$i] : ((XR @ Xa) = (ccmpt @ (^ [Xa:$i] : ccrp) @ (^ [Xa:$i] : (cco @ (ccfv @ (ccv @ Xa) @ ccchp) @ (ccv @ Xa) @ ccmin))))) => (! [Xa:$i] : (cwrex @ (^ [Xc:$i] : (cwral @ (^ [Xk:$i] : (cwral @ (^ [Xm:$i] : (cwbr @ (ccfv @ (ccsu @ (cco @ (ccv @ Xk) @ (ccv @ Xm) @ ccfz) @ (^ [Xn:$i] : (cco @ (ccfv @ (ccv @ Xn) @ (XR @ Xa)) @ (cco @ (ccv @ Xn) @ (cco @ (ccv @ Xn) @ cc1 @ ccaddc) @ ccmul) @ ccdiv))) @ ccabs) @ (ccv @ Xc) @ ccle)) @ (^ [Xm:$i] : ccz))) @ (^ [Xk:$i] : ccn))) @ (^ [Xc:$i] : ccrp)))))).
