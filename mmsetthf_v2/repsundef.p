...
thf(adf_reps_ax,axiom,(ccreps = (ccmpt2 @ (^ [Xs1:$i] : (^ [Xn:$i] : ccvv)) @ (^ [Xs1:$i] : (^ [Xn:$i] : ccn0)) @ (^ [Xs1:$i] : (^ [Xn:$i] : (ccmpt @ (^ [Xx3:$i] : (cco @ ccc0 @ (ccv @ Xn) @ ccfzo)) @ (^ [Xx3:$i] : (ccv @ Xs1)))))))).
...
thf(crepsundef_conj,conjecture,(! [XS:($i > $o)] : (! [XN:($i > $o)] : ((cwnel @ XN @ ccn0) => ((cco @ XS @ XN @ ccreps) = cc0))))).
