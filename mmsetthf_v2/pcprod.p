...
thf(cpcprod_conj,conjecture,(! [XF:($i > ($i > $o))] : (! [XN:($i > $o)] : ((! [Xn:$i] : ((XF @ Xn) = (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccif @ (cwcel @ (ccv @ Xn) @ ccprime) @ (cco @ (ccv @ Xn) @ (cco @ (ccv @ Xn) @ XN @ ccpc) @ ccexp) @ cc1))))) => (! [Xn:$i] : ((cwcel @ XN @ ccn) => ((ccfv @ XN @ (ccseq @ ccmul @ (XF @ Xn) @ cc1)) = XN))))))).
