...
thf(adf_coe1_ax,axiom,(ccco1 = (ccmpt @ (^ [Xf1:$i] : ccvv) @ (^ [Xf1:$i] : (ccmpt @ (^ [Xn:$i] : ccn0) @ (^ [Xn:$i] : (ccfv @ (ccxp @ cc1o @ (ccsn @ (ccv @ Xn))) @ (ccv @ Xf1)))))))).
...
thf(ccoe1fval_conj,conjecture,(! [XA2:($i > ($i > $o))] : (! [XF:($i > $o)] : (! [XV:($i > ($i > $o))] : ((! [Xn:$i] : ((XA2 @ Xn) = (ccfv @ XF @ ccco1))) => (! [Xn:$i] : ((cwcel @ XF @ (XV @ Xn)) => ((XA2 @ Xn) = (ccmpt @ (^ [Xn:$i] : ccn0) @ (^ [Xn:$i] : (ccfv @ (ccxp @ cc1o @ (ccsn @ (ccv @ Xn))) @ XF))))))))))).
