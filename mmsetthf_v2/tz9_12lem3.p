...
thf(atz9_12lem1_ax,axiom,(! [XA2:($i > $o)] : (! [XF:($i > ($i > ($i > $o)))] : ((cwcel @ XA2 @ ccvv) => ((! [Xz:$i] : (! [Xv:$i] : ((XF @ Xz @ Xv) = (ccmpt @ (^ [Xz:$i] : ccvv) @ (^ [Xz:$i] : (ccint @ (ccrab @ (^ [Xv:$i] : (cwcel @ (ccv @ Xz) @ (ccfv @ (ccv @ Xv) @ ccr1))) @ (^ [Xv:$i] : ccon0)))))))) => (! [Xz:$i] : (! [Xv:$i] : (cwss @ (ccima @ (XF @ Xz @ Xv) @ XA2) @ ccon0)))))))).
