...
thf(cdfnbgr3_conj,conjecture,(! [XG:($i > ($i > $o))] : (! [XI:($i > $o)] : (! [XN:($i > $o)] : (! [XV:($i > ($i > $o))] : ((! [Xi:$i] : ((XV @ Xi) = (ccfv @ (XG @ Xi) @ ccvtx))) => ((! [Xi:$i] : (XI = (ccfv @ (XG @ Xi) @ cciedg))) => (! [Xi:$i] : (((cwcel @ XN @ (XV @ Xi)) & (cwfun @ XI)) => ((cco @ (XG @ Xi) @ XN @ ccnbgr) = (ccrab @ (^ [Xn:$i] : (cwrex @ (^ [Xi:$i] : (cwss @ (ccpr @ XN @ (ccv @ Xn)) @ (ccfv @ (ccv @ Xi) @ XI))) @ (^ [Xi:$i] : (ccdm @ XI)))) @ (^ [Xn:$i] : (ccdif @ (XV @ Xi) @ (ccsn @ XN)))))))))))))).
