...
thf(cpclem_conj,conjecture,(! [XA2:($i > ($i > $o))] : (! [XP:($i > $o)] : (! [XN:($i > $o)] : ((! [Xn:$i] : ((XA2 @ Xn) = (ccrab @ (^ [Xn:$i] : (cwbr @ (cco @ XP @ (ccv @ Xn) @ ccexp) @ XN @ ccdvds)) @ (^ [Xn:$i] : ccn0)))) => (! [Xn:$i] : (((cwcel @ XP @ (ccfv @ cc2 @ ccuz)) & ((cwcel @ XN @ ccz) & (cwne @ XN @ ccc0))) => (cw3a @ (cwss @ (XA2 @ Xn) @ ccz) @ (cwne @ (XA2 @ Xn) @ cc0) @ (cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccle)) @ (^ [Xy1:$i] : (XA2 @ Xn)))) @ (^ [Xx3:$i] : ccz)))))))))).
