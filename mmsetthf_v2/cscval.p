...
thf(adf_csc_ax,axiom,(cccsc = (ccmpt @ (^ [Xx3:$i] : (ccrab @ (^ [Xy1:$i] : (cwne @ (ccfv @ (ccv @ Xy1) @ ccsin) @ ccc0)) @ (^ [Xy1:$i] : ccc))) @ (^ [Xx3:$i] : (cco @ cc1 @ (ccfv @ (ccv @ Xx3) @ ccsin) @ ccdiv))))).
thf(ccop_tp,type,(ccop : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(aovex_ax,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XF:($i > $o)] : (cwcel @ (cco @ XA2 @ XB2 @ XF) @ ccvv))))).
thf(ccscval_conj,conjecture,(! [XA2:($i > $o)] : (((cwcel @ XA2 @ ccc) & (cwne @ (ccfv @ XA2 @ ccsin) @ ccc0)) => ((ccfv @ XA2 @ cccsc) = (cco @ cc1 @ (ccfv @ XA2 @ ccsin) @ ccdiv))))).
