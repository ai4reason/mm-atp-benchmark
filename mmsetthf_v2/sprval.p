...
thf(adf_spr_ax,axiom,(ccspr = (ccmpt @ (^ [Xv:$i] : ccvv) @ (^ [Xv:$i] : (ccab @ (^ [Xp:$i] : (cwrex @ (^ [Xa:$i] : (cwrex @ (^ [Xb:$i] : ((ccv @ Xp) = (ccpr @ (ccv @ Xa) @ (ccv @ Xb)))) @ (^ [Xb:$i] : (ccv @ Xv)))) @ (^ [Xa:$i] : (ccv @ Xv))))))))).
...
thf(aeueq_ax,axiom,(! [XA2:($i > $o)] : ((cwcel @ XA2 @ ccvv) <=> (cweu @ (^ [Xx3:$i] : ((ccv @ Xx3) = XA2)))))).
thf(cwmo_tp,type,(cwmo : (($i > $o) > $o))).
thf(aeuabex_ax,axiom,(! [Xph:($i > $o)] : ((cweu @ (^ [Xx3:$i] : (Xph @ Xx3))) => (cwcel @ (ccab @ (^ [Xx3:$i] : (Xph @ Xx3))) @ ccvv)))).
thf(cciun_tp,type,(cciun : (($i > ($i > $o)) > (($i > ($i > $o)) > ($i > $o))))).
thf(cwsb_tp,type,(cwsb : (($i > $o) > ($i > $o)))).
thf(aabrexex2g_ax,axiom,(! [Xph:($i > ($i > $o))] : (! [XA2:($i > $o)] : (! [XV:($i > $o)] : (! [XW:($i > $o)] : (((cwcel @ XA2 @ XV) & (cwral @ (^ [Xx3:$i] : (cwcel @ (ccab @ (^ [Xy1:$i] : (Xph @ Xx3 @ Xy1))) @ XW)) @ (^ [Xx3:$i] : XA2))) => (cwcel @ (ccab @ (^ [Xy1:$i] : (cwrex @ (^ [Xx3:$i] : (Xph @ Xx3 @ Xy1)) @ (^ [Xx3:$i] : XA2)))) @ ccvv))))))).
thf(csprval_conj,conjecture,(! [XV:($i > $o)] : (! [XW:($i > $o)] : ((cwcel @ XV @ XW) => ((ccfv @ XV @ ccspr) = (ccab @ (^ [Xp:$i] : (cwrex @ (^ [Xa:$i] : (cwrex @ (^ [Xb:$i] : ((ccv @ Xp) = (ccpr @ (ccv @ Xa) @ (ccv @ Xb)))) @ (^ [Xb:$i] : XV))) @ (^ [Xa:$i] : XV))))))))).
