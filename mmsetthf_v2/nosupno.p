...
thf(adford5_ax,axiom,(! [XA2:($i > $o)] : ((cword @ XA2) <=> ((cwss @ XA2 @ ccon0) & (cwtr @ XA2))))).
...
thf(anoprefixmo_ax,axiom,(! [XA2:($i > $o)] : (! [XG:($i > $o)] : ((cwss @ XA2 @ ccsur) => (cwmo @ (^ [Xx3:$i] : (cwrex @ (^ [Xu:$i] : (cw3a @ (cwcel @ XG @ (ccdm @ (ccv @ Xu))) @ (cwral @ (^ [Xv:$i] : ((~ (cwbr @ (ccv @ Xv) @ (ccv @ Xu) @ ccslt)) => ((ccres @ (ccv @ Xu) @ (ccsuc @ XG)) = (ccres @ (ccv @ Xv) @ (ccsuc @ XG))))) @ (^ [Xv:$i] : XA2)) @ ((ccfv @ XG @ (ccv @ Xu)) = (ccv @ Xx3)))) @ (^ [Xu:$i] : XA2)))))))).
