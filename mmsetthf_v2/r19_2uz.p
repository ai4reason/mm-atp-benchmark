...
thf(cr19_2uz_conj,conjecture,(! [Xph:($i > $o)] : (! [XM:($i > ($i > $o))] : (! [XZ:($i > $o)] : ((! [Xk:$i] : (XZ = (ccfv @ (XM @ Xk) @ ccuz))) => ((cwrex @ (^ [Xj:$i] : (cwral @ (^ [Xk:$i] : (Xph @ Xk)) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xj) @ ccuz)))) @ (^ [Xj:$i] : XZ)) => (cwrex @ (^ [Xk:$i] : (Xph @ Xk)) @ (^ [Xk:$i] : XZ)))))))).
