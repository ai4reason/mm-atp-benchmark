...
thf(aprnmadd_ax,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (((cwcel @ XA2 @ ccnp) & (cwcel @ XB2 @ XA2)) => (? [Xx3:$i] : (cwcel @ (cco @ XB2 @ (ccv @ Xx3) @ ccplq) @ XA2)))))).
...
thf(cltexprlem1_conj,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > ($i > $o))] : ((! [Xy1:$i] : ((XC @ Xy1) = (ccab @ (^ [Xx3:$i] : (? [Xy1:$i] : ((~ (cwcel @ (ccv @ Xy1) @ XA2)) & (cwcel @ (cco @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccplq) @ XB2))))))) => (! [Xy1:$i] : ((cwcel @ XB2 @ ccnp) => ((cwpss @ XA2 @ XB2) => (cwne @ (XC @ Xy1) @ cc0))))))))).
