...
thf(adf_grp_ax,axiom,(ccgrp = (ccrab @ (^ [Xg1:$i] : (cwral @ (^ [Xa:$i] : (cwrex @ (^ [Xm:$i] : ((cco @ (ccv @ Xm) @ (ccv @ Xa) @ (ccfv @ (ccv @ Xg1) @ ccplusg)) = (ccfv @ (ccv @ Xg1) @ cc0g))) @ (^ [Xm:$i] : (ccfv @ (ccv @ Xg1) @ ccbs)))) @ (^ [Xa:$i] : (ccfv @ (ccv @ Xg1) @ ccbs)))) @ (^ [Xg1:$i] : ccmnd)))).
thf(cisgrp_conj,conjecture,(! [XB2:($i > $o)] : (! [Xc_pl:($i > ($i > ($i > $o)))] : (! [XG:($i > $o)] : (! [Xc_0:($i > ($i > ($i > $o)))] : ((XB2 = (ccfv @ XG @ ccbs)) => ((! [Xm:$i] : (! [Xa:$i] : ((Xc_pl @ Xm @ Xa) = (ccfv @ XG @ ccplusg)))) => ((! [Xm:$i] : (! [Xa:$i] : ((Xc_0 @ Xm @ Xa) = (ccfv @ XG @ cc0g)))) => ((cwcel @ XG @ ccgrp) <=> ((cwcel @ XG @ ccmnd) & (cwral @ (^ [Xa:$i] : (cwrex @ (^ [Xm:$i] : ((cco @ (ccv @ Xm) @ (ccv @ Xa) @ (Xc_pl @ Xm @ Xa)) = (Xc_0 @ Xm @ Xa))) @ (^ [Xm:$i] : XB2))) @ (^ [Xa:$i] : XB2)))))))))))).
