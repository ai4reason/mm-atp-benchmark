...
thf(covolgelb_conj,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XS:($i > ($i > $o))] : ((! [Xg1:$i] : ((XS @ Xg1) = (ccseq @ ccaddc @ (cccom @ (cccom @ ccabs @ ccmin) @ (ccv @ Xg1)) @ cc1))) => ((cw3a @ (cwss @ XA2 @ ccr) @ (cwcel @ (ccfv @ XA2 @ ccovol) @ ccr) @ (cwcel @ XB2 @ ccrp)) => (cwrex @ (^ [Xg1:$i] : ((cwss @ XA2 @ (ccuni @ (ccrn @ (cccom @ ccioo @ (ccv @ Xg1))))) & (cwbr @ (ccsup @ (ccrn @ (XS @ Xg1)) @ ccxr @ cclt) @ (cco @ (ccfv @ XA2 @ ccovol) @ XB2 @ ccaddc) @ ccle))) @ (^ [Xg1:$i] : (cco @ (ccin @ ccle @ (ccxp @ ccr @ ccr)) @ ccn @ ccmap))))))))).
