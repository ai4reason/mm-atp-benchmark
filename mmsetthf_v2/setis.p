thf(ccsetrecs_tp,type,(ccsetrecs : (($i > $o) > ($i > $o)))).
thf(ccv_tp,type,(ccv : ($i > ($i > $o)))).
thf(cwral_tp,type,(cwral : (($i > $o) > (($i > ($i > $o)) > $o)))).
thf(ccfv_tp,type,(ccfv : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(cwcel_tp,type,(cwcel : (($i > $o) > (($i > $o) > $o)))).
thf(ccab_tp,type,(ccab : (($i > $o) > ($i > $o)))).
thf(cwss_tp,type,(cwss : (($i > $o) > (($i > $o) > $o)))).
thf(ampbidi_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (! [Xth:$o] : ((Xth => (Xph => Xps)) => ((Xph => (Xps <=> Xch)) => (Xth => (Xph => Xch))))))))).
thf(asseld_ax,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : ((Xph => (cwss @ XA2 @ XB2)) => (Xph => ((cwcel @ XC @ XA2) => (cwcel @ XC @ XB2))))))))).
thf(asetrec2v_ax,axiom,(! [Xph:($i > $o)] : (! [XB2:($i > ($i > $o))] : (! [XC:($i > $o)] : (! [XF:($i > $o)] : ((! [Xa:$i] : ((XB2 @ Xa) = (ccsetrecs @ XF))) => ((! [Xa:$i] : ((Xph @ Xa) => (! [Xa:$i] : ((cwss @ (ccv @ Xa) @ XC) => (cwss @ (ccfv @ (ccv @ Xa) @ XF) @ XC))))) => (! [Xa:$i] : ((Xph @ Xa) => (cwss @ (XB2 @ Xa) @ XC)))))))))).
thf(asylibr_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((Xph => Xps) => ((Xch <=> Xps) => (Xph => Xch))))))).
thf(aalbii_ax,axiom,(! [Xph:($i > $o)] : (! [Xps:($i > $o)] : ((! [Xx3:$i] : ((Xph @ Xx3) <=> (Xps @ Xx3))) => ((! [Xx3:$i] : (Xph @ Xx3)) <=> (! [Xx3:$i] : (Xps @ Xx3))))))).
thf(aimbi12i_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (! [Xth:$o] : ((Xph <=> Xps) => ((Xch <=> Xth) => ((Xph => Xch) <=> (Xps => Xth))))))))).
thf(assabral_ax,axiom,(! [Xph:($i > $o)] : (! [XA2:($i > $o)] : ((cwss @ XA2 @ (ccab @ (^ [Xx3:$i] : (Xph @ Xx3)))) <=> (cwral @ (^ [Xx3:$i] : (Xph @ Xx3)) @ (^ [Xx3:$i] : XA2)))))).
thf(aelabg_ax,axiom,(! [Xph:($i > $o)] : (! [Xps:$o] : (! [XA2:($i > $o)] : (! [XV:($i > ($i > $o))] : ((! [Xx3:$i] : (((ccv @ Xx3) = XA2) => ((Xph @ Xx3) <=> Xps))) => (! [Xx3:$i] : ((cwcel @ XA2 @ (XV @ Xx3)) => ((cwcel @ XA2 @ (ccab @ (^ [Xx3:$i] : (Xph @ Xx3)))) <=> Xps))))))))).
thf(csetis_conj,conjecture,(! [Xph:($i > ($i > $o))] : (! [Xps:($i > $o)] : (! [Xch:($i > $o)] : (! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > ($i > $o)))] : (! [XF:($i > $o)] : ((! [Xa:$i] : (! [Xb:$i] : ((XB2 @ Xa @ Xb) = (ccsetrecs @ XF)))) => ((! [Xa:$i] : (! [Xb:$i] : (((ccv @ Xb) = (XA2 @ Xa)) => ((Xps @ Xb) <=> (Xch @ Xa))))) => ((! [Xa:$i] : (! [Xb:$i] : ((Xph @ Xa @ Xb) => (! [Xa:$i] : ((cwral @ (^ [Xb:$i] : (Xps @ Xb)) @ (^ [Xb:$i] : (ccv @ Xa))) => (cwral @ (^ [Xb:$i] : (Xps @ Xb)) @ (^ [Xb:$i] : (ccfv @ (ccv @ Xa) @ XF)))))))) => (! [Xa:$i] : (! [Xb:$i] : ((Xph @ Xa @ Xb) => ((cwcel @ (XA2 @ Xa) @ (XB2 @ Xa @ Xb)) => (Xch @ Xa))))))))))))))).
