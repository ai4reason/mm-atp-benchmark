...
thf(clmnn_conj,conjecture,(! [Xph:$o] : (! [XD:($i > $o)] : (! [XP:($i > $o)] : (! [XF:($i > $o)] : (! [XJ:($i > ($i > $o))] : (! [XX:($i > $o)] : ((! [Xk:$i] : ((XJ @ Xk) = (ccfv @ XD @ ccmopn))) => ((Xph => (cwcel @ XD @ (ccfv @ XX @ ccxmt))) => ((Xph => (cwcel @ XP @ XX)) => ((Xph => (cwf @ ccn @ XX @ XF)) => ((! [Xk:$i] : ((Xph & (cwcel @ (ccv @ Xk) @ ccn)) => (cwbr @ (cco @ (ccfv @ (ccv @ Xk) @ XF) @ XP @ XD) @ (cco @ cc1 @ (ccv @ Xk) @ ccdiv) @ cclt))) => (! [Xk:$i] : (Xph => (cwbr @ XF @ XP @ (ccfv @ (XJ @ Xk) @ cclm)))))))))))))))).
