...
thf(cinfregelb_b_conj,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : (! [Xy1:$i] : (((cw3a @ (cwss @ XA2 @ ccr) @ (cwne @ XA2 @ cc0) @ (cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xx3) @ (ccv @ Xy1) @ ccle)) @ (^ [Xy1:$i] : XA2))) @ (^ [Xx3:$i] : ccr))) & (cwcel @ (XB2 @ Xy1) @ ccr)) => ((cwbr @ (XB2 @ Xy1) @ (ccinf @ XA2 @ ccr @ cclt) @ ccle) <=> (cwral @ (^ [Xx3:$i] : (cwbr @ (XB2 @ Xy1) @ (ccv @ Xx3) @ ccle)) @ (^ [Xx3:$i] : XA2)))))))).
