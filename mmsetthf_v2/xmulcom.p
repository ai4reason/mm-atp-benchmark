thf(cwcel_tp,type,(cwcel : (($i > $o) > (($i > $o) > $o)))).
thf(ccxr_tp,type,(ccxr : ($i > $o))).
thf(cco_tp,type,(cco : (($i > $o) > (($i > $o) > (($i > $o) > ($i > $o)))))).
thf(ccxmu_tp,type,(ccxmu : ($i > $o))).
thf(ccif_tp,type,(ccif : ($o > (($i > $o) > (($i > $o) > ($i > $o)))))).
thf(ccc0_tp,type,(ccc0 : ($i > $o))).
thf(cwbr_tp,type,(cwbr : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(cclt_tp,type,(cclt : ($i > $o))).
thf(ccpnf_tp,type,(ccpnf : ($i > $o))).
thf(ccmnf_tp,type,(ccmnf : ($i > $o))).
thf(ccmul_tp,type,(ccmul : ($i > $o))).
thf(ccr_tp,type,(ccr : ($i > $o))).
thf(a_3eqtr4d_ax,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : (! [XD:($i > $o)] : ((Xph => (XA2 = XB2)) => ((Xph => (XC = XA2)) => ((Xph => (XD = XB2)) => (Xph => (XC = XD))))))))))).
thf(aeqtrd_ax,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : ((Xph => (XA2 = XB2)) => ((Xph => (XB2 = XC)) => (Xph => (XA2 = XC))))))))).
thf(aifeq2da_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : (((Xph & (~ Xps)) => (XA2 = XB2)) => (Xph => ((ccif @ Xps @ XC @ XA2) = (ccif @ Xps @ XC @ XB2)))))))))).
thf(ccc_tp,type,(ccc : ($i > $o))).
thf(amulcomd_ax,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : ((Xph => (cwcel @ XA2 @ ccc)) => ((Xph => (cwcel @ XB2 @ ccc)) => (Xph => ((cco @ XA2 @ XB2 @ ccmul) = (cco @ XB2 @ XA2 @ ccmul))))))))).
thf(arecnd_ax,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : ((Xph => (cwcel @ XA2 @ ccr)) => (Xph => (cwcel @ XA2 @ ccc)))))).
thf(cwor_tp,type,(cwor : (($i > $o) > (($i > $o) > $o)))).
thf(cw3o_tp,type,(cw3o : ($o > ($o > ($o > $o))))).
thf(axmullem_ax,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : ((((((cwcel @ XA2 @ ccxr) & (cwcel @ XB2 @ ccxr)) & (~ ((XA2 = ccc0) | (XB2 = ccc0)))) & (~ ((((cwbr @ ccc0 @ XB2 @ cclt) & (XA2 = ccpnf)) | ((cwbr @ XB2 @ ccc0 @ cclt) & (XA2 = ccmnf))) | (((cwbr @ ccc0 @ XA2 @ cclt) & (XB2 = ccpnf)) | ((cwbr @ XA2 @ ccc0 @ cclt) & (XB2 = ccmnf)))))) & (~ ((((cwbr @ ccc0 @ XB2 @ cclt) & (XA2 = ccmnf)) | ((cwbr @ XB2 @ ccc0 @ cclt) & (XA2 = ccpnf))) | (((cwbr @ ccc0 @ XA2 @ cclt) & (XB2 = ccmnf)) | ((cwbr @ XA2 @ ccc0 @ cclt) & (XB2 = ccpnf)))))) => (cwcel @ XA2 @ ccr))))).
thf(asyl2anb_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (! [Xth:$o] : (! [Xta:$o] : ((Xph <=> Xps) => ((Xta <=> Xch) => (((Xps & Xch) => Xth) => ((Xph & Xta) => Xth)))))))))).
thf(aanbi12i_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (! [Xth:$o] : ((Xph <=> Xps) => ((Xch <=> Xth) => ((Xph & Xch) <=> (Xps & Xth))))))))).
thf(aancom_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : ((Xph & Xps) <=> (Xps & Xph))))).
thf(anotbii_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : ((Xph <=> Xps) => ((~ Xph) <=> (~ Xps)))))).
thf(aorcom_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : ((Xph | Xps) <=> (Xps | Xph))))).
thf(aifbid_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : ((Xph => (Xps <=> Xch)) => (Xph => ((ccif @ Xps @ XA2 @ XB2) = (ccif @ Xch @ XA2 @ XB2)))))))))).
thf(aa1i_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (Xph => (Xps => Xph))))).
thf(ccv_tp,type,(ccv : ($i > ($i > $o)))).
thf(axmulval_ax,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (((cwcel @ XA2 @ ccxr) & (cwcel @ XB2 @ ccxr)) => ((cco @ XA2 @ XB2 @ ccxmu) = (ccif @ ((XA2 = ccc0) | (XB2 = ccc0)) @ ccc0 @ (ccif @ ((((cwbr @ ccc0 @ XB2 @ cclt) & (XA2 = ccpnf)) | ((cwbr @ XB2 @ ccc0 @ cclt) & (XA2 = ccmnf))) | (((cwbr @ ccc0 @ XA2 @ cclt) & (XB2 = ccpnf)) | ((cwbr @ XA2 @ ccc0 @ cclt) & (XB2 = ccmnf)))) @ ccpnf @ (ccif @ ((((cwbr @ ccc0 @ XB2 @ cclt) & (XA2 = ccmnf)) | ((cwbr @ XB2 @ ccc0 @ cclt) & (XA2 = ccpnf))) | (((cwbr @ ccc0 @ XA2 @ cclt) & (XB2 = ccmnf)) | ((cwbr @ XA2 @ ccc0 @ cclt) & (XB2 = ccpnf)))) @ ccmnf @ (cco @ XA2 @ XB2 @ ccmul))))))))).
thf(aancoms_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (((Xph & Xps) => Xch) => ((Xps & Xph) => Xch)))))).
thf(cxmulcom_conj,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (((cwcel @ XA2 @ ccxr) & (cwcel @ XB2 @ ccxr)) => ((cco @ XA2 @ XB2 @ ccxmu) = (cco @ XB2 @ XA2 @ ccxmu)))))).
