...
thf(adf_acn_ax,axiom,(! [XA2:($i > $o)] : ((cwacn @ XA2) = (ccab @ (^ [Xx3:$i] : ((cwcel @ XA2 @ ccvv) & (cwral @ (^ [Xf1:$i] : (? [Xg1:$i] : (cwral @ (^ [Xy1:$i] : (cwcel @ (ccfv @ (ccv @ Xy1) @ (ccv @ Xg1)) @ (ccfv @ (ccv @ Xy1) @ (ccv @ Xf1)))) @ (^ [Xy1:$i] : XA2)))) @ (^ [Xf1:$i] : (cco @ (ccdif @ (ccpw @ (ccv @ Xx3)) @ (ccsn @ cc0)) @ XA2 @ ccmap))))))))).
thf(cacnrcl_conj,conjecture,(! [XA2:($i > $o)] : (! [XX:($i > $o)] : ((cwcel @ XX @ (cwacn @ XA2)) => (cwcel @ XA2 @ ccvv))))).
