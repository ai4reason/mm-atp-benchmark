...
thf(ccfiluexsm_conj,conjecture,(! [XU:($i > ($i > $o))] : (! [XF:($i > $o)] : (! [XV:($i > $o)] : (! [XX:($i > ($i > $o))] : (! [Xa:$i] : ((cw3a @ (cwcel @ (XU @ Xa) @ (ccfv @ (XX @ Xa) @ ccust)) @ (cwcel @ XF @ (ccfv @ (XU @ Xa) @ cccfilu)) @ (cwcel @ XV @ (XU @ Xa))) => (cwrex @ (^ [Xa:$i] : (cwss @ (ccxp @ (ccv @ Xa) @ (ccv @ Xa)) @ XV)) @ (^ [Xa:$i] : XF))))))))).
