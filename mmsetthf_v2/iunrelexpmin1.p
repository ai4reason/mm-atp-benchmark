...
thf(ciunrelexpmin1_conj,conjecture,(! [XC:($i > ($i > $o))] : (! [XR:($i > $o)] : (! [XN:($i > $o)] : (! [XV:($i > $o)] : ((! [Xs1:$i] : ((XC @ Xs1) = (ccmpt @ (^ [Xr:$i] : ccvv) @ (^ [Xr:$i] : (cciun @ (^ [Xn:$i] : XN) @ (^ [Xn:$i] : (cco @ (ccv @ Xr) @ (ccv @ Xn) @ ccrelexp))))))) => (((cwcel @ XR @ XV) & (XN = ccn)) => (! [Xs1:$i] : (((cwss @ XR @ (ccv @ Xs1)) & (cwss @ (cccom @ (ccv @ Xs1) @ (ccv @ Xs1)) @ (ccv @ Xs1))) => (cwss @ (ccfv @ XR @ (XC @ Xs1)) @ (ccv @ Xs1))))))))))).
