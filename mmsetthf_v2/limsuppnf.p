...
thf(alimsuppnflem_ax,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XF:($i > ($i > $o))] : ((cwnfc @ (^ [Xj:$i] : (XF @ Xj))) => ((Xph => (cwss @ XA2 @ ccr)) => ((! [Xj:$i] : (Xph => (cwf @ XA2 @ ccxr @ (XF @ Xj)))) => (! [Xj:$i] : (Xph => (((ccfv @ (XF @ Xj) @ cclsp) = ccpnf) <=> (cwral @ (^ [Xx3:$i] : (cwral @ (^ [Xk:$i] : (cwrex @ (^ [Xj:$i] : ((cwbr @ (ccv @ Xk) @ (ccv @ Xj) @ ccle) & (cwbr @ (ccv @ Xx3) @ (ccfv @ (ccv @ Xj) @ (XF @ Xj)) @ ccle))) @ (^ [Xj:$i] : XA2))) @ (^ [Xk:$i] : ccr))) @ (^ [Xx3:$i] : ccr)))))))))))).
