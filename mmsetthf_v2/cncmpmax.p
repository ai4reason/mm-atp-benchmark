...
thf(aubelsupr_ax,axiom,(! [XA2:($i > $o)] : (! [XU:($i > $o)] : ((cw3a @ (cwss @ XA2 @ ccr) @ (cwcel @ XU @ XA2) @ (cwral @ (^ [Xx3:$i] : (cwbr @ (ccv @ Xx3) @ XU @ ccle)) @ (^ [Xx3:$i] : XA2))) => (XU = (ccsup @ XA2 @ ccr @ cclt)))))).
...
thf(ccncmpmax_conj,conjecture,(! [Xph:$o] : (! [XT:($i > $o)] : (! [XF:($i > $o)] : (! [XJ:($i > $o)] : (! [XK:($i > $o)] : ((XT = (ccuni @ XJ)) => ((XK = (ccfv @ (ccrn @ ccioo) @ cctg)) => ((Xph => (cwcel @ XJ @ cccmp)) => ((Xph => (cwcel @ XF @ (cco @ XJ @ XK @ cccn))) => ((Xph => (cwne @ XT @ cc0)) => (Xph => (cw3a @ (cwcel @ (ccsup @ (ccrn @ XF) @ ccr @ cclt) @ (ccrn @ XF)) @ (cwcel @ (ccsup @ (ccrn @ XF) @ ccr @ cclt) @ ccr) @ (cwral @ (^ [Xt:$i] : (cwbr @ (ccfv @ (ccv @ Xt) @ XF) @ (ccsup @ (ccrn @ XF) @ ccr @ cclt) @ ccle)) @ (^ [Xt:$i] : XT))))))))))))))).
