...
thf(arelint_ax,axiom,(! [XA2:($i > $o)] : ((cwrex @ (^ [Xx3:$i] : (cwrel @ (ccv @ Xx3))) @ (^ [Xx3:$i] : XA2)) => (cwrel @ (ccint @ XA2))))).
thf(cclrellem_conj,conjecture,(! [Xph:$o] : (! [Xps:($i > $o)] : (! [Xch:$o] : (! [XX:($i > $o)] : (! [XY:($i > $o)] : ((Xph => (cwcel @ XY @ ccvv)) => ((Xph => (cwrel @ XX)) => ((! [Xx3:$i] : (((ccv @ Xx3) = (cccnv @ (cccnv @ XY))) => ((Xps @ Xx3) <=> Xch))) => ((Xph => (cwss @ XX @ XY)) => ((Xph => Xch) => (Xph => (cwrel @ (ccint @ (ccab @ (^ [Xx3:$i] : ((cwss @ XX @ (ccv @ Xx3)) & (Xps @ Xx3)))))))))))))))))).
