thf(cwcel_tp,type,(cwcel : (($i > $o) > (($i > $o) > $o)))).
thf(ccop_tp,type,(ccop : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(cc0_tp,type,(cc0 : ($i > $o))).
thf(ccupgr_tp,type,(ccupgr : ($i > $o))).
thf(ccvv_tp,type,(ccvv : ($i > $o))).
thf(ccfv_tp,type,(ccfv : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(ccv_tp,type,(ccv : ($i > ($i > $o)))).
thf(ccvtx_tp,type,(ccvtx : ($i > $o))).
thf(cciedg_tp,type,(cciedg : ($i > $o))).
thf(cwsbc_tp,type,(cwsbc : (($i > $o) > (($i > $o) > $o)))).
thf(agropeld_ax,axiom,(! [Xph:$o] : (! [XC:($i > $o)] : (! [XU:($i > ($i > $o))] : (! [XE:($i > $o)] : (! [XV:($i > $o)] : (! [XW:($i > ($i > $o))] : ((Xph => (! [Xg1:$i] : ((((ccfv @ (ccv @ Xg1) @ ccvtx) = XV) & ((ccfv @ (ccv @ Xg1) @ cciedg) = XE)) => (cwcel @ (ccv @ Xg1) @ XC)))) => ((! [Xg1:$i] : (Xph => (cwcel @ XV @ (XU @ Xg1)))) => ((! [Xg1:$i] : (Xph => (cwcel @ XE @ (XW @ Xg1)))) => (Xph => (cwcel @ (ccop @ XV @ XE) @ XC)))))))))))).
thf(aa1i_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (Xph => (Xps => Xph))))).
thf(aax_gen_ax,axiom,(! [Xph:($i > $o)] : ((! [Xx3:$i] : (Xph @ Xx3)) => (! [Xx3:$i] : (Xph @ Xx3))))).
thf(ccumgr_tp,type,(ccumgr : ($i > $o))).
thf(aupgr0e_ax,axiom,(! [Xph:$o] : (! [XG:($i > $o)] : (! [XW:($i > $o)] : ((Xph => (cwcel @ XG @ XW)) => ((Xph => ((ccfv @ XG @ cciedg) = cc0)) => (Xph => (cwcel @ XG @ ccupgr)))))))).
thf(avex_ax,axiom,(! [Xx3:$i] : (cwcel @ (ccv @ Xx3) @ ccvv))).
thf(asimpr_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : ((Xph & Xps) => Xps)))).
thf(aid_ax,axiom,(! [Xph:$o] : (Xph => Xph))).
thf(a_0ex_ax,axiom,(cwcel @ cc0 @ ccvv)).
thf(cupgr0eopALT_conj,conjecture,(! [XV:($i > $o)] : (! [XW:($i > $o)] : ((cwcel @ XV @ XW) => (cwcel @ (ccop @ XV @ cc0) @ ccupgr))))).
