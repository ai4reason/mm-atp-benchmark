...
thf(chlimconvi_conj,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XF:($i > $o)] : ((cwcel @ XA2 @ ccvv) => (((cwbr @ XF @ XA2 @ cchli) & (cwcel @ XB2 @ ccrp)) => (cwrex @ (^ [Xy1:$i] : (cwral @ (^ [Xz:$i] : (cwbr @ (ccfv @ (cco @ (ccfv @ (ccv @ Xz) @ XF) @ XA2 @ ccmv) @ ccno) @ XB2 @ cclt)) @ (^ [Xz:$i] : (ccfv @ (ccv @ Xy1) @ ccuz)))) @ (^ [Xy1:$i] : ccn)))))))).
