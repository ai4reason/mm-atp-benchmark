...
thf(aeqnbrtrd_ax,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : (! [XR:($i > $o)] : ((Xph => (XA2 = XB2)) => ((Xph => (~ (cwbr @ XB2 @ XC @ XR))) => (Xph => (~ (cwbr @ XA2 @ XC @ XR))))))))))).
...
thf(cnoprefixmo_conj,conjecture,(! [XA2:($i > $o)] : (! [XG:($i > $o)] : ((cwss @ XA2 @ ccsur) => (cwmo @ (^ [Xx3:$i] : (cwrex @ (^ [Xu:$i] : (cw3a @ (cwcel @ XG @ (ccdm @ (ccv @ Xu))) @ (cwral @ (^ [Xv:$i] : ((~ (cwbr @ (ccv @ Xv) @ (ccv @ Xu) @ ccslt)) => ((ccres @ (ccv @ Xu) @ (ccsuc @ XG)) = (ccres @ (ccv @ Xv) @ (ccsuc @ XG))))) @ (^ [Xv:$i] : XA2)) @ ((ccfv @ XG @ (ccv @ Xu)) = (ccv @ Xx3)))) @ (^ [Xu:$i] : XA2)))))))).
