...
thf(aresincncf_ax,axiom,(cwcel @ (ccres @ ccsin @ ccr) @ (cco @ ccr @ ccr @ cccncf))).
...
thf(cfourierdlem18_conj,conjecture,(! [Xph:$o] : (! [XS:($i > ($i > $o))] : (! [XN:($i > $o)] : ((Xph => (cwcel @ XN @ ccr)) => ((! [Xs1:$i] : ((XS @ Xs1) = (ccmpt @ (^ [Xs1:$i] : (cco @ (ccneg @ ccpi) @ ccpi @ ccicc)) @ (^ [Xs1:$i] : (ccfv @ (cco @ (cco @ XN @ (cco @ cc1 @ cc2 @ ccdiv) @ ccaddc) @ (ccv @ Xs1) @ ccmul) @ ccsin))))) => (! [Xs1:$i] : (Xph => (cwcel @ (XS @ Xs1) @ (cco @ (cco @ (ccneg @ ccpi) @ ccpi @ ccicc) @ ccr @ cccncf)))))))))).
