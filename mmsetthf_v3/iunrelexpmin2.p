...
thf(ciunrelexpmin2_conj,conjecture,(! [XC:($i > ($i > $o))] : (! [XR:($i > $o)] : (! [XN:($i > $o)] : (! [XV:($i > $o)] : ((! [Xs1:$i] : ((XC @ Xs1) = (ccmpt @ (^ [Xr:$i] : ccvv) @ (^ [Xr:$i] : (cciun @ (^ [Xn:$i] : XN) @ (^ [Xn:$i] : (cco @ (ccv @ Xr) @ (ccv @ Xn) @ ccrelexp))))))) => (((cwcel @ XR @ XV) & (XN = ccn0)) => (! [Xs1:$i] : (((cwss @ (ccres @ ccid @ (ccun @ (ccdm @ XR) @ (ccrn @ XR))) @ (ccv @ Xs1)) & (cwss @ XR @ (ccv @ Xs1)) & (cwss @ (cccom @ (ccv @ Xs1) @ (ccv @ Xs1)) @ (ccv @ Xs1))) => (cwss @ (ccfv @ XR @ (XC @ Xs1)) @ (ccv @ Xs1))))))))))).
