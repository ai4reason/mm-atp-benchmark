...
thf(adiveq1_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (((cwcel @ XA2 @ ccc) & (cwcel @ XB2 @ ccc) & (XB2 != ccc0)) => (((cco @ XA2 @ XB2 @ ccdiv) = cc1) <=> (XA2 = XB2)))))).
...
thf(coddprmdvds_conj,conjecture,(! [XK:($i > $o)] : (((cwcel @ XK @ ccn) & (~ (cwrex @ (^ [Xn:$i] : (XK = (cco @ cc2 @ (ccv @ Xn) @ ccexp))) @ (^ [Xn:$i] : ccn0)))) => (cwrex @ (^ [Xp:$i] : (cwbr @ (ccv @ Xp) @ XK @ ccdvds)) @ (^ [Xp:$i] : (ccdif @ ccprime @ (ccsn @ cc2))))))).
