...
thf(climsupval_conj,conjecture,(! [XF:($i > $o)] : (! [XG:($i > ($i > $o))] : (! [XV:($i > ($i > $o))] : ((! [Xk:$i] : ((XG @ Xk) = (ccmpt @ (^ [Xk:$i] : ccr) @ (^ [Xk:$i] : (ccsup @ (ccin @ (ccima @ XF @ (cco @ (ccv @ Xk) @ ccpnf @ ccico)) @ ccxr) @ ccxr @ cclt))))) => (! [Xk:$i] : ((cwcel @ XF @ (XV @ Xk)) => ((ccfv @ XF @ cclsp) = (ccinf @ (ccrn @ (XG @ Xk)) @ ccxr @ cclt))))))))).
