...
thf(aabsprodnn_thm,axiom,(! [Xph:$o] : (! [XP:($i > ($i > $o))] : (! [XZ:($i > $o)] : ((Xph => (cwss @ XZ @ ccz)) => ((Xph => (cwcel @ XZ @ ccfn)) => ((! [Xz:$i] : ((XP @ Xz) = (ccfv @ (ccprod @ (^ [Xz:$i] : XZ) @ (^ [Xz:$i] : (ccv @ Xz))) @ ccabs))) => ((Xph => (cwnel @ ccc0 @ XZ)) => (! [Xz:$i] : (Xph => (cwcel @ (XP @ Xz) @ ccn))))))))))).
...
thf(cfissn0dvds_conj,conjecture,(! [XZ:($i > $o)] : (((cwss @ XZ @ ccz) & (cwcel @ XZ @ ccfn) & (cwnel @ ccc0 @ XZ)) => (cwrex @ (^ [Xn:$i] : (cwral @ (^ [Xm:$i] : (cwbr @ (ccv @ Xm) @ (ccv @ Xn) @ ccdvds)) @ (^ [Xm:$i] : XZ))) @ (^ [Xn:$i] : ccn))))).
