...
thf(cnbuhgr2vtx1edgb_conj,conjecture,(! [XE:($i > ($i > $o))] : (! [XG:($i > $o)] : (! [XV:($i > $o)] : ((XV = (ccfv @ XG @ ccvtx)) => ((! [Xv:$i] : ((XE @ Xv) = (ccfv @ XG @ ccedg))) => (! [Xv:$i] : (((cwcel @ XG @ ccuhgr) & ((ccfv @ XV @ cchash) = cc2)) => ((cwcel @ XV @ (XE @ Xv)) <=> (cwral @ (^ [Xv:$i] : (cwral @ (^ [Xn:$i] : (cwcel @ (ccv @ Xn) @ (cco @ XG @ (ccv @ Xv) @ ccnbgr))) @ (^ [Xn:$i] : (ccdif @ XV @ (ccsn @ (ccv @ Xv)))))) @ (^ [Xv:$i] : XV))))))))))).
