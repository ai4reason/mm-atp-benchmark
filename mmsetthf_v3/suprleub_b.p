...
thf(csuprleub_b_conj,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : (! [Xy1:$i] : ((((cwss @ XA2 @ ccr) & (XA2 != cc0) & (cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccle)) @ (^ [Xy1:$i] : XA2))) @ (^ [Xx3:$i] : ccr))) & (cwcel @ (XB2 @ Xy1) @ ccr)) => ((cwbr @ (ccsup @ XA2 @ ccr @ cclt) @ (XB2 @ Xy1) @ ccle) <=> (cwral @ (^ [Xx3:$i] : (cwbr @ (ccv @ Xx3) @ (XB2 @ Xy1) @ ccle)) @ (^ [Xx3:$i] : XA2)))))))).
