...
thf(aicoval_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (((cwcel @ XA2 @ ccxr) & (cwcel @ XB2 @ ccxr)) => ((cco @ XA2 @ XB2 @ ccico) = (ccrab @ (^ [Xx3:$i] : ((cwbr @ XA2 @ (ccv @ Xx3) @ ccle) & (cwbr @ (ccv @ Xx3) @ XB2 @ cclt))) @ (^ [Xx3:$i] : ccxr))))))).
thf(celicores_conj,conjecture,(! [XA2:($i > $o)] : ((cwcel @ XA2 @ (ccrn @ (ccres @ ccico @ (ccxp @ ccr @ ccr)))) <=> (cwrex @ (^ [Xx3:$i] : (cwrex @ (^ [Xy1:$i] : (XA2 = (cco @ (ccv @ Xx3) @ (ccv @ Xy1) @ ccico))) @ (^ [Xy1:$i] : ccr))) @ (^ [Xx3:$i] : ccr))))).
