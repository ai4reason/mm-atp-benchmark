...
thf(aupgrres1_thm,axiom,(! [XS:($i > ($i > $o))] : (! [XE:($i > $o)] : (! [XF:($i > ($i > $o))] : (! [XG:($i > $o)] : (! [XN:($i > $o)] : (! [XV:($i > $o)] : ((XV = (ccfv @ XG @ ccvtx)) => ((XE = (ccfv @ XG @ ccedg)) => ((! [Xe:$i] : ((XF @ Xe) = (ccrab @ (^ [Xe:$i] : (cwnel @ XN @ (ccv @ Xe))) @ (^ [Xe:$i] : XE)))) => ((! [Xe:$i] : ((XS @ Xe) = (ccop @ (ccdif @ XV @ (ccsn @ XN)) @ (ccres @ ccid @ (XF @ Xe))))) => (! [Xe:$i] : (((cwcel @ XG @ ccupgr) & (cwcel @ XN @ XV)) => (cwcel @ (XS @ Xe) @ ccupgr)))))))))))))).
