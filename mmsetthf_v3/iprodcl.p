...
thf(ciprodcl_conj,conjecture,(! [Xph:$o] : (! [XA2:($i > ($i > $o))] : (! [XF:($i > $o)] : (! [XM:($i > $o)] : (! [XZ:($i > $o)] : ((XZ = (ccfv @ XM @ ccuz)) => ((Xph => (cwcel @ XM @ ccz)) => ((Xph => (cwrex @ (^ [Xn:$i] : (? [Xy1:$i] : (((ccv @ Xy1) != ccc0) & (cwbr @ (ccseq @ ccmul @ XF @ (ccv @ Xn)) @ (ccv @ Xy1) @ ccli)))) @ (^ [Xn:$i] : XZ))) => ((! [Xk:$i] : ((Xph & (cwcel @ (ccv @ Xk) @ XZ)) => ((ccfv @ (ccv @ Xk) @ XF) = (XA2 @ Xk)))) => ((! [Xk:$i] : ((Xph & (cwcel @ (ccv @ Xk) @ XZ)) => (cwcel @ (XA2 @ Xk) @ ccc))) => (Xph => (cwcel @ (ccprod @ (^ [Xk:$i] : XZ) @ (^ [Xk:$i] : (XA2 @ Xk))) @ ccc))))))))))))).
