...
thf(adfsmo2_thm,axiom,(! [XF:($i > $o)] : ((cwsmo @ XF) <=> ((cwf @ (ccdm @ XF) @ ccon0 @ XF) & (cword @ (ccdm @ XF)) & (cwral @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwcel @ (ccfv @ (ccv @ Xy1) @ XF) @ (ccfv @ (ccv @ Xx3) @ XF))) @ (^ [Xy1:$i] : (ccv @ Xx3)))) @ (^ [Xx3:$i] : (ccdm @ XF))))))).
...
thf(csmofvon2_conj,conjecture,(! [XB2:($i > $o)] : (! [XF:($i > $o)] : ((cwsmo @ XF) => (cwcel @ (ccfv @ XB2 @ XF) @ ccon0))))).
