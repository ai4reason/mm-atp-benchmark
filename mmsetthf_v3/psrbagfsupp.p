...
thf(cpsrbagfsupp_conj,conjecture,(! [XD:($i > ($i > $o))] : (! [XI:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [XX:($i > $o)] : ((! [Xh:$i] : ((XD @ Xh) = (ccrab @ (^ [Xh:$i] : (cwcel @ (ccima @ (cccnv @ (ccv @ Xh)) @ ccn) @ ccfn)) @ (^ [Xh:$i] : (cco @ ccn0 @ XI @ ccmap))))) => (! [Xh:$i] : (((cwcel @ XX @ (XD @ Xh)) & (cwcel @ XI @ (XV @ Xh))) => (cwbr @ XX @ ccc0 @ ccfsupp))))))))).
