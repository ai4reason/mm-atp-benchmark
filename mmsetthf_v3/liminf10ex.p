...
thf(ainfpr_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : (! [XR:($i > $o)] : (((cwor @ XA2 @ XR) & (cwcel @ XB2 @ XA2) & (cwcel @ XC @ XA2)) => ((ccinf @ (ccpr @ XB2 @ XC) @ XA2 @ XR) = (ccif @ (cwbr @ XB2 @ XC @ XR) @ XB2 @ XC)))))))).
...
thf(climinf10ex_conj,conjecture,(! [XF:($i > ($i > $o))] : ((! [Xn:$i] : ((XF @ Xn) = (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccif @ (cwbr @ cc2 @ (ccv @ Xn) @ ccdvds) @ ccc0 @ cc1))))) => (! [Xn:$i] : ((ccfv @ (XF @ Xn) @ cclsi) = ccc0))))).
