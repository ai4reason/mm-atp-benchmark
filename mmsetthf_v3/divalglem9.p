...
thf(cdivalglem9_conj,conjecture,(! [XD:($i > $o)] : (! [XR:($i > ($i > $o))] : (! [XS:($i > ($i > $o))] : (! [XN:($i > $o)] : ((cwcel @ XN @ ccz) => ((cwcel @ XD @ ccz) => ((XD != ccc0) => ((! [Xr:$i] : ((XS @ Xr) = (ccrab @ (^ [Xr:$i] : (cwbr @ XD @ (cco @ XN @ (ccv @ Xr) @ ccmin) @ ccdvds)) @ (^ [Xr:$i] : ccn0)))) => ((! [Xr:$i] : ((XR @ Xr) = (ccinf @ (XS @ Xr) @ ccr @ cclt))) => (! [Xr:$i] : (cwreu @ (^ [Xx3:$i] : (cwbr @ (ccv @ Xx3) @ (ccfv @ XD @ ccabs) @ cclt)) @ (^ [Xx3:$i] : (XS @ Xr)))))))))))))).
