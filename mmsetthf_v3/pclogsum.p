...
thf(apcprod_thm,axiom,(! [XF:($i > ($i > $o))] : (! [XN:($i > $o)] : ((! [Xn:$i] : ((XF @ Xn) = (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccif @ (cwcel @ (ccv @ Xn) @ ccprime) @ (cco @ (ccv @ Xn) @ (cco @ (ccv @ Xn) @ XN @ ccpc) @ ccexp) @ cc1))))) => (! [Xn:$i] : ((cwcel @ XN @ ccn) => ((ccfv @ XN @ (ccseq @ ccmul @ (XF @ Xn) @ cc1)) = XN))))))).
thf(cpclogsum_conj,conjecture,(! [XA2:($i > $o)] : ((cwcel @ XA2 @ ccn) => ((ccsu @ (ccin @ (cco @ cc1 @ XA2 @ ccfz) @ ccprime) @ (^ [Xp:$i] : (cco @ (cco @ (ccv @ Xp) @ XA2 @ ccpc) @ (ccfv @ (ccv @ Xp) @ cclog) @ ccmul))) = (ccfv @ XA2 @ cclog))))).
