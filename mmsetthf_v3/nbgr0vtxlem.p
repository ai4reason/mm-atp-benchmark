...
thf(cnbgr0vtxlem_conj,conjecture,(! [Xph:($i > ($i > $o))] : (! [XG:($i > $o)] : (! [XK:($i > $o)] : ((! [Xe:$i] : (! [Xn:$i] : ((Xph @ Xe @ Xn) => (cwral @ (^ [Xn:$i] : (~ (cwrex @ (^ [Xe:$i] : (cwss @ (ccpr @ XK @ (ccv @ Xn)) @ (ccv @ Xe))) @ (^ [Xe:$i] : (ccfv @ XG @ ccedg))))) @ (^ [Xn:$i] : (ccdif @ (ccfv @ XG @ ccvtx) @ (ccsn @ XK))))))) => (! [Xe:$i] : (! [Xn:$i] : ((Xph @ Xe @ Xn) => ((cco @ XG @ XK @ ccnbgr) = cc0))))))))).
