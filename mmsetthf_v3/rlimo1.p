...
thf(aelo12_thm,axiom,(! [XA2:($i > $o)] : (! [XF:($i > $o)] : (((cwf @ XA2 @ ccc @ XF) & (cwss @ XA2 @ ccr)) => ((cwcel @ XF @ cco1) <=> (cwrex @ (^ [Xx3:$i] : (cwrex @ (^ [Xm:$i] : (cwral @ (^ [Xy1:$i] : ((cwbr @ (ccv @ Xx3) @ (ccv @ Xy1) @ ccle) => (cwbr @ (ccfv @ (ccfv @ (ccv @ Xy1) @ XF) @ ccabs) @ (ccv @ Xm) @ ccle))) @ (^ [Xy1:$i] : XA2))) @ (^ [Xm:$i] : ccr))) @ (^ [Xx3:$i] : ccr))))))).
thf(crlimo1_conj,conjecture,(! [XA2:($i > $o)] : (! [XF:($i > $o)] : ((cwbr @ XF @ XA2 @ ccrli) => (cwcel @ XF @ cco1))))).
