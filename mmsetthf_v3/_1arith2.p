...
thf(a_1arith_thm,axiom,(! [XR:($i > ($i > ($i > $o)))] : (! [XM:($i > ($i > ($i > $o)))] : ((! [Xn:$i] : (! [Xp:$i] : ((XM @ Xn @ Xp) = (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccmpt @ (^ [Xp:$i] : ccprime) @ (^ [Xp:$i] : (cco @ (ccv @ Xp) @ (ccv @ Xn) @ ccpc)))))))) => ((! [Xe:$i] : (! [Xp:$i] : ((XR @ Xe @ Xp) = (ccrab @ (^ [Xe:$i] : (cwcel @ (ccima @ (cccnv @ (ccv @ Xe)) @ ccn) @ ccfn)) @ (^ [Xe:$i] : (cco @ ccn0 @ ccprime @ ccmap)))))) => (! [Xe:$i] : (! [Xn:$i] : (! [Xp:$i] : (cwf1o @ ccn @ (XR @ Xe @ Xp) @ (XM @ Xn @ Xp)))))))))).
