...
thf(cflcidc_conj,conjecture,(! [Xph:$o] : (! [XB2:($i > ($i > $o))] : (! [XS:($i > $o)] : (! [XF:($i > ($i > $o))] : (! [XK:($i > $o)] : ((! [Xj:$i] : (Xph => ((XF @ Xj) = (ccmpt @ (^ [Xj:$i] : XS) @ (^ [Xj:$i] : (ccif @ ((ccv @ Xj) = XK) @ cc1 @ ccc0)))))) => ((Xph => (cwcel @ XS @ ccfn)) => ((Xph => (cwcel @ XK @ XS)) => ((! [Xi:$i] : ((Xph & (cwcel @ (ccv @ Xi) @ XS)) => (cwcel @ (XB2 @ Xi) @ ccc))) => (! [Xj:$i] : (Xph => ((ccsu @ XS @ (^ [Xi:$i] : (cco @ (ccfv @ (ccv @ Xi) @ (XF @ Xj)) @ (XB2 @ Xi) @ ccmul))) = (ccsb @ XK @ (^ [Xi:$i] : (XB2 @ Xi)))))))))))))))).
