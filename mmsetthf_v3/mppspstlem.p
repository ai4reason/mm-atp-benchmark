...
thf(cmppspstlem_conj,conjecture,(! [XC:($i > $o)] : (! [XP:($i > $o)] : (! [XT:($i > $o)] : (! [XJ:($i > ($i > ($i > ($i > $o))))] : ((XP = (ccfv @ XT @ ccmpst)) => ((! [Xh:$i] : (! [Xa:$i] : (! [Xd:$i] : ((XJ @ Xh @ Xa @ Xd) = (ccfv @ XT @ ccmpps))))) => ((XC = (ccfv @ XT @ ccmcls)) => (cwss @ (ccoprab @ (^ [Xd:$i] : (^ [Xh:$i] : (^ [Xa:$i] : ((cwcel @ (ccotp @ (ccv @ Xd) @ (ccv @ Xh) @ (ccv @ Xa)) @ XP) & (cwcel @ (ccv @ Xa) @ (cco @ (ccv @ Xd) @ (ccv @ Xh) @ XC))))))) @ XP))))))))).
