...
thf(atdeglem4_thm,axiom,(! [XA2:($i > ($i > $o))] : (! [XH:($i > ($i > ($i > $o)))] : (! [XI:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [XX:($i > $o)] : ((! [Xm:$i] : ((XA2 @ Xm) = (ccrab @ (^ [Xm:$i] : (cwcel @ (ccima @ (cccnv @ (ccv @ Xm)) @ ccn) @ ccfn)) @ (^ [Xm:$i] : (cco @ ccn0 @ XI @ ccmap))))) => ((! [Xh:$i] : (! [Xm:$i] : ((XH @ Xh @ Xm) = (ccmpt @ (^ [Xh:$i] : (XA2 @ Xm)) @ (^ [Xh:$i] : (cco @ cccnfld @ (ccv @ Xh) @ ccgsu)))))) => (! [Xh:$i] : (! [Xm:$i] : (((cwcel @ XI @ (XV @ Xm)) & (cwcel @ XX @ (XA2 @ Xm))) => (((ccfv @ XX @ (XH @ Xh @ Xm)) = ccc0) <=> (XX = (ccxp @ XI @ (ccsn @ ccc0))))))))))))))).
