...
thf(aeffsumlt_thm,axiom,(! [Xph:($i > $o)] : (! [XA2:($i > $o)] : (! [XF:($i > ($i > $o))] : (! [XN:($i > ($i > $o))] : ((! [Xn:$i] : ((XF @ Xn) = (ccmpt @ (^ [Xn:$i] : ccn0) @ (^ [Xn:$i] : (cco @ (cco @ XA2 @ (ccv @ Xn) @ ccexp) @ (ccfv @ (ccv @ Xn) @ ccfa) @ ccdiv))))) => ((! [Xn:$i] : ((Xph @ Xn) => (cwcel @ XA2 @ ccrp))) => ((! [Xn:$i] : ((Xph @ Xn) => (cwcel @ (XN @ Xn) @ ccn0))) => (! [Xn:$i] : ((Xph @ Xn) => (cwbr @ (ccfv @ (XN @ Xn) @ (ccseq @ ccaddc @ (XF @ Xn) @ ccc0)) @ (ccfv @ XA2 @ cce) @ cclt))))))))))).
