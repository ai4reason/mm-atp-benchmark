...
thf(c_0ram_conj,conjecture,(! [XR:($i > $o)] : (! [XF:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [Xy1:$i] : ((((cwcel @ XR @ (XV @ Xy1)) & (XR != cc0) & (cwf @ XR @ ccn0 @ XF)) & (cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccle)) @ (^ [Xy1:$i] : (ccrn @ XF)))) @ (^ [Xx3:$i] : ccz))) => ((cco @ ccc0 @ XF @ ccram) = (ccsup @ (ccrn @ XF) @ ccr @ cclt)))))))).
