...
thf(cdivalg_conj,conjecture,(! [XD:($i > $o)] : (! [XN:($i > $o)] : (((cwcel @ XN @ ccz) & (cwcel @ XD @ ccz) & (XD != ccc0)) => (cwreu @ (^ [Xr:$i] : (cwrex @ (^ [Xq:$i] : ((cwbr @ ccc0 @ (ccv @ Xr) @ ccle) & (cwbr @ (ccv @ Xr) @ (ccfv @ XD @ ccabs) @ cclt) & (XN = (cco @ (cco @ (ccv @ Xq) @ XD @ ccmul) @ (ccv @ Xr) @ ccaddc)))) @ (^ [Xq:$i] : ccz))) @ (^ [Xr:$i] : ccz)))))).
