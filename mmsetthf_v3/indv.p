...
thf(cindv_conj,conjecture,(! [XO:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [Xx3:$i] : ((cwcel @ XO @ (XV @ Xx3)) => ((ccfv @ XO @ ccind) = (ccmpt @ (^ [Xa:$i] : (ccpw @ XO)) @ (^ [Xa:$i] : (ccmpt @ (^ [Xx3:$i] : XO) @ (^ [Xx3:$i] : (ccif @ (cwcel @ (ccv @ Xx3) @ (ccv @ Xa)) @ cc1 @ ccc0))))))))))).
