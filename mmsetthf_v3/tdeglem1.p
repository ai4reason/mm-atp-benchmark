...
thf(ctdeglem1_conj,conjecture,(! [XA2:($i > ($i > $o))] : (! [XH:($i > ($i > ($i > $o)))] : (! [XI:($i > $o)] : (! [XV:($i > ($i > $o))] : ((! [Xm:$i] : ((XA2 @ Xm) = (ccrab @ (^ [Xm:$i] : (cwcel @ (ccima @ (cccnv @ (ccv @ Xm)) @ ccn) @ ccfn)) @ (^ [Xm:$i] : (cco @ ccn0 @ XI @ ccmap))))) => ((! [Xh:$i] : (! [Xm:$i] : ((XH @ Xh @ Xm) = (ccmpt @ (^ [Xh:$i] : (XA2 @ Xm)) @ (^ [Xh:$i] : (cco @ cccnfld @ (ccv @ Xh) @ ccgsu)))))) => (! [Xh:$i] : (! [Xm:$i] : ((cwcel @ XI @ (XV @ Xm)) => (cwf @ (XA2 @ Xm) @ ccn0 @ (XH @ Xh @ Xm)))))))))))).
