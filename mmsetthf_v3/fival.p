...
thf(adf_fi_ax,axiom,(ccfi = (ccmpt @ (^ [Xx3:$i] : ccvv) @ (^ [Xx3:$i] : (^ [Xz:$i] : (cwrex @ (^ [Xy1:$i] : ((ccv @ Xz) = (ccint @ (ccv @ Xy1)))) @ (^ [Xy1:$i] : (ccin @ (ccpw @ (ccv @ Xx3)) @ ccfn)))))))).
thf(cfival_conj,conjecture,(! [XA2:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [Xy1:$i] : ((cwcel @ XA2 @ (XV @ Xy1)) => ((ccfv @ XA2 @ ccfi) = (^ [Xy1:$i] : (cwrex @ (^ [Xx3:$i] : ((ccv @ Xy1) = (ccint @ (ccv @ Xx3)))) @ (^ [Xx3:$i] : (ccin @ (ccpw @ XA2) @ ccfn)))))))))).
