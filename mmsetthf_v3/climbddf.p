...
thf(cclimbddf_conj,conjecture,(! [XF:($i > ($i > $o))] : (! [XM:($i > $o)] : (! [XZ:($i > $o)] : ((cwnfc @ (^ [Xk:$i] : (XF @ Xk))) => ((XZ = (ccfv @ XM @ ccuz)) => (! [Xk:$i] : (((cwcel @ XM @ ccz) & (cwcel @ (XF @ Xk) @ (ccdm @ ccli)) & (cwral @ (^ [Xk:$i] : (cwcel @ (ccfv @ (ccv @ Xk) @ (XF @ Xk)) @ ccc)) @ (^ [Xk:$i] : XZ))) => (cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xk:$i] : (cwbr @ (ccfv @ (ccfv @ (ccv @ Xk) @ (XF @ Xk)) @ ccabs) @ (ccv @ Xx3) @ ccle)) @ (^ [Xk:$i] : XZ))) @ (^ [Xx3:$i] : ccr)))))))))).
