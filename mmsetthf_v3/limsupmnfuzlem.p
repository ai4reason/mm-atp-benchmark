...
thf(climsupmnfuzlem_conj,conjecture,(! [Xph:$o] : (! [XF:($i > $o)] : (! [XM:($i > ($i > $o))] : (! [XZ:($i > $o)] : ((! [Xx3:$i] : (Xph => (cwcel @ (XM @ Xx3) @ ccz))) => ((! [Xx3:$i] : (XZ = (ccfv @ (XM @ Xx3) @ ccuz))) => ((Xph => (cwf @ XZ @ ccxr @ XF)) => (Xph => (((ccfv @ XF @ cclsp) = ccmnf) <=> (cwral @ (^ [Xx3:$i] : (cwrex @ (^ [Xk:$i] : (cwral @ (^ [Xj:$i] : (cwbr @ (ccfv @ (ccv @ Xj) @ XF) @ (ccv @ Xx3) @ ccle)) @ (^ [Xj:$i] : (ccfv @ (ccv @ Xk) @ ccuz)))) @ (^ [Xk:$i] : XZ))) @ (^ [Xx3:$i] : ccr)))))))))))).
