...
thf(c_2zrngnmlid_conj,conjecture,(! [XR:($i > $o)] : (! [XE:($i > $o)] : (! [XM:($i > ($i > ($i > $o)))] : ((XE = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xx3:$i] : ((ccv @ Xz) = (cco @ cc2 @ (ccv @ Xx3) @ ccmul))) @ (^ [Xx3:$i] : ccz))) @ (^ [Xz:$i] : ccz))) => ((XR = (cco @ cccnfld @ XE @ ccress)) => ((! [Xx3:$i] : (! [Xz:$i] : ((XM @ Xx3 @ Xz) = (ccfv @ XR @ ccmgp)))) => (cwral @ (^ [Xb:$i] : (cwrex @ (^ [Xa:$i] : ((cco @ (ccv @ Xb) @ (ccv @ Xa) @ ccmul) != (ccv @ Xa))) @ (^ [Xa:$i] : XE))) @ (^ [Xb:$i] : XE))))))))).
