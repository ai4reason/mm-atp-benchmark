...
thf(adf_yon_ax,axiom,(ccyon = (ccmpt @ (^ [Xc:$i] : cccat) @ (^ [Xc:$i] : (cco @ (ccop @ (ccv @ Xc) @ (ccfv @ (ccv @ Xc) @ ccoppc)) @ (ccfv @ (ccfv @ (ccv @ Xc) @ ccoppc) @ cchof) @ cccurf))))).
...
thf(cyonval_conj,conjecture,(! [Xph:$o] : (! [XC:($i > $o)] : (! [XM:($i > $o)] : (! [XO:($i > $o)] : (! [XY:($i > $o)] : ((XY = (ccfv @ XC @ ccyon)) => ((Xph => (cwcel @ XC @ cccat)) => ((XO = (ccfv @ XC @ ccoppc)) => ((XM = (ccfv @ XO @ cchof)) => (Xph => (XY = (cco @ (ccop @ XC @ XO) @ XM @ cccurf))))))))))))).
