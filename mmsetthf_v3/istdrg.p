...
thf(adf_tdrg_ax,axiom,(cctdrg = (ccrab @ (^ [Xr:$i] : (cwcel @ (cco @ (ccfv @ (ccv @ Xr) @ ccmgp) @ (ccfv @ (ccv @ Xr) @ ccui) @ ccress) @ cctgp)) @ (^ [Xr:$i] : (ccin @ cctrg @ ccdr))))).
thf(adf_3an_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((Xph & Xps & Xch) <=> ((Xph & Xps) & Xch)))))).
thf(cistdrg_conj,conjecture,(! [XR:($i > $o)] : (! [XU:($i > $o)] : (! [XM:($i > $o)] : ((XM = (ccfv @ XR @ ccmgp)) => ((XU = (ccfv @ XR @ ccui)) => ((cwcel @ XR @ cctdrg) <=> ((cwcel @ XR @ cctrg) & (cwcel @ XR @ ccdr) & (cwcel @ (cco @ XM @ XU @ ccress) @ cctgp))))))))).
