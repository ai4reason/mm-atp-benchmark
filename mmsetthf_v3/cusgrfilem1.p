...
thf(ccusgrfilem1_conj,conjecture,(! [XP:($i > ($i > ($i > $o)))] : (! [XG:($i > ($i > $o))] : (! [XN:($i > $o)] : (! [XV:($i > $o)] : ((! [Xa:$i] : (XV = (ccfv @ (XG @ Xa) @ ccvtx))) => ((! [Xx3:$i] : (! [Xa:$i] : ((XP @ Xx3 @ Xa) = (ccrab @ (^ [Xx3:$i] : (cwrex @ (^ [Xa:$i] : (((ccv @ Xa) != XN) & ((ccv @ Xx3) = (ccpr @ (ccv @ Xa) @ XN)))) @ (^ [Xa:$i] : XV))) @ (^ [Xx3:$i] : (ccpw @ XV)))))) => (! [Xx3:$i] : (! [Xa:$i] : (((cwcel @ (XG @ Xa) @ cccusgr) & (cwcel @ XN @ XV)) => (cwss @ (XP @ Xx3 @ Xa) @ (ccfv @ (XG @ Xa) @ ccedg)))))))))))).
