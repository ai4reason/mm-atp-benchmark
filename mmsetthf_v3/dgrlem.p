...
thf(cdgrlem_conj,conjecture,(! [XA2:($i > $o)] : (! [XS:($i > $o)] : (! [XF:($i > $o)] : ((XA2 = (ccfv @ XF @ cccoe)) => ((cwcel @ XF @ (ccfv @ XS @ ccply)) => ((cwf @ ccn0 @ (ccun @ XS @ (ccsn @ ccc0)) @ XA2) & (cwrex @ (^ [Xn:$i] : (cwral @ (^ [Xx3:$i] : (cwbr @ (ccv @ Xx3) @ (ccv @ Xn) @ ccle)) @ (^ [Xx3:$i] : (ccima @ (cccnv @ XA2) @ (ccdif @ ccc @ (ccsn @ ccc0)))))) @ (^ [Xn:$i] : ccz))))))))).
