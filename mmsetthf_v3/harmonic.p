...
thf(charmonic_conj,conjecture,(! [XF:($i > ($i > $o))] : (! [XH:($i > ($i > $o))] : ((! [Xn:$i] : ((XF @ Xn) = (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (cco @ cc1 @ (ccv @ Xn) @ ccdiv))))) => ((! [Xn:$i] : ((XH @ Xn) = (ccseq @ ccaddc @ (XF @ Xn) @ cc1))) => (! [Xn:$i] : (~ (cwcel @ (XH @ Xn) @ (ccdm @ ccli))))))))).
