...
thf(ardgeq2_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XF:($i > $o)] : ((XA2 = XB2) => ((ccrdg @ XF @ XA2) = (ccrdg @ XF @ XB2))))))).
...
thf(chsmex_conj,conjecture,(! [XV:($i > ($i > ($i > $o)))] : (! [XX:($i > $o)] : (! [Xx3:$i] : (! [Xs1:$i] : ((cwcel @ XX @ (XV @ Xx3 @ Xs1)) => (cwcel @ (ccrab @ (^ [Xs1:$i] : (cwral @ (^ [Xx3:$i] : (cwbr @ (ccv @ Xx3) @ XX @ ccdom)) @ (^ [Xx3:$i] : (ccfv @ (ccsn @ (ccv @ Xs1)) @ cctc)))) @ (^ [Xs1:$i] : (ccuni @ (ccima @ ccr1 @ ccon0)))) @ ccvv))))))).
