...
thf(crusgr1vtxlem_conj,conjecture,(! [XA2:($i > ($i > $o))] : (! [XK:($i > $o)] : (! [XV:($i > $o)] : (! [XW:($i > ($i > $o))] : (! [Xv:$i] : ((((cwral @ (^ [Xv:$i] : ((ccfv @ (XA2 @ Xv) @ cchash) = XK)) @ (^ [Xv:$i] : XV)) & (cwral @ (^ [Xv:$i] : ((XA2 @ Xv) = cc0)) @ (^ [Xv:$i] : XV))) & ((cwcel @ XV @ (XW @ Xv)) & ((ccfv @ XV @ cchash) = cc1))) => (XK = ccc0)))))))).
