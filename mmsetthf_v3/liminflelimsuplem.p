...
thf(climinflelimsuplem_conj,conjecture,(! [Xph:($i > $o)] : (! [XF:($i > $o)] : (! [XV:($i > ($i > ($i > $o)))] : ((! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xk) => (cwcel @ XF @ (XV @ Xj @ Xk))))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwral @ (^ [Xk:$i] : (cwrex @ (^ [Xj:$i] : ((ccin @ (ccima @ XF @ (cco @ (ccv @ Xj) @ ccpnf @ ccico)) @ ccxr) != cc0)) @ (^ [Xj:$i] : (cco @ (ccv @ Xk) @ ccpnf @ ccico)))) @ (^ [Xk:$i] : ccr)))) => (! [Xk:$i] : ((Xph @ Xk) => (cwbr @ (ccfv @ XF @ cclsi) @ (ccfv @ XF @ cclsp) @ ccle))))))))).
