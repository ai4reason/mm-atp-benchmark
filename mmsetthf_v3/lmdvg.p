...
thf(clmdvg_conj,conjecture,(! [Xph:$o] : (! [XF:($i > $o)] : ((Xph => (cwf @ ccn @ (cco @ ccc0 @ ccpnf @ ccico) @ XF)) => ((! [Xk:$i] : ((Xph & (cwcel @ (ccv @ Xk) @ ccn)) => (cwbr @ (ccfv @ (ccv @ Xk) @ XF) @ (ccfv @ (cco @ (ccv @ Xk) @ cc1 @ ccaddc) @ XF) @ ccle))) => ((Xph => (~ (cwcel @ XF @ (ccdm @ ccli)))) => (Xph => (cwral @ (^ [Xx3:$i] : (cwrex @ (^ [Xj:$i] : (cwral @ (^ [Xk:$i] : (cwbr @ (ccv @ Xx3) @ (ccfv @ (ccv @ Xk) @ XF) @ cclt)) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xj) @ ccuz)))) @ (^ [Xj:$i] : ccn))) @ (^ [Xx3:$i] : ccr))))))))).
