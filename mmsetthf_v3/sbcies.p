...
thf(csbcies_conj,conjecture,(! [Xph:$o] : (! [Xps:($i > $o)] : (! [Xw:$i] : (! [XA2:($i > ($i > $o))] : (! [XE:($i > $o)] : (! [XW:($i > $o)] : ((! [Xa:$i] : ((XA2 @ Xa) = (ccfv @ XW @ XE))) => ((! [Xa:$i] : (((ccv @ Xa) = (XA2 @ Xa)) => (Xph <=> (Xps @ Xa)))) => (((ccv @ Xw) = XW) => ((cwsbc @ (^ [Xa:$i] : (Xps @ Xa)) @ (ccfv @ (ccv @ Xw) @ XE)) <=> Xph))))))))))).
