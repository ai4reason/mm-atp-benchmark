...
thf(cintima0_conj,conjecture,(! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : ((cciin @ (^ [Xa:$i] : (XA2 @ Xa)) @ (^ [Xa:$i] : (ccima @ (ccv @ Xa) @ (XB2 @ Xa)))) = (ccint @ (^ [Xx3:$i] : (cwrex @ (^ [Xa:$i] : ((ccv @ Xx3) = (ccima @ (ccv @ Xa) @ (XB2 @ Xa)))) @ (^ [Xa:$i] : (XA2 @ Xa))))))))).
