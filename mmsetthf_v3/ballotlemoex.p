...
thf(cballotlemoex_conj,conjecture,(! [XM:($i > $o)] : (! [XN:($i > $o)] : (! [XO:($i > $o)] : ((cwcel @ XM @ ccn) => ((cwcel @ XN @ ccn) => ((XO = (ccrab @ (^ [Xc:$i] : ((ccfv @ (ccv @ Xc) @ cchash) = XM)) @ (^ [Xc:$i] : (ccpw @ (cco @ cc1 @ (cco @ XM @ XN @ ccaddc) @ ccfz))))) => (cwcel @ XO @ ccvv)))))))).
