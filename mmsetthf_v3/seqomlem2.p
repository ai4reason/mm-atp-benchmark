...
thf(cseqomlem2_conj,conjecture,(! [XQ:($i > $o)] : (! [XF:($i > $o)] : (! [XI:($i > ($i > ($i > $o)))] : ((! [Xv:$i] : (! [Xi:$i] : (XQ = (ccrdg @ (ccmpt2 @ (^ [Xi:$i] : (^ [Xv:$i] : ccom)) @ (^ [Xi:$i] : (^ [Xv:$i] : ccvv)) @ (^ [Xi:$i] : (^ [Xv:$i] : (ccop @ (ccsuc @ (ccv @ Xi)) @ (cco @ (ccv @ Xi) @ (ccv @ Xv) @ XF))))) @ (ccop @ cc0 @ (ccfv @ (XI @ Xv @ Xi) @ ccid)))))) => (cwfn @ (ccima @ XQ @ ccom) @ ccom)))))).
