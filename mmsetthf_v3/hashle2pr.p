...
thf(aspc2ev_thm,axiom,(! [Xph:($i > ($i > $o))] : (! [Xps:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : ((cwcel @ XA2 @ ccvv) => ((cwcel @ XB2 @ ccvv) => ((! [Xx3:$i] : (! [Xy1:$i] : ((((ccv @ Xx3) = XA2) & ((ccv @ Xy1) = XB2)) => ((Xph @ Xx3 @ Xy1) <=> Xps)))) => (Xps => (? [Xx3:$i] : (? [Xy1:$i] : (Xph @ Xx3 @ Xy1)))))))))))).
...
thf(chashle2pr_conj,conjecture,(! [XP:($i > $o)] : (! [XV:($i > ($i > ($i > $o)))] : (! [Xa:$i] : (! [Xb:$i] : (((cwcel @ XP @ (XV @ Xa @ Xb)) & (XP != cc0)) => ((cwbr @ (ccfv @ XP @ cchash) @ cc2 @ ccle) <=> (? [Xa:$i] : (? [Xb:$i] : (XP = (ccpr @ (ccv @ Xa) @ (ccv @ Xb)))))))))))).
