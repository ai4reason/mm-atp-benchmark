...
thf(cxlimmnfvlem1_conj,conjecture,(! [Xph:$o] : (! [XF:($i > $o)] : (! [XM:($i > ($i > $o))] : (! [XX:($i > $o)] : (! [XZ:($i > $o)] : ((! [Xk:$i] : (Xph => (cwcel @ (XM @ Xk) @ ccz))) => ((! [Xk:$i] : (XZ = (ccfv @ (XM @ Xk) @ ccuz))) => ((Xph => (cwf @ XZ @ ccxr @ XF)) => ((Xph => (cwbr @ XF @ ccmnf @ cclsxlim)) => ((Xph => (cwcel @ XX @ ccr)) => (Xph => (cwrex @ (^ [Xj:$i] : (cwral @ (^ [Xk:$i] : (cwbr @ (ccfv @ (ccv @ Xk) @ XF) @ XX @ ccle)) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xj) @ ccuz)))) @ (^ [Xj:$i] : XZ)))))))))))))).
