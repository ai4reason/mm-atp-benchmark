...
thf(a_2rexuz_thm,conjecture,(! [Xph:($i > ($i > $o))] : ((? [Xm:$i] : (cwrex @ (^ [Xn:$i] : (Xph @ Xm @ Xn)) @ (^ [Xn:$i] : (ccfv @ (ccv @ Xm) @ ccuz)))) <=> (cwrex @ (^ [Xm:$i] : (cwrex @ (^ [Xn:$i] : ((cwbr @ (ccv @ Xm) @ (ccv @ Xn) @ ccle) & (Xph @ Xm @ Xn))) @ (^ [Xn:$i] : ccz))) @ (^ [Xm:$i] : ccz))))).
