...
thf(adivalglem2_thm,conjecture,(! [XD:($i > $o)] : (! [XS:($i > ($i > $o))] : (! [XN:($i > $o)] : ((cwcel @ XN @ ccz) => ((cwcel @ XD @ ccz) => ((XD != ccc0) => ((! [Xr:$i] : ((XS @ Xr) = (ccrab @ (^ [Xr:$i] : (cwbr @ XD @ (cco @ XN @ (ccv @ Xr) @ ccmin) @ ccdvds)) @ (^ [Xr:$i] : ccn0)))) => (! [Xr:$i] : (cwcel @ (ccinf @ (XS @ Xr) @ ccr @ cclt) @ (XS @ Xr))))))))))).
