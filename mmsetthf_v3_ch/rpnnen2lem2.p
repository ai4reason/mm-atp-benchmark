...
thf(arpnnen2lem2_thm,conjecture,(! [XA2:($i > $o)] : (! [XF:($i > ($i > ($i > $o)))] : ((! [Xx3:$i] : (! [Xn:$i] : ((XF @ Xx3 @ Xn) = (ccmpt @ (^ [Xx3:$i] : (ccpw @ ccn)) @ (^ [Xx3:$i] : (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccif @ (cwcel @ (ccv @ Xn) @ (ccv @ Xx3)) @ (cco @ (cco @ cc1 @ cc3 @ ccdiv) @ (ccv @ Xn) @ ccexp) @ ccc0)))))))) => (! [Xx3:$i] : (! [Xn:$i] : ((cwss @ XA2 @ ccn) => (cwf @ ccn @ ccr @ (ccfv @ XA2 @ (XF @ Xx3 @ Xn)))))))))).
