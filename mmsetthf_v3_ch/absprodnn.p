...
thf(aabsprodnn_thm,conjecture,(! [Xph:$o] : (! [XP:($i > ($i > $o))] : (! [XZ:($i > $o)] : ((Xph => (cwss @ XZ @ ccz)) => ((Xph => (cwcel @ XZ @ ccfn)) => ((! [Xz:$i] : ((XP @ Xz) = (ccfv @ (ccprod @ (^ [Xz:$i] : XZ) @ (^ [Xz:$i] : (ccv @ Xz))) @ ccabs))) => ((Xph => (~ (wnel @ ccc0 @ XZ))) => (! [Xz:$i] : (Xph => (cwcel @ (XP @ Xz) @ ccn))))))))))).
