...
thf(ametustexhalf_thm,conjecture,(! [XA2:($i > $o)] : (! [XD:($i > $o)] : (! [XF:($i > $o)] : (! [XX:($i > $o)] : ((XF = (ccrn @ (ccmpt @ (^ [Xa:$i] : ccrp) @ (^ [Xa:$i] : (ccima @ (cccnv @ XD) @ (cco @ ccc0 @ (ccv @ Xa) @ ccico)))))) => ((((XX != cc0) & (cwcel @ XD @ (ccfv @ XX @ ccpsmet))) & (cwcel @ XA2 @ XF)) => (cwrex @ (^ [Xv:$i] : (cwss @ (cccom @ (ccv @ Xv) @ (ccv @ Xv)) @ XA2)) @ (^ [Xv:$i] : XF))))))))).
