...
thf(alimsuplt_thm,conjecture,(! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : (! [XF:($i > $o)] : (! [XG:($i > ($i > $o))] : ((! [Xk:$i] : ((XG @ Xk) = (ccmpt @ (^ [Xk:$i] : ccr) @ (^ [Xk:$i] : (ccsup @ (ccin @ (ccima @ XF @ (cco @ (ccv @ Xk) @ ccpnf @ ccico)) @ ccxr) @ ccxr @ cclt))))) => (! [Xk:$i] : (((cwss @ (XB2 @ Xk) @ ccr) & (cwf @ (XB2 @ Xk) @ ccxr @ XF) & (cwcel @ (XA2 @ Xk) @ ccxr)) => ((cwbr @ (ccfv @ XF @ cclsp) @ (XA2 @ Xk) @ cclt) <=> (cwrex @ (^ [Xj:$i] : (cwbr @ (ccfv @ (ccv @ Xj) @ (XG @ Xk)) @ (XA2 @ Xk) @ cclt)) @ (^ [Xj:$i] : ccr))))))))))).
