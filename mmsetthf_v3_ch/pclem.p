...
thf(apclem_thm,conjecture,(! [XA2:($i > ($i > $o))] : (! [XP:($i > $o)] : (! [XN:($i > $o)] : ((! [Xn:$i] : ((XA2 @ Xn) = (ccrab @ (^ [Xn:$i] : (cwbr @ (cco @ XP @ (ccv @ Xn) @ ccexp) @ XN @ ccdvds)) @ (^ [Xn:$i] : ccn0)))) => (! [Xn:$i] : (((cwcel @ XP @ (ccfv @ cc2 @ ccuz)) & ((cwcel @ XN @ ccz) & (XN != ccc0))) => ((cwss @ (XA2 @ Xn) @ ccz) & ((XA2 @ Xn) != cc0) & (cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccle)) @ (^ [Xy1:$i] : (XA2 @ Xn)))) @ (^ [Xx3:$i] : ccz)))))))))).
