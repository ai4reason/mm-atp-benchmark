include('Axioms/mmaxv3_000').
include('Axioms/mmaxv3_001').
include('Axioms/mmaxv3_002').
include('Axioms/mmaxv3_003').
include('Axioms/mmaxv3_004').
include('Axioms/mmaxv3_005').
include('Axioms/mmaxv3_006').
include('Axioms/mmaxv3_007').
include('Axioms/mmaxv3_008').
include('Axioms/mmaxv3_009').
include('Axioms/mmaxv3_010').
include('Axioms/mmaxv3_011').
include('Axioms/mmaxv3_012').
include('Axioms/mmaxv3_013').
include('Axioms/mmaxv3_014').
include('Axioms/mmaxv3_015').
include('Axioms/mmaxv3_016').
include('Axioms/mmaxv3_017').
include('Axioms/mmaxv3_018').
include('Axioms/mmaxv3_019').
include('Axioms/mmaxv3_020').
include('Axioms/mmaxv3_021').
include('Axioms/mmaxv3_022').
include('Axioms/mmaxv3_023').
include('Axioms/mmaxv3_024').
include('Axioms/mmaxv3_025').
include('Axioms/mmaxv3_026').
include('Axioms/mmaxv3_027').
include('Axioms/mmaxv3_028').
include('Axioms/mmaxv3_029').
include('Axioms/mmaxv3_030').
include('Axioms/mmaxv3_031').
include('Axioms/mmaxv3_032').
include('Axioms/mmaxv3_033').
include('Axioms/mmaxv3_034').
include('Axioms/mmaxv3_035').
include('Axioms/mmaxv3_036').
include('Axioms/mmaxv3_037').
include('Axioms/mmaxv3_038').
include('Axioms/mmaxv3_039').
include('Axioms/mmaxv3_040').
include('Axioms/mmaxv3_041').
include('Axioms/mmaxv3_042').
include('Axioms/mmaxv3_043').
include('Axioms/mmaxv3_044').
include('Axioms/mmaxv3_045').
include('Axioms/mmaxv3_046').
include('Axioms/mmaxv3_047').
include('Axioms/mmaxv3_048').
include('Axioms/mmaxv3_049').
include('Axioms/mmaxv3_050').
include('Axioms/mmaxv3_051').
include('Axioms/mmaxv3_052').
include('Axioms/mmaxv3_053').
include('Axioms/mmaxv3_054').
include('Axioms/mmaxv3_055').
include('Axioms/mmaxv3_056').
include('Axioms/mmaxv3_057').
include('Axioms/mmaxv3_058').
include('Axioms/mmaxv3_059').
include('Axioms/mmaxv3_060').
include('Axioms/mmaxv3_061').
include('Axioms/mmaxv3_062').
include('Axioms/mmaxv3_063').
include('Axioms/mmaxv3_064').
include('Axioms/mmaxv3_065').
include('Axioms/mmaxv3_066').
include('Axioms/mmaxv3_067').
include('Axioms/mmaxv3_068').
include('Axioms/mmaxv3_069').
include('Axioms/mmaxv3_070').
include('Axioms/mmaxv3_071').
include('Axioms/mmaxv3_072').
include('Axioms/mmaxv3_073').
include('Axioms/mmaxv3_074').
include('Axioms/mmaxv3_075').
include('Axioms/mmaxv3_076').
include('Axioms/mmaxv3_077').
include('Axioms/mmaxv3_078').
include('Axioms/mmaxv3_079').
include('Axioms/mmaxv3_080').
include('Axioms/mmaxv3_081').
include('Axioms/mmaxv3_082').
include('Axioms/mmaxv3_083').
include('Axioms/mmaxv3_084').
include('Axioms/mmaxv3_085').
include('Axioms/mmaxv3_086').
include('Axioms/mmaxv3_087').
include('Axioms/mmaxv3_088').
include('Axioms/mmaxv3_089').
include('Axioms/mmaxv3_090').
include('Axioms/mmaxv3_091').
include('Axioms/mmaxv3_092').
include('Axioms/mmaxv3_093').
include('Axioms/mmaxv3_094').
include('Axioms/mmaxv3_095').
include('Axioms/mmaxv3_096').
include('Axioms/mmaxv3_097').
include('Axioms/mmaxv3_098').
include('Axioms/mmaxv3_099').
include('Axioms/mmaxv3_100').
include('Axioms/mmaxv3_101').
include('Axioms/mmaxv3_102').
include('Axioms/mmaxv3_103').
include('Axioms/mmaxv3_104').
include('Axioms/mmaxv3_105').
include('Axioms/mmaxv3_106').
include('Axioms/mmaxv3_107').
include('Axioms/mmaxv3_108').
include('Axioms/mmaxv3_109').
include('Axioms/mmaxv3_110').
include('Axioms/mmaxv3_111').
include('Axioms/mmaxv3_112').
include('Axioms/mmaxv3_113').
include('Axioms/mmaxv3_114').
include('Axioms/mmaxv3_115').
include('Axioms/mmaxv3_116').
include('Axioms/mmaxv3_117').
include('Axioms/mmaxv3_118').
include('Axioms/mmaxv3_119').
include('Axioms/mmaxv3_120').
include('Axioms/mmaxv3_121').
include('Axioms/mmaxv3_122').
include('Axioms/mmaxv3_123').
include('Axioms/mmaxv3_124').
include('Axioms/mmaxv3_125').
include('Axioms/mmaxv3_126').
include('Axioms/mmaxv3_127').
include('Axioms/mmaxv3_128').
include('Axioms/mmaxv3_129').
include('Axioms/mmaxv3_130').
include('Axioms/mmaxv3_131').
include('Axioms/mmaxv3_132').
include('Axioms/mmaxv3_133').
include('Axioms/mmaxv3_134').
include('Axioms/mmaxv3_135').
include('Axioms/mmaxv3_136').
include('Axioms/mmaxv3_137').
include('Axioms/mmaxv3_138').
include('Axioms/mmaxv3_139').
include('Axioms/mmaxv3_140').
include('Axioms/mmaxv3_141').
include('Axioms/mmaxv3_142').
include('Axioms/mmaxv3_143').
include('Axioms/mmaxv3_144').
include('Axioms/mmaxv3_145').
include('Axioms/mmaxv3_146').
include('Axioms/mmaxv3_147').
include('Axioms/mmaxv3_148').
include('Axioms/mmaxv3_149').
include('Axioms/mmaxv3_150').
include('Axioms/mmaxv3_151').
include('Axioms/mmaxv3_152').
include('Axioms/mmaxv3_153').
include('Axioms/mmaxv3_154').
include('Axioms/mmaxv3_155').
include('Axioms/mmaxv3_156').
include('Axioms/mmaxv3_157').
include('Axioms/mmaxv3_158').
include('Axioms/mmaxv3_159').
include('Axioms/mmaxv3_160').
include('Axioms/mmaxv3_161').
include('Axioms/mmaxv3_162').
include('Axioms/mmaxv3_163').
include('Axioms/mmaxv3_164').
include('Axioms/mmaxv3_165').
include('Axioms/mmaxv3_166').
include('Axioms/mmaxv3_167').
include('Axioms/mmaxv3_168').
include('Axioms/mmaxv3_169').
include('Axioms/mmaxv3_170').
include('Axioms/mmaxv3_171').
include('Axioms/mmaxv3_172').
include('Axioms/mmaxv3_173').
include('Axioms/mmaxv3_174').
include('Axioms/mmaxv3_175').
include('Axioms/mmaxv3_176').
include('Axioms/mmaxv3_177').
include('Axioms/mmaxv3_178').
include('Axioms/mmaxv3_179').
include('Axioms/mmaxv3_180').
include('Axioms/mmaxv3_181').
include('Axioms/mmaxv3_182').
include('Axioms/mmaxv3_183').
include('Axioms/mmaxv3_184').
include('Axioms/mmaxv3_185').
include('Axioms/mmaxv3_186').
include('Axioms/mmaxv3_187').
include('Axioms/mmaxv3_188').
include('Axioms/mmaxv3_189').
include('Axioms/mmaxv3_190').
include('Axioms/mmaxv3_191').
include('Axioms/mmaxv3_192').
include('Axioms/mmaxv3_193').
include('Axioms/mmaxv3_194').
include('Axioms/mmaxv3_195').
include('Axioms/mmaxv3_196').
include('Axioms/mmaxv3_197').
include('Axioms/mmaxv3_198').
include('Axioms/mmaxv3_199').
include('Axioms/mmaxv3_200').
include('Axioms/mmaxv3_201').
include('Axioms/mmaxv3_202').
include('Axioms/mmaxv3_203').
include('Axioms/mmaxv3_204').
include('Axioms/mmaxv3_205').
include('Axioms/mmaxv3_206').
include('Axioms/mmaxv3_207').
thf(aomege0_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XO:($i > $o)] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XO @ ccome)) => ((XX = (ccuni @ (ccdm @ XO))) => ((Xph => (cwss @ XA2 @ XX)) => (Xph => (cwbr @ ccc0 @ (ccfv @ XA2 @ XO) @ ccle)))))))))).
thf(aomess0_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XO:($i > $o)] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XO @ ccome)) => ((XX = (ccuni @ (ccdm @ XO))) => ((Xph => (cwss @ XA2 @ XX)) => ((Xph => ((ccfv @ XA2 @ XO) = ccc0)) => ((Xph => (cwss @ XB2 @ XA2)) => (Xph => ((ccfv @ XB2 @ XO) = ccc0))))))))))))).
thf(acaragencmpl_thm,axiom,(! [Xph:$o] : (! [XS:($i > $o)] : (! [XE:($i > $o)] : (! [XO:($i > $o)] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XO @ ccome)) => ((XX = (ccuni @ (ccdm @ XO))) => ((Xph => (cwss @ XE @ XX)) => ((Xph => ((ccfv @ XE @ XO) = ccc0)) => ((XS = (ccfv @ XO @ cccaragen)) => (Xph => (cwcel @ XE @ XS))))))))))))).
thf(ccovoln_tp,type,(ccovoln : ($i > $o))).
thf(adf_ovoln_ax,axiom,(ccovoln = (ccmpt @ (^ [Xx3:$i] : ccfn) @ (^ [Xx3:$i] : (ccmpt @ (^ [Xy1:$i] : (ccpw @ (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xy1:$i] : (ccif @ ((ccv @ Xx3) = cc0) @ ccc0 @ (ccinf @ (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (ccv @ Xy1) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : (ccv @ Xx3)) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : (ccv @ Xx3)) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ (ccv @ Xx3) @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr)) @ ccxr @ cclt)))))))).
thf(ccvoln_tp,type,(ccvoln : ($i > $o))).
thf(adf_voln_ax,axiom,(ccvoln = (ccmpt @ (^ [Xx3:$i] : ccfn) @ (^ [Xx3:$i] : (ccres @ (ccfv @ (ccv @ Xx3) @ ccovoln) @ (ccfv @ (ccfv @ (ccv @ Xx3) @ ccovoln) @ cccaragen)))))).
thf(avonval_thm,axiom,(! [Xph:$o] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => (Xph => ((ccfv @ XX @ ccvoln) = (ccres @ (ccfv @ XX @ ccovoln) @ (ccfv @ (ccfv @ XX @ ccovoln) @ cccaragen)))))))).
thf(aovnval_thm,axiom,(! [Xph:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((! [Xy1:$i] : (! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xy1 @ Xz @ Xi @ Xj @ Xk) => (cwcel @ XX @ ccfn))))))) => (! [Xy1:$i] : (! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xy1 @ Xz @ Xi @ Xj @ Xk) => ((ccfv @ XX @ ccovoln) = (ccmpt @ (^ [Xy1:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xy1:$i] : (ccif @ (XX = cc0) @ ccc0 @ (ccinf @ (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (ccv @ Xy1) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr)) @ ccxr @ cclt))))))))))))))).
thf(aelhoi_thm,axiom,(! [Xph:($i > $o)] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [XX:($i > $o)] : (! [XY:($i > $o)] : ((! [Xx3:$i] : ((Xph @ Xx3) => (cwcel @ XX @ (XV @ Xx3)))) => (! [Xx3:$i] : ((Xph @ Xx3) => ((cwcel @ XY @ (cco @ (cco @ XA2 @ XB2 @ ccico) @ XX @ ccmap)) <=> ((cwf @ XX @ ccxr @ XY) & (cwral @ (^ [Xx3:$i] : (cwcel @ (ccfv @ (ccv @ Xx3) @ XY) @ (cco @ XA2 @ XB2 @ ccico))) @ (^ [Xx3:$i] : XX)))))))))))))).
thf(aicoresmbl_thm,axiom,(cwss @ (ccrn @ (ccres @ ccico @ (ccxp @ ccr @ ccr))) @ (ccdm @ ccvol))).
thf(ahoissre_thm,axiom,(! [Xph:$o] : (! [Xk:$i] : (! [XI:($i > $o)] : (! [XX:($i > $o)] : ((Xph => (cwf @ XX @ (ccxp @ ccr @ ccr) @ XI)) => ((Xph & (cwcel @ (ccv @ Xk) @ XX)) => (cwss @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ XI)) @ ccr)))))))).
thf(aovnval2_thm,axiom,(! [Xph:($i > ($i > ($i > ($i > $o))))] : (! [XA2:($i > ($i > ($i > $o)))] : (! [XM:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => (cwcel @ XX @ ccfn)))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => (cwss @ (XA2 @ Xj @ Xk) @ (cco @ ccr @ XX @ ccmap))))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((XM @ Xz @ Xi @ Xj @ Xk) = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (XA2 @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr))))))) => (! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => ((ccfv @ (XA2 @ Xj @ Xk) @ (ccfv @ XX @ ccovoln)) = (ccif @ (XX = cc0) @ ccc0 @ (ccinf @ (XM @ Xz @ Xi @ Xj @ Xk) @ ccxr @ cclt)))))))))))))))).
thf(avolicorecl_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (((cwcel @ XA2 @ ccr) & (cwcel @ XB2 @ ccr)) => (cwcel @ (ccfv @ (cco @ XA2 @ XB2 @ ccico) @ ccvol) @ ccr))))).
thf(ahoiprodcl_thm,axiom,(! [Xph:($i > $o)] : (! [XI:($i > ($i > $o))] : (! [XX:($i > $o)] : (((? [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X)) => (! [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwcel @ XX @ ccfn))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwf @ XX @ (ccxp @ ccr @ ccr) @ (XI @ Xk)))) => (! [Xk:$i] : ((Xph @ Xk) => (cwcel @ (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (XI @ Xk))) @ ccvol))) @ (cco @ ccc0 @ ccpnf @ ccico))))))))))).
thf(ahoicvr_thm,axiom,(! [Xph:$o] : (! [XI:($i > ($i > ($i > ($i > $o))))] : (! [XX:($i > $o)] : ((! [Xx3:$i] : (! [Xi:$i] : (! [Xj:$i] : ((XI @ Xx3 @ Xi @ Xj) = (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccmpt @ (^ [Xx3:$i] : XX) @ (^ [Xx3:$i] : (ccop @ (ccneg @ (ccv @ Xj)) @ (ccv @ Xj)))))))))) => ((Xph => (cwcel @ XX @ ccfn)) => (! [Xx3:$i] : (Xph => (cwss @ (cco @ ccr @ XX @ ccmap) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xi:$i] : XX) @ (^ [Xi:$i] : (ccfv @ (ccv @ Xi) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (XI @ Xx3 @ Xi @ Xj))))))))))))))))).
thf(ahoissrrn_thm,axiom,(! [Xph:$o] : (! [XI:($i > ($i > $o))] : (! [XX:($i > $o)] : ((! [Xk:$i] : (Xph => (cwf @ XX @ (ccxp @ ccr @ ccr) @ (XI @ Xk)))) => (Xph => (cwss @ (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (XI @ Xk))))) @ (cco @ ccr @ XX @ ccmap)))))))).
thf(aovn0val_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : ((Xph => (cwss @ XA2 @ (cco @ ccr @ cc0 @ ccmap))) => (Xph => ((ccfv @ XA2 @ (ccfv @ cc0 @ ccovoln)) = ccc0)))))).
thf(aovnn0val_thm,axiom,(! [Xph:($i > ($i > ($i > ($i > $o))))] : (! [XA2:($i > ($i > ($i > $o)))] : (! [XM:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => (cwcel @ XX @ ccfn)))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => (XX != cc0)))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => (cwss @ (XA2 @ Xj @ Xk) @ (cco @ ccr @ XX @ ccmap))))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((XM @ Xz @ Xi @ Xj @ Xk) = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (XA2 @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr))))))) => (! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => ((ccfv @ (XA2 @ Xj @ Xk) @ (ccfv @ XX @ ccovoln)) = (ccinf @ (XM @ Xz @ Xi @ Xj @ Xk) @ ccxr @ cclt)))))))))))))))).
thf(aovnval2b_thm,axiom,(! [Xph:($i > ($i > ($i > ($i > $o))))] : (! [XA2:($i > ($i > ($i > $o)))] : (! [XL:($i > ($i > ($i > ($i > ($i > ($i > $o))))))] : (! [XX:($i > $o)] : ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => (cwcel @ XX @ ccfn)))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => (cwss @ (XA2 @ Xj @ Xk) @ (cco @ ccr @ XX @ ccmap))))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : ((XL @ Xz @ Xi @ Xj @ Xk @ Xa) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (ccv @ Xa) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr)))))))))) => (! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : ((Xph @ Xz @ Xi @ Xj @ Xk) => ((ccfv @ (XA2 @ Xj @ Xk) @ (ccfv @ XX @ ccovoln)) = (ccif @ (XX = cc0) @ ccc0 @ (ccinf @ (ccfv @ (XA2 @ Xj @ Xk) @ (XL @ Xz @ Xi @ Xj @ Xk @ Xa)) @ ccxr @ cclt))))))))))))))))).
thf(avolicorescl_thm,axiom,(! [XA2:($i > $o)] : ((cwcel @ XA2 @ (ccrn @ (ccres @ ccico @ (ccxp @ ccr @ ccr)))) => (cwcel @ (ccfv @ XA2 @ ccvol) @ ccr)))).
thf(aovnprodcl_thm,axiom,(! [Xph:($i > $o)] : (! [XF:($i > ($i > $o))] : (! [XI:($i > ($i > $o))] : (! [XX:($i > $o)] : (((? [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X)) => (! [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwcel @ XX @ ccfn))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwf @ ccn @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ (XF @ Xk)))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwcel @ (XI @ Xk) @ ccn))) => (! [Xk:$i] : ((Xph @ Xk) => (cwcel @ (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (XI @ Xk) @ (XF @ Xk)))) @ ccvol))) @ (cco @ ccc0 @ ccpnf @ ccico))))))))))))).
thf(ahoiprodcl2_thm,axiom,(! [Xph:($i > $o)] : (! [XI:($i > $o)] : (! [XL:($i > ($i > ($i > $o)))] : (! [XX:($i > $o)] : (((? [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X)) => (! [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwcel @ XX @ ccfn))) => ((! [Xi:$i] : (! [Xk:$i] : ((XL @ Xi @ Xk) = (ccmpt @ (^ [Xi:$i] : (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap)) @ (^ [Xi:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccv @ Xi))) @ ccvol)))))))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwf @ XX @ (ccxp @ ccr @ ccr) @ XI))) => (! [Xi:$i] : (! [Xk:$i] : ((Xph @ Xk) => (cwcel @ (ccfv @ XI @ (XL @ Xi @ Xk)) @ (cco @ ccc0 @ ccpnf @ ccico)))))))))))))).
thf(ahoicvrrex_thm,axiom,(! [Xph:($i > $o)] : (! [XX:($i > $o)] : (! [XY:($i > ($i > ($i > $o)))] : ((! [Xi:$i] : ((Xph @ Xi) => (cwcel @ XX @ ccfn))) => ((! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xi) => (cwss @ (XY @ Xj @ Xk) @ (cco @ ccr @ XX @ ccmap)))))) => (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xi) => (cwrex @ (^ [Xi:$i] : ((cwss @ (XY @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & (ccpnf = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap))))))))))))).
thf(aovnsupge0_thm,axiom,(! [Xph:$o] : (! [XA2:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XM:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > ($i > ($i > $o)))] : ((! [Xz:$i] : (! [Xi:$i] : (Xph => (cwcel @ (XX @ Xz @ Xi) @ ccfn)))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (Xph => (cwss @ (XA2 @ Xz @ Xi @ Xj @ Xk) @ (cco @ ccr @ (XX @ Xz @ Xi) @ ccmap))))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((XM @ Xz @ Xi @ Xj @ Xk) = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (XA2 @ Xz @ Xi @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : (XX @ Xz @ Xi)) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : (XX @ Xz @ Xi)) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ (XX @ Xz @ Xi) @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr))))))) => (! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (Xph => (cwss @ (XM @ Xz @ Xi @ Xj @ Xk) @ (cco @ ccc0 @ ccpnf @ ccicc))))))))))))))).
thf(aovnlecvr_thm,axiom,(! [Xph:$o] : (! [XA2:($i > ($i > ($i > $o)))] : (! [XI:($i > $o)] : (! [XL:($i > ($i > ($i > $o)))] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => ((Xph => (XX != cc0)) => ((! [Xj:$i] : (! [Xk:$i] : ((XL @ Xj @ Xk) = (ccmpt @ (^ [Xi:$i] : (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap)) @ (^ [Xi:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccv @ Xi))) @ ccvol)))))))) => ((Xph => (cwf @ ccn @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ XI)) => ((! [Xj:$i] : (! [Xk:$i] : (Xph => (cwss @ (XA2 @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ XI))))))))))) => (! [Xj:$i] : (! [Xk:$i] : (Xph => (cwbr @ (ccfv @ (XA2 @ Xj @ Xk) @ (ccfv @ XX @ ccovoln)) @ (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccfv @ (ccfv @ (ccv @ Xj) @ XI) @ (XL @ Xj @ Xk)))) @ ccsumge0) @ ccle))))))))))))))).
thf(aovnpnfelsup_thm,axiom,(! [Xph:($i > ($i > $o))] : (! [XA2:($i > ($i > ($i > $o)))] : (! [XM:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((! [Xz:$i] : (! [Xi:$i] : ((Xph @ Xz @ Xi) => (cwcel @ XX @ ccfn)))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi) => (cwss @ (XA2 @ Xj @ Xk) @ (cco @ ccr @ XX @ ccmap))))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((XM @ Xz @ Xi @ Xj @ Xk) = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (XA2 @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr))))))) => (! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xz @ Xi) => (cwcel @ ccpnf @ (XM @ Xz @ Xi @ Xj @ Xk))))))))))))))).
thf(aovnsslelem_thm,axiom,(! [Xph:($i > ($i > $o))] : (! [XA2:($i > ($i > ($i > $o)))] : (! [XB2:($i > ($i > ($i > $o)))] : (! [XM:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XN:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xj @ Xk) => (cwcel @ XX @ ccfn)))) => ((! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xj @ Xk) => (XX != cc0)))) => ((! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xj @ Xk) => (cwss @ (XA2 @ Xj @ Xk) @ (XB2 @ Xj @ Xk))))) => ((! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xj @ Xk) => (cwss @ (XB2 @ Xj @ Xk) @ (cco @ ccr @ XX @ ccmap))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((XM @ Xz @ Xi @ Xj @ Xk) = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (XA2 @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr))))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((XN @ Xz @ Xi @ Xj @ Xk) = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (XB2 @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr))))))) => (! [Xj:$i] : (! [Xk:$i] : ((Xph @ Xj @ Xk) => (cwbr @ (ccfv @ (XA2 @ Xj @ Xk) @ (ccfv @ XX @ ccovoln)) @ (ccfv @ (XB2 @ Xj @ Xk) @ (ccfv @ XX @ ccovoln)) @ ccle))))))))))))))))).
thf(aovnssle_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => ((Xph => (cwss @ XA2 @ XB2)) => ((Xph => (cwss @ XB2 @ (cco @ ccr @ XX @ ccmap))) => (Xph => (cwbr @ (ccfv @ XA2 @ (ccfv @ XX @ ccovoln)) @ (ccfv @ XB2 @ (ccfv @ XX @ ccovoln)) @ ccle)))))))))).
thf(aovnlerp_thm,axiom,(! [Xph:$o] : (! [XA2:($i > ($i > ($i > $o)))] : (! [XE:($i > ($i > ($i > ($i > $o))))] : (! [XM:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => ((Xph => (XX != cc0)) => ((! [Xj:$i] : (! [Xk:$i] : (Xph => (cwss @ (XA2 @ Xj @ Xk) @ (cco @ ccr @ XX @ ccmap))))) => ((! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (Xph => (cwcel @ (XE @ Xi @ Xj @ Xk) @ ccrp))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : ((XM @ Xz @ Xi @ Xj @ Xk) = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (XA2 @ Xj @ Xk) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr))))))) => (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (Xph => (cwrex @ (^ [Xz:$i] : (cwbr @ (ccv @ Xz) @ (cco @ (ccfv @ (XA2 @ Xj @ Xk) @ (ccfv @ XX @ ccovoln)) @ (XE @ Xi @ Xj @ Xk) @ ccxad) @ ccle)) @ (^ [Xz:$i] : (XM @ Xz @ Xi @ Xj @ Xk)))))))))))))))))).
thf(aovnf_thm,axiom,(! [Xph:$o] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => (Xph => (cwf @ (ccpw @ (cco @ ccr @ XX @ ccmap)) @ (cco @ ccc0 @ ccpnf @ ccicc) @ (ccfv @ XX @ ccovoln))))))).
thf(aovncvrrp_thm,axiom,(! [Xph:($i > ($i > $o))] : (! [XA2:($i > ($i > ($i > ($i > $o))))] : (! [XC:($i > ($i > ($i > ($i > ($i > ($i > $o))))))] : (! [XD:($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > $o))))))))] : (! [XE:($i > ($i > ($i > ($i > ($i > ($i > $o))))))] : (! [XL:($i > ($i > ($i > ($i > ($i > ($i > $o))))))] : (! [XX:($i > $o)] : ((! [Xh:$i] : (! [Xl:$i] : ((Xph @ Xh @ Xl) => (cwcel @ XX @ ccfn)))) => ((! [Xh:$i] : (! [Xl:$i] : ((Xph @ Xh @ Xl) => (XX != cc0)))) => ((! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xl:$i] : ((Xph @ Xh @ Xl) => (cwss @ (XA2 @ Xh @ Xj @ Xk) @ (cco @ ccr @ XX @ ccmap))))))) => ((! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xl:$i] : ((Xph @ Xh @ Xl) => (cwcel @ (XE @ Xh @ Xj @ Xk @ Xa @ Xl) @ ccrp))))))) => ((! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xl:$i] : ((XC @ Xh @ Xj @ Xk @ Xa @ Xl) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccrab @ (^ [Xl:$i] : (cwss @ (ccv @ Xa) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xl)))))))))) @ (^ [Xl:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap))))))))))) => ((! [Xh:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xl:$i] : ((XL @ Xh @ Xi @ Xj @ Xk @ Xl) = (ccmpt @ (^ [Xh:$i] : (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap)) @ (^ [Xh:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccv @ Xh))) @ ccvol))))))))))) => ((! [Xe:$i] : (! [Xh:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xl:$i] : ((XD @ Xe @ Xh @ Xi @ Xj @ Xk @ Xa @ Xl) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccmpt @ (^ [Xe:$i] : ccrp) @ (^ [Xe:$i] : (ccrab @ (^ [Xi:$i] : (cwbr @ (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccfv @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)) @ (XL @ Xh @ Xi @ Xj @ Xk @ Xl)))) @ ccsumge0) @ (cco @ (ccfv @ (ccv @ Xa) @ (ccfv @ XX @ ccovoln)) @ (ccv @ Xe) @ ccxad) @ ccle)) @ (^ [Xi:$i] : (ccfv @ (ccv @ Xa) @ (XC @ Xh @ Xj @ Xk @ Xa @ Xl)))))))))))))))) => (! [Xe:$i] : (! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xl:$i] : ((Xph @ Xh @ Xl) => (? [Xi:$i] : (cwcel @ (ccv @ Xi) @ (ccfv @ (XE @ Xh @ Xj @ Xk @ Xa @ Xl) @ (ccfv @ (XA2 @ Xh @ Xj @ Xk) @ (XD @ Xe @ Xh @ Xi @ Xj @ Xk @ Xa @ Xl))))))))))))))))))))))))))).
thf(aovn0lem_thm,axiom,(! [Xph:($i > ($i > $o))] : (! [XI:($i > ($i > $o))] : (! [XM:($i > ($i > ($i > ($i > ($i > ($i > $o))))))] : (! [XX:($i > $o)] : ((! [Xz:$i] : (! [Xi:$i] : ((Xph @ Xz @ Xi) => (cwcel @ XX @ ccfn)))) => ((! [Xz:$i] : (! [Xi:$i] : ((Xph @ Xz @ Xi) => (XX != cc0)))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xl:$i] : ((XM @ Xz @ Xi @ Xj @ Xk @ Xl) = (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr)))))))) => ((! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xl:$i] : ((Xph @ Xz @ Xi) => (cwcel @ (ccinf @ (XM @ Xz @ Xi @ Xj @ Xk @ Xl) @ ccxr @ cclt) @ (cco @ ccc0 @ ccpnf @ ccicc)))))))) => ((! [Xz:$i] : ((XI @ Xz) = (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccmpt @ (^ [Xl:$i] : XX) @ (^ [Xl:$i] : (ccop @ cc1 @ ccc0))))))) => (! [Xz:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xl:$i] : ((Xph @ Xz @ Xi) => ((ccinf @ (XM @ Xz @ Xi @ Xj @ Xk @ Xl) @ ccxr @ cclt) = ccc0))))))))))))))))).
thf(aovn0_thm,axiom,(! [Xph:$o] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => (Xph => ((ccfv @ cc0 @ (ccfv @ XX @ ccovoln)) = ccc0)))))).
thf(aovncl_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => ((Xph => (cwss @ XA2 @ (cco @ ccr @ XX @ ccmap))) => (Xph => (cwcel @ (ccfv @ XA2 @ (ccfv @ XX @ ccovoln)) @ (cco @ ccc0 @ ccpnf @ ccicc))))))))).
thf(aovn02_thm,axiom,((ccfv @ cc0 @ ccovoln) = (ccmpt @ (^ [Xx3:$i] : (ccpw @ (ccsn @ cc0))) @ (^ [Xx3:$i] : ccc0)))).
thf(aovnxrcl_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => ((Xph => (cwss @ XA2 @ (cco @ ccr @ XX @ ccmap))) => (Xph => (cwcel @ (ccfv @ XA2 @ (ccfv @ XX @ ccovoln)) @ ccxr)))))))).
thf(aovnsubaddlem1_thm,axiom,(! [Xph:($i > ($i > $o))] : (! [XA2:($i > ($i > ($i > ($i > $o))))] : (! [XC:($i > ($i > ($i > ($i > ($i > ($i > ($i > $o)))))))] : (! [XD:($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > $o)))))))))] : (! [XE:($i > ($i > ($i > ($i > ($i > ($i > ($i > $o)))))))] : (! [XF:($i > ($i > $o))] : (! [XG:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XI:($i > ($i > ($i > ($i > $o))))] : (! [XL:($i > ($i > ($i > ($i > $o))))] : (! [XX:($i > $o)] : (! [XZ:($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > $o))))))))))] : ((! [Xz:$i] : (! [Xh:$i] : ((Xph @ Xz @ Xh) => (cwcel @ XX @ ccfn)))) => ((! [Xz:$i] : (! [Xh:$i] : ((Xph @ Xz @ Xh) => (XX != cc0)))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xm:$i] : ((Xph @ Xz @ Xh) => (cwf @ ccn @ (ccpw @ (cco @ ccr @ XX @ ccmap)) @ (XA2 @ Xj @ Xk @ Xm)))))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xm:$i] : (! [Xa:$i] : ((Xph @ Xz @ Xh) => (cwcel @ (XE @ Xz @ Xh @ Xj @ Xk @ Xm @ Xa) @ ccrp)))))))) => ((! [Xz:$i] : (! [Xe:$i] : (! [Xh:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xm:$i] : (! [Xn:$i] : (! [Xa:$i] : ((XZ @ Xz @ Xe @ Xh @ Xi @ Xj @ Xk @ Xm @ Xn @ Xa) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (ccv @ Xa) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr)))))))))))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xm:$i] : (! [Xn:$i] : ((XC @ Xz @ Xh @ Xj @ Xk @ Xm @ Xn) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccrab @ (^ [Xh:$i] : (cwss @ (ccv @ Xa) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xh)))))))))) @ (^ [Xh:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))))))))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xk:$i] : ((XL @ Xz @ Xh @ Xk) = (ccmpt @ (^ [Xi:$i] : (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap)) @ (^ [Xi:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccv @ Xi))) @ ccvol))))))))) => ((! [Xz:$i] : (! [Xe:$i] : (! [Xh:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xm:$i] : (! [Xn:$i] : (! [Xa:$i] : ((XD @ Xz @ Xe @ Xh @ Xi @ Xj @ Xk @ Xm @ Xa) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccmpt @ (^ [Xe:$i] : ccrp) @ (^ [Xe:$i] : (ccrab @ (^ [Xi:$i] : (cwbr @ (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccfv @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)) @ (XL @ Xz @ Xh @ Xk)))) @ ccsumge0) @ (cco @ (ccfv @ (ccv @ Xa) @ (ccfv @ XX @ ccovoln)) @ (ccv @ Xe) @ ccxad) @ ccle)) @ (^ [Xi:$i] : (ccfv @ (ccv @ Xa) @ (XC @ Xz @ Xh @ Xj @ Xk @ Xm @ Xn)))))))))))))))))) => ((! [Xz:$i] : (! [Xe:$i] : (! [Xh:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xm:$i] : (! [Xn:$i] : (! [Xa:$i] : (((Xph @ Xz @ Xh) & (cwcel @ (ccv @ Xn) @ ccn)) => (cwcel @ (ccfv @ (ccv @ Xn) @ (XI @ Xz @ Xe @ Xa)) @ (ccfv @ (cco @ (XE @ Xz @ Xh @ Xj @ Xk @ Xm @ Xa) @ (cco @ cc2 @ (ccv @ Xn) @ ccexp) @ ccdiv) @ (ccfv @ (ccfv @ (ccv @ Xn) @ (XA2 @ Xj @ Xk @ Xm)) @ (XD @ Xz @ Xe @ Xh @ Xi @ Xj @ Xk @ Xm @ Xa)))))))))))))) => ((! [Xz:$i] : (! [Xh:$i] : ((Xph @ Xz @ Xh) => (cwf1o @ ccn @ (ccxp @ ccn @ ccn) @ (XF @ Xz))))) => ((! [Xz:$i] : (! [Xe:$i] : (! [Xh:$i] : (! [Xa:$i] : ((XG @ Xz @ Xe @ Xh @ Xa) = (ccmpt @ (^ [Xm:$i] : ccn) @ (^ [Xm:$i] : (ccfv @ (ccfv @ (ccfv @ (ccv @ Xm) @ (XF @ Xz)) @ cc2nd) @ (ccfv @ (ccfv @ (ccfv @ (ccv @ Xm) @ (XF @ Xz)) @ cc1st) @ (XI @ Xz @ Xe @ Xa)))))))))) => (! [Xz:$i] : (! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xm:$i] : (! [Xa:$i] : ((Xph @ Xz @ Xh) => (cwbr @ (ccfv @ (cciun @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccfv @ (ccv @ Xn) @ (XA2 @ Xj @ Xk @ Xm)))) @ (ccfv @ XX @ ccovoln)) @ (cco @ (ccfv @ (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccfv @ (ccfv @ (ccv @ Xn) @ (XA2 @ Xj @ Xk @ Xm)) @ (ccfv @ XX @ ccovoln)))) @ ccsumge0) @ (XE @ Xz @ Xh @ Xj @ Xk @ Xm @ Xa) @ ccxad) @ ccle))))))))))))))))))))))))))))))).
thf(aovnsubaddlem2_thm,axiom,(! [Xph:($i > ($i > ($i > $o)))] : (! [XA2:($i > ($i > $o))] : (! [XC:($i > ($i > ($i > ($i > ($i > ($i > ($i > $o)))))))] : (! [XD:($i > ($i > ($i > ($i > $o))))] : (! [XE:($i > ($i > ($i > ($i > $o))))] : (! [XL:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : (! [XZ:($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > ($i > $o))))))))))] : ((! [Xz:$i] : (! [Xh:$i] : (! [Xl:$i] : ((Xph @ Xz @ Xh @ Xl) => (cwcel @ XX @ ccfn))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xl:$i] : ((Xph @ Xz @ Xh @ Xl) => (XX != cc0))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xl:$i] : ((Xph @ Xz @ Xh @ Xl) => (cwf @ ccn @ (ccpw @ (cco @ ccr @ XX @ ccmap)) @ (XA2 @ Xh)))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xl:$i] : ((Xph @ Xz @ Xh @ Xl) => (cwcel @ (XE @ Xz @ Xh @ Xl) @ ccrp))))) => ((! [Xz:$i] : (! [Xe:$i] : (! [Xh:$i] : (! [Xi:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xn:$i] : (! [Xa:$i] : (! [Xl:$i] : ((XZ @ Xz @ Xe @ Xh @ Xi @ Xj @ Xk @ Xn @ Xa @ Xl) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccrab @ (^ [Xz:$i] : (cwrex @ (^ [Xi:$i] : ((cwss @ (ccv @ Xa) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi))))))))) & ((ccv @ Xz) = (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)))) @ ccvol))))) @ ccsumge0)))) @ (^ [Xi:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))) @ (^ [Xz:$i] : ccxr)))))))))))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xn:$i] : (! [Xl:$i] : ((XC @ Xz @ Xh @ Xj @ Xk @ Xn @ Xl) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccrab @ (^ [Xl:$i] : (cwss @ (ccv @ Xa) @ (cciun @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccfv @ (ccv @ Xj) @ (ccv @ Xl)))))))))) @ (^ [Xl:$i] : (cco @ (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap) @ ccn @ ccmap)))))))))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xk:$i] : (! [Xl:$i] : ((XL @ Xz @ Xh @ Xk @ Xl) = (ccmpt @ (^ [Xh:$i] : (cco @ (ccxp @ ccr @ ccr) @ XX @ ccmap)) @ (^ [Xh:$i] : (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (ccfv @ (ccv @ Xk) @ (cccom @ ccico @ (ccv @ Xh))) @ ccvol)))))))))) => ((! [Xz:$i] : (! [Xh:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xn:$i] : (! [Xl:$i] : ((XD @ Xz @ Xh @ Xl) = (ccmpt @ (^ [Xa:$i] : (ccpw @ (cco @ ccr @ XX @ ccmap))) @ (^ [Xa:$i] : (ccmpt @ (^ [Xe:$i] : ccrp) @ (^ [Xe:$i] : (ccrab @ (^ [Xi:$i] : (cwbr @ (ccfv @ (ccmpt @ (^ [Xj:$i] : ccn) @ (^ [Xj:$i] : (ccfv @ (ccfv @ (ccv @ Xj) @ (ccv @ Xi)) @ (XL @ Xz @ Xh @ Xk @ Xl)))) @ ccsumge0) @ (cco @ (ccfv @ (ccv @ Xa) @ (ccfv @ XX @ ccovoln)) @ (ccv @ Xe) @ ccxad) @ ccle)) @ (^ [Xi:$i] : (ccfv @ (ccv @ Xa) @ (XC @ Xz @ Xh @ Xj @ Xk @ Xn @ Xl))))))))))))))) => (! [Xz:$i] : (! [Xh:$i] : (! [Xl:$i] : ((Xph @ Xz @ Xh @ Xl) => (cwbr @ (ccfv @ (cciun @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccfv @ (ccv @ Xn) @ (XA2 @ Xh)))) @ (ccfv @ XX @ ccovoln)) @ (cco @ (ccfv @ (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccfv @ (ccfv @ (ccv @ Xn) @ (XA2 @ Xh)) @ (ccfv @ XX @ ccovoln)))) @ ccsumge0) @ (XE @ Xz @ Xh @ Xl) @ ccxad) @ ccle)))))))))))))))))))))).
thf(aovnsubadd_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => ((Xph => (cwf @ ccn @ (ccpw @ (cco @ ccr @ XX @ ccmap)) @ XA2)) => (Xph => (cwbr @ (ccfv @ (cciun @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccfv @ (ccv @ Xn) @ XA2))) @ (ccfv @ XX @ ccovoln)) @ (ccfv @ (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccfv @ (ccfv @ (ccv @ Xn) @ XA2) @ (ccfv @ XX @ ccovoln)))) @ ccsumge0) @ ccle)))))))).
thf(aovnome_thm,axiom,(! [Xph:$o] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => (Xph => (cwcel @ (ccfv @ XX @ ccovoln) @ ccome)))))).
thf(avonmea_thm,axiom,(! [Xph:$o] : (! [XX:($i > $o)] : ((Xph => (cwcel @ XX @ ccfn)) => (Xph => (cwcel @ (ccfv @ XX @ ccvoln) @ ccmea)))))).
thf(avolicon0_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : ((Xph => (cwcel @ XA2 @ ccr)) => ((Xph => (cwcel @ XB2 @ ccr)) => ((Xph => (cwbr @ XA2 @ XB2 @ cclt)) => (Xph => ((ccfv @ (cco @ XA2 @ XB2 @ ccico) @ ccvol) = (cco @ XB2 @ XA2 @ ccmin)))))))))).
thf(ahsphoif_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : (! [XH:($i > ($i > ($i > ($i > $o))))] : (! [XV:($i > ($i > ($i > ($i > $o))))] : (! [XX:($i > $o)] : (! [XY:($i > ($i > $o))] : ((! [Xx3:$i] : (! [Xj:$i] : (! [Xa:$i] : ((XH @ Xx3 @ Xj @ Xa) = (ccmpt @ (^ [Xx3:$i] : ccr) @ (^ [Xx3:$i] : (ccmpt @ (^ [Xa:$i] : (cco @ ccr @ XX @ ccmap)) @ (^ [Xa:$i] : (ccmpt @ (^ [Xj:$i] : XX) @ (^ [Xj:$i] : (ccif @ (cwcel @ (ccv @ Xj) @ (XY @ Xj)) @ (ccfv @ (ccv @ Xj) @ (ccv @ Xa)) @ (ccif @ (cwbr @ (ccfv @ (ccv @ Xj) @ (ccv @ Xa)) @ (ccv @ Xx3) @ ccle) @ (ccfv @ (ccv @ Xj) @ (ccv @ Xa)) @ (ccv @ Xx3))))))))))))) => ((Xph => (cwcel @ XA2 @ ccr)) => ((! [Xx3:$i] : (! [Xj:$i] : (! [Xa:$i] : (Xph => (cwcel @ XX @ (XV @ Xx3 @ Xj @ Xa)))))) => ((! [Xx3:$i] : (Xph => (cwf @ XX @ ccr @ (XB2 @ Xx3)))) => (! [Xx3:$i] : (! [Xj:$i] : (! [Xa:$i] : (Xph => (cwf @ XX @ ccr @ (ccfv @ (XB2 @ Xx3) @ (ccfv @ XA2 @ (XH @ Xx3 @ Xj @ Xa)))))))))))))))))))).
thf(ahoidmvval_thm,axiom,(! [Xph:($i > $o)] : (! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : (! [XL:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((! [Xx3:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : ((XL @ Xx3 @ Xk @ Xa @ Xb) = (ccmpt @ (^ [Xx3:$i] : ccfn) @ (^ [Xx3:$i] : (ccmpt2 @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (ccif @ ((ccv @ Xx3) = cc0) @ ccc0 @ (ccprod @ (^ [Xk:$i] : (ccv @ Xx3)) @ (^ [Xk:$i] : (ccfv @ (cco @ (ccfv @ (ccv @ Xk) @ (ccv @ Xa)) @ (ccfv @ (ccv @ Xk) @ (ccv @ Xb)) @ ccico) @ ccvol)))))))))))))) => ((! [Xx3:$i] : (! [Xk:$i] : ((Xph @ Xk) => (cwf @ XX @ ccr @ (XA2 @ Xx3))))) => ((! [Xx3:$i] : (! [Xk:$i] : ((Xph @ Xk) => (cwf @ XX @ ccr @ (XB2 @ Xx3))))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwcel @ XX @ ccfn))) => (! [Xx3:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : ((Xph @ Xk) => ((cco @ (XA2 @ Xx3) @ (XB2 @ Xx3) @ (ccfv @ XX @ (XL @ Xx3 @ Xk @ Xa @ Xb))) = (ccif @ (XX = cc0) @ ccc0 @ (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (cco @ (ccfv @ (ccv @ Xk) @ (XA2 @ Xx3)) @ (ccfv @ (ccv @ Xk) @ (XB2 @ Xx3)) @ ccico) @ ccvol)))))))))))))))))))).
thf(ahoissrrn2_thm,axiom,(! [Xph:($i > $o)] : (! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : (! [XX:($i > $o)] : (((? [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X)) => (! [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X))) => ((! [Xk:$i] : (((Xph @ Xk) & (cwcel @ (ccv @ Xk) @ XX)) => (cwcel @ (XA2 @ Xk) @ ccr))) => ((! [Xk:$i] : (((Xph @ Xk) & (cwcel @ (ccv @ Xk) @ XX)) => (cwcel @ (XB2 @ Xk) @ ccxr))) => (! [Xk:$i] : ((Xph @ Xk) => (cwss @ (ccixp @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (cco @ (XA2 @ Xk) @ (XB2 @ Xk) @ ccico))) @ (cco @ ccr @ XX @ ccmap)))))))))))).
thf(ahsphoival_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : (! [XH:($i > ($i > ($i > ($i > $o))))] : (! [XK:($i > ($i > ($i > $o)))] : (! [XV:($i > ($i > ($i > ($i > $o))))] : (! [XX:($i > $o)] : (! [XY:($i > $o)] : ((! [Xx3:$i] : (! [Xj:$i] : (! [Xa:$i] : ((XH @ Xx3 @ Xj @ Xa) = (ccmpt @ (^ [Xx3:$i] : ccr) @ (^ [Xx3:$i] : (ccmpt @ (^ [Xa:$i] : (cco @ ccr @ XX @ ccmap)) @ (^ [Xa:$i] : (ccmpt @ (^ [Xj:$i] : XX) @ (^ [Xj:$i] : (ccif @ (cwcel @ (ccv @ Xj) @ XY) @ (ccfv @ (ccv @ Xj) @ (ccv @ Xa)) @ (ccif @ (cwbr @ (ccfv @ (ccv @ Xj) @ (ccv @ Xa)) @ (ccv @ Xx3) @ ccle) @ (ccfv @ (ccv @ Xj) @ (ccv @ Xa)) @ (ccv @ Xx3))))))))))))) => ((Xph => (cwcel @ XA2 @ ccr)) => ((! [Xx3:$i] : (! [Xj:$i] : (! [Xa:$i] : (Xph => (cwcel @ XX @ (XV @ Xx3 @ Xj @ Xa)))))) => ((! [Xx3:$i] : (Xph => (cwf @ XX @ ccr @ (XB2 @ Xx3)))) => ((! [Xx3:$i] : (! [Xa:$i] : (Xph => (cwcel @ (XK @ Xx3 @ Xa) @ XX)))) => (! [Xx3:$i] : (! [Xj:$i] : (! [Xa:$i] : (Xph => ((ccfv @ (XK @ Xx3 @ Xa) @ (ccfv @ (XB2 @ Xx3) @ (ccfv @ XA2 @ (XH @ Xx3 @ Xj @ Xa)))) = (ccif @ (cwcel @ (XK @ Xx3 @ Xa) @ XY) @ (ccfv @ (XK @ Xx3 @ Xa) @ (XB2 @ Xx3)) @ (ccif @ (cwbr @ (ccfv @ (XK @ Xx3 @ Xa) @ (XB2 @ Xx3)) @ XA2 @ ccle) @ (ccfv @ (XK @ Xx3 @ Xa) @ (XB2 @ Xx3)) @ XA2))))))))))))))))))))).
thf(ahoiprodcl3_thm,axiom,(! [Xph:($i > $o)] : (! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : (! [XX:($i > $o)] : (((? [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X)) => (! [X:$i] : ((^ [Xk:$i] : (Xph @ Xk)) @ X))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwcel @ XX @ ccfn))) => ((! [Xk:$i] : (((Xph @ Xk) & (cwcel @ (ccv @ Xk) @ XX)) => (cwcel @ (XA2 @ Xk) @ ccr))) => ((! [Xk:$i] : (((Xph @ Xk) & (cwcel @ (ccv @ Xk) @ XX)) => (cwcel @ (XB2 @ Xk) @ ccr))) => (! [Xk:$i] : ((Xph @ Xk) => (cwcel @ (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (cco @ (XA2 @ Xk) @ (XB2 @ Xk) @ ccico) @ ccvol))) @ (cco @ ccc0 @ ccpnf @ ccico))))))))))))).
thf(avolicore_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (((cwcel @ XA2 @ ccr) & (cwcel @ XB2 @ ccr)) => (cwcel @ (ccfv @ (cco @ XA2 @ XB2 @ ccico) @ ccvol) @ ccr))))).
thf(ahoidmvcl_thm,axiom,(! [Xph:$o] : (! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : (! [XL:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((! [Xx3:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : ((XL @ Xx3 @ Xk @ Xa @ Xb) = (ccmpt @ (^ [Xx3:$i] : ccfn) @ (^ [Xx3:$i] : (ccmpt2 @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (ccif @ ((ccv @ Xx3) = cc0) @ ccc0 @ (ccprod @ (^ [Xk:$i] : (ccv @ Xx3)) @ (^ [Xk:$i] : (ccfv @ (cco @ (ccfv @ (ccv @ Xk) @ (ccv @ Xa)) @ (ccfv @ (ccv @ Xk) @ (ccv @ Xb)) @ ccico) @ ccvol)))))))))))))) => ((Xph => (cwcel @ XX @ ccfn)) => ((! [Xx3:$i] : (Xph => (cwf @ XX @ ccr @ (XA2 @ Xx3)))) => ((! [Xx3:$i] : (Xph => (cwf @ XX @ ccr @ (XB2 @ Xx3)))) => (! [Xx3:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (Xph => (cwcel @ (cco @ (XA2 @ Xx3) @ (XB2 @ Xx3) @ (ccfv @ XX @ (XL @ Xx3 @ Xk @ Xa @ Xb))) @ (cco @ ccc0 @ ccpnf @ ccico))))))))))))))))).
thf(ahoidmv0val_thm,axiom,(! [Xph:($i > $o)] : (! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : (! [XL:($i > ($i > ($i > ($i > ($i > $o)))))] : ((! [Xx3:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : ((XL @ Xx3 @ Xk @ Xa @ Xb) = (ccmpt @ (^ [Xx3:$i] : ccfn) @ (^ [Xx3:$i] : (ccmpt2 @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (ccif @ ((ccv @ Xx3) = cc0) @ ccc0 @ (ccprod @ (^ [Xk:$i] : (ccv @ Xx3)) @ (^ [Xk:$i] : (ccfv @ (cco @ (ccfv @ (ccv @ Xk) @ (ccv @ Xa)) @ (ccfv @ (ccv @ Xk) @ (ccv @ Xb)) @ ccico) @ ccvol)))))))))))))) => ((! [Xx3:$i] : (! [Xk:$i] : ((Xph @ Xk) => (cwf @ cc0 @ ccr @ (XA2 @ Xx3))))) => ((! [Xx3:$i] : (! [Xk:$i] : ((Xph @ Xk) => (cwf @ cc0 @ ccr @ (XB2 @ Xx3))))) => (! [Xx3:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : ((Xph @ Xk) => ((cco @ (XA2 @ Xx3) @ (XB2 @ Xx3) @ (ccfv @ cc0 @ (XL @ Xx3 @ Xk @ Xa @ Xb))) = ccc0)))))))))))))).
thf(ahoidmvn0val_thm,axiom,(! [Xph:($i > $o)] : (! [XA2:($i > ($i > $o))] : (! [XB2:($i > ($i > $o))] : (! [XL:($i > ($i > ($i > ($i > ($i > $o)))))] : (! [XX:($i > $o)] : ((! [Xx3:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : ((XL @ Xx3 @ Xk @ Xa @ Xb) = (ccmpt @ (^ [Xx3:$i] : ccfn) @ (^ [Xx3:$i] : (ccmpt2 @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (ccif @ ((ccv @ Xx3) = cc0) @ ccc0 @ (ccprod @ (^ [Xk:$i] : (ccv @ Xx3)) @ (^ [Xk:$i] : (ccfv @ (cco @ (ccfv @ (ccv @ Xk) @ (ccv @ Xa)) @ (ccfv @ (ccv @ Xk) @ (ccv @ Xb)) @ ccico) @ ccvol)))))))))))))) => ((! [Xk:$i] : ((Xph @ Xk) => (cwcel @ XX @ ccfn))) => ((! [Xk:$i] : ((Xph @ Xk) => (XX != cc0))) => ((! [Xx3:$i] : (! [Xk:$i] : ((Xph @ Xk) => (cwf @ XX @ ccr @ (XA2 @ Xx3))))) => ((! [Xx3:$i] : (! [Xk:$i] : ((Xph @ Xk) => (cwf @ XX @ ccr @ (XB2 @ Xx3))))) => (! [Xx3:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : ((Xph @ Xk) => ((cco @ (XA2 @ Xx3) @ (XB2 @ Xx3) @ (ccfv @ XX @ (XL @ Xx3 @ Xk @ Xa @ Xb))) = (ccprod @ (^ [Xk:$i] : XX) @ (^ [Xk:$i] : (ccfv @ (cco @ (ccfv @ (ccv @ Xk) @ (XA2 @ Xx3)) @ (ccfv @ (ccv @ Xk) @ (XB2 @ Xx3)) @ ccico) @ ccvol)))))))))))))))))))).
thf(ahsphoidmvle2_thm,axiom,(! [Xph:$o] : (! [XA2:($i > ($i > ($i > ($i > $o))))] : (! [XB2:($i > ($i > $o))] : (! [XC:($i > $o)] : (! [XD:($i > $o)] : (! [XH:($i > ($i > ($i > ($i > $o))))] : (! [XL:($i > ($i > ($i > ($i > ($i > ($i > ($i > $o)))))))] : (! [XX:($i > $o)] : (! [XY:($i > ($i > ($i > ($i > $o))))] : (! [XZ:($i > ($i > ($i > $o)))] : ((! [Xx3:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (! [Xc:$i] : ((XL @ Xx3 @ Xj @ Xk @ Xa @ Xb @ Xc) = (ccmpt @ (^ [Xx3:$i] : ccfn) @ (^ [Xx3:$i] : (ccmpt2 @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (ccif @ ((ccv @ Xx3) = cc0) @ ccc0 @ (ccprod @ (^ [Xk:$i] : (ccv @ Xx3)) @ (^ [Xk:$i] : (ccfv @ (cco @ (ccfv @ (ccv @ Xk) @ (ccv @ Xa)) @ (ccfv @ (ccv @ Xk) @ (ccv @ Xb)) @ ccico) @ ccvol)))))))))))))))) => ((Xph => (cwcel @ XX @ ccfn)) => ((! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (Xph => (cwcel @ (XZ @ Xa @ Xb) @ (ccdif @ XX @ (XY @ Xk @ Xa @ Xb))))))) => ((! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (XX = (ccun @ (XY @ Xk @ Xa @ Xb) @ (ccsn @ (XZ @ Xa @ Xb))))))) => ((Xph => (cwcel @ XC @ ccr)) => ((Xph => (cwcel @ XD @ ccr)) => ((Xph => (cwbr @ XC @ XD @ ccle)) => ((! [Xx3:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (! [Xc:$i] : ((XH @ Xx3 @ Xj @ Xc) = (ccmpt @ (^ [Xx3:$i] : ccr) @ (^ [Xx3:$i] : (ccmpt @ (^ [Xc:$i] : (cco @ ccr @ XX @ ccmap)) @ (^ [Xc:$i] : (ccmpt @ (^ [Xj:$i] : XX) @ (^ [Xj:$i] : (ccif @ (cwcel @ (ccv @ Xj) @ (XY @ Xk @ Xa @ Xb)) @ (ccfv @ (ccv @ Xj) @ (ccv @ Xc)) @ (ccif @ (cwbr @ (ccfv @ (ccv @ Xj) @ (ccv @ Xc)) @ (ccv @ Xx3) @ ccle) @ (ccfv @ (ccv @ Xj) @ (ccv @ Xc)) @ (ccv @ Xx3)))))))))))))))) => ((! [Xx3:$i] : (! [Xj:$i] : (! [Xc:$i] : (Xph => (cwf @ XX @ ccr @ (XA2 @ Xx3 @ Xj @ Xc)))))) => ((! [Xx3:$i] : (Xph => (cwf @ XX @ ccr @ (XB2 @ Xx3)))) => (! [Xx3:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (! [Xc:$i] : (Xph => (cwbr @ (cco @ (XA2 @ Xx3 @ Xj @ Xc) @ (ccfv @ (XB2 @ Xx3) @ (ccfv @ XC @ (XH @ Xx3 @ Xj @ Xc))) @ (ccfv @ XX @ (XL @ Xx3 @ Xj @ Xk @ Xa @ Xb @ Xc))) @ (cco @ (XA2 @ Xx3 @ Xj @ Xc) @ (ccfv @ (XB2 @ Xx3) @ (ccfv @ XD @ (XH @ Xx3 @ Xj @ Xc))) @ (ccfv @ XX @ (XL @ Xx3 @ Xj @ Xk @ Xa @ Xb @ Xc))) @ ccle))))))))))))))))))))))))))))).
thf(ahsphoidmvle_thm,conjecture,(! [Xph:$o] : (! [XA2:($i > ($i > ($i > ($i > $o))))] : (! [XB2:($i > ($i > $o))] : (! [XC:($i > $o)] : (! [XH:($i > ($i > ($i > ($i > $o))))] : (! [XL:($i > ($i > ($i > ($i > ($i > ($i > ($i > $o)))))))] : (! [XX:($i > $o)] : (! [XY:($i > ($i > ($i > ($i > $o))))] : (! [XZ:($i > ($i > ($i > $o)))] : ((! [Xx3:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (! [Xc:$i] : ((XL @ Xx3 @ Xj @ Xk @ Xa @ Xb @ Xc) = (ccmpt @ (^ [Xx3:$i] : ccfn) @ (^ [Xx3:$i] : (ccmpt2 @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (cco @ ccr @ (ccv @ Xx3) @ ccmap))) @ (^ [Xa:$i] : (^ [Xb:$i] : (ccif @ ((ccv @ Xx3) = cc0) @ ccc0 @ (ccprod @ (^ [Xk:$i] : (ccv @ Xx3)) @ (^ [Xk:$i] : (ccfv @ (cco @ (ccfv @ (ccv @ Xk) @ (ccv @ Xa)) @ (ccfv @ (ccv @ Xk) @ (ccv @ Xb)) @ ccico) @ ccvol)))))))))))))))) => ((Xph => (cwcel @ XX @ ccfn)) => ((! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (Xph => (cwcel @ (XZ @ Xa @ Xb) @ (ccdif @ XX @ (XY @ Xk @ Xa @ Xb))))))) => ((! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (XX = (ccun @ (XY @ Xk @ Xa @ Xb) @ (ccsn @ (XZ @ Xa @ Xb))))))) => ((Xph => (cwcel @ XC @ ccr)) => ((! [Xx3:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (! [Xc:$i] : ((XH @ Xx3 @ Xj @ Xc) = (ccmpt @ (^ [Xx3:$i] : ccr) @ (^ [Xx3:$i] : (ccmpt @ (^ [Xc:$i] : (cco @ ccr @ XX @ ccmap)) @ (^ [Xc:$i] : (ccmpt @ (^ [Xj:$i] : XX) @ (^ [Xj:$i] : (ccif @ (cwcel @ (ccv @ Xj) @ (XY @ Xk @ Xa @ Xb)) @ (ccfv @ (ccv @ Xj) @ (ccv @ Xc)) @ (ccif @ (cwbr @ (ccfv @ (ccv @ Xj) @ (ccv @ Xc)) @ (ccv @ Xx3) @ ccle) @ (ccfv @ (ccv @ Xj) @ (ccv @ Xc)) @ (ccv @ Xx3)))))))))))))))) => ((! [Xx3:$i] : (! [Xj:$i] : (! [Xc:$i] : (Xph => (cwf @ XX @ ccr @ (XA2 @ Xx3 @ Xj @ Xc)))))) => ((! [Xx3:$i] : (Xph => (cwf @ XX @ ccr @ (XB2 @ Xx3)))) => (! [Xx3:$i] : (! [Xj:$i] : (! [Xk:$i] : (! [Xa:$i] : (! [Xb:$i] : (! [Xc:$i] : (Xph => (cwbr @ (cco @ (XA2 @ Xx3 @ Xj @ Xc) @ (ccfv @ (XB2 @ Xx3) @ (ccfv @ XC @ (XH @ Xx3 @ Xj @ Xc))) @ (ccfv @ XX @ (XL @ Xx3 @ Xj @ Xk @ Xa @ Xb @ Xc))) @ (cco @ (XA2 @ Xx3 @ Xj @ Xc) @ (XB2 @ Xx3) @ (ccfv @ XX @ (XL @ Xx3 @ Xj @ Xk @ Xa @ Xb @ Xc))) @ ccle)))))))))))))))))))))))))).
