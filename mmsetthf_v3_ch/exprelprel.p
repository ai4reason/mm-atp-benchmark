...
thf(aexprelprel_thm,conjecture,(! [XV:($i > $o)] : (! [XX:($i > ($i > $o))] : (! [Xe:$i] : ((cwrex @ (^ [Xp:$i] : (cwcel @ (ccv @ Xp) @ (XX @ Xe))) @ (^ [Xp:$i] : (ccrab @ (^ [Xe:$i] : ((ccfv @ (ccv @ Xe) @ cchash) = cc2)) @ (^ [Xe:$i] : (ccpw @ XV))))) => (cwrex @ (^ [Xv:$i] : (cwrex @ (^ [Xw:$i] : (cwcel @ (ccpr @ (ccv @ Xv) @ (ccv @ Xw)) @ (XX @ Xe))) @ (^ [Xw:$i] : XV))) @ (^ [Xv:$i] : XV))))))).
