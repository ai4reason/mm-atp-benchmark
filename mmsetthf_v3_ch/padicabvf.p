...
thf(apadicabvf_thm,conjecture,(! [XA2:($i > $o)] : (! [XQ:($i > ($i > $o))] : (! [XJ:($i > ($i > ($i > $o)))] : ((! [Xq:$i] : ((XQ @ Xq) = (cco @ cccnfld @ ccq @ ccress))) => ((! [Xq:$i] : (XA2 = (ccfv @ (XQ @ Xq) @ ccabv))) => ((! [Xx3:$i] : (! [Xq:$i] : ((XJ @ Xx3 @ Xq) = (ccmpt @ (^ [Xq:$i] : ccprime) @ (^ [Xq:$i] : (ccmpt @ (^ [Xx3:$i] : ccq) @ (^ [Xx3:$i] : (ccif @ ((ccv @ Xx3) = ccc0) @ ccc0 @ (cco @ (ccv @ Xq) @ (ccneg @ (cco @ (ccv @ Xq) @ (ccv @ Xx3) @ ccpc)) @ ccexp))))))))) => (! [Xx3:$i] : (! [Xq:$i] : (cwf @ ccprime @ XA2 @ (XJ @ Xx3 @ Xq))))))))))).
