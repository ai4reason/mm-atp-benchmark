...
thf(acnheibor_thm,conjecture,(! [XT:($i > $o)] : (! [XJ:($i > $o)] : (! [XX:($i > $o)] : ((XJ = (ccfv @ cccnfld @ cctopn)) => ((XT = (cco @ XJ @ XX @ ccrest)) => ((cwss @ XX @ ccc) => ((cwcel @ XT @ cccmp) <=> ((cwcel @ XX @ (ccfv @ XJ @ cccld)) & (cwrex @ (^ [Xr:$i] : (cwral @ (^ [Xx3:$i] : (cwbr @ (ccfv @ (ccv @ Xx3) @ ccabs) @ (ccv @ Xr) @ ccle)) @ (^ [Xx3:$i] : XX))) @ (^ [Xr:$i] : ccr))))))))))).
