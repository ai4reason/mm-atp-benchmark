...
thf(alcmf_thm,conjecture,(! [XK:($i > $o)] : (! [XZ:($i > $o)] : (((cwcel @ XK @ ccn) & ((cwss @ XZ @ ccz) & (cwcel @ XZ @ ccfn) & (~ (wnel @ ccc0 @ XZ)))) => ((XK = (ccfv @ XZ @ cclcmf)) <=> ((cwral @ (^ [Xm:$i] : (cwbr @ (ccv @ Xm) @ XK @ ccdvds)) @ (^ [Xm:$i] : XZ)) & (cwral @ (^ [Xk:$i] : ((cwral @ (^ [Xm:$i] : (cwbr @ (ccv @ Xm) @ (ccv @ Xk) @ ccdvds)) @ (^ [Xm:$i] : XZ)) => (cwbr @ XK @ (ccv @ Xk) @ ccle))) @ (^ [Xk:$i] : ccn)))))))).
