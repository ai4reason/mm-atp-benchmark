...
thf(aordtbaslem_thm,conjecture,(! [XA2:($i > ($i > ($i > $o)))] : (! [XR:($i > $o)] : (! [XX:($i > $o)] : ((XX = (ccdm @ XR)) => ((! [Xx3:$i] : (! [Xy1:$i] : ((XA2 @ Xx3 @ Xy1) = (ccrn @ (ccmpt @ (^ [Xx3:$i] : XX) @ (^ [Xx3:$i] : (ccrab @ (^ [Xy1:$i] : (~ (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ XR))) @ (^ [Xy1:$i] : XX)))))))) => (! [Xx3:$i] : (! [Xy1:$i] : ((cwcel @ XR @ cctsr) => ((ccfv @ (XA2 @ Xx3 @ Xy1) @ ccfi) = (XA2 @ Xx3 @ Xy1))))))))))).
