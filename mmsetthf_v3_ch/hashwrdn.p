...
thf(ahashwrdn_thm,conjecture,(! [XN:($i > $o)] : (! [XV:($i > $o)] : (((cwcel @ XV @ ccfn) & (cwcel @ XN @ ccn0)) => ((ccfv @ (ccrab @ (^ [Xw:$i] : ((ccfv @ (ccv @ Xw) @ cchash) = XN)) @ (^ [Xw:$i] : (ccword @ XV))) @ cchash) = (cco @ (ccfv @ XV @ cchash) @ XN @ ccexp)))))).
