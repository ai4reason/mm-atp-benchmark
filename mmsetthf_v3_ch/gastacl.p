...
thf(agastacl_thm,conjecture,(! [XA2:($i > $o)] : (! [Xc_po:($i > $o)] : (! [XG:($i > $o)] : (! [XH:($i > ($i > $o))] : (! [XX:($i > $o)] : (! [XY:($i > ($i > $o))] : ((XX = (ccfv @ XG @ ccbs)) => ((! [Xu:$i] : ((XH @ Xu) = (ccrab @ (^ [Xu:$i] : ((cco @ (ccv @ Xu) @ XA2 @ Xc_po) = XA2)) @ (^ [Xu:$i] : XX)))) => (! [Xu:$i] : (((cwcel @ Xc_po @ (cco @ XG @ (XY @ Xu) @ ccga)) & (cwcel @ XA2 @ (XY @ Xu))) => (cwcel @ (XH @ Xu) @ (ccfv @ XG @ ccsubg))))))))))))).
