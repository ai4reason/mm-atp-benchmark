...
thf(alcmfnnval_thm,conjecture,(! [XZ:($i > $o)] : (((cwss @ XZ @ ccn) & (cwcel @ XZ @ ccfn)) => ((ccfv @ XZ @ cclcmf) = (ccinf @ (ccrab @ (^ [Xn:$i] : (cwral @ (^ [Xm:$i] : (cwbr @ (ccv @ Xm) @ (ccv @ Xn) @ ccdvds)) @ (^ [Xm:$i] : XZ))) @ (^ [Xn:$i] : ccn)) @ ccr @ cclt))))).
