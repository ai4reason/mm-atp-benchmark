...
thf(aelply_thm,conjecture,(! [XS:($i > $o)] : (! [XF:($i > ($i > ($i > $o)))] : (! [Xz:$i] : (! [Xk:$i] : ((cwcel @ (XF @ Xz @ Xk) @ (ccfv @ XS @ ccply)) <=> ((cwss @ XS @ ccc) & (cwrex @ (^ [Xn:$i] : (cwrex @ (^ [Xa:$i] : ((XF @ Xz @ Xk) = (ccmpt @ (^ [Xz:$i] : ccc) @ (^ [Xz:$i] : (ccsu @ (cco @ ccc0 @ (ccv @ Xn) @ ccfz) @ (^ [Xk:$i] : (cco @ (ccfv @ (ccv @ Xk) @ (ccv @ Xa)) @ (cco @ (ccv @ Xz) @ (ccv @ Xk) @ ccexp) @ ccmul))))))) @ (^ [Xa:$i] : (cco @ (ccun @ XS @ (ccsn @ ccc0)) @ ccn0 @ ccmap)))) @ (^ [Xn:$i] : ccn0))))))))).
