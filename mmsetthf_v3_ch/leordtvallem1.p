...
thf(aleordtvallem1_thm,conjecture,(! [XA2:($i > ($i > ($i > $o)))] : ((! [Xx3:$i] : (! [Xy1:$i] : ((XA2 @ Xx3 @ Xy1) = (ccrn @ (ccmpt @ (^ [Xx3:$i] : ccxr) @ (^ [Xx3:$i] : (cco @ (ccv @ Xx3) @ ccpnf @ ccioc))))))) => (! [Xx3:$i] : (! [Xy1:$i] : ((XA2 @ Xx3 @ Xy1) = (ccrn @ (ccmpt @ (^ [Xx3:$i] : ccxr) @ (^ [Xx3:$i] : (ccrab @ (^ [Xy1:$i] : (~ (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccle))) @ (^ [Xy1:$i] : ccxr))))))))))).
