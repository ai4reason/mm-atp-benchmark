...
thf(areperflem_thm,conjecture,(! [XS:($i > $o)] : (! [XJ:($i > ($i > $o))] : ((! [Xv:$i] : ((XJ @ Xv) = (ccfv @ cccnfld @ cctopn))) => ((! [Xv:$i] : (! [Xu:$i] : (((cwcel @ (ccv @ Xu) @ XS) & (cwcel @ (ccv @ Xv) @ ccr)) => (cwcel @ (cco @ (ccv @ Xu) @ (ccv @ Xv) @ ccaddc) @ XS)))) => ((cwss @ XS @ ccc) => (! [Xv:$i] : (cwcel @ (cco @ (XJ @ Xv) @ XS @ ccrest) @ ccperf)))))))).
