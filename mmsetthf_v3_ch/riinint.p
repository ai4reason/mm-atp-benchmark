...
thf(ariinint_thm,conjecture,(! [XS:($i > ($i > $o))] : (! [XI:($i > ($i > $o))] : (! [XV:($i > $o)] : (! [XX:($i > $o)] : (((cwcel @ XX @ XV) & (cwral @ (^ [Xk:$i] : (cwss @ (XS @ Xk) @ XX)) @ (^ [Xk:$i] : (XI @ Xk)))) => ((ccin @ XX @ (cciin @ (^ [Xk:$i] : (XI @ Xk)) @ (^ [Xk:$i] : (XS @ Xk)))) = (ccint @ (ccun @ (ccsn @ XX) @ (ccrn @ (ccmpt @ (^ [Xk:$i] : (XI @ Xk)) @ (^ [Xk:$i] : (XS @ Xk))))))))))))).
