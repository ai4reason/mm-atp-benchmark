...
thf(avoliunlem2_thm,conjecture,(! [Xph:($i > $o)] : (! [XF:($i > $o)] : (! [XH:($i > ($i > ($i > ($i > $o))))] : ((! [Xi:$i] : ((Xph @ Xi) => (cwf @ ccn @ (ccdm @ ccvol) @ XF))) => ((! [Xi:$i] : ((Xph @ Xi) => (cwdisj @ (^ [Xi:$i] : ccn) @ (^ [Xi:$i] : (ccfv @ (ccv @ Xi) @ XF))))) => ((! [Xx3:$i] : (! [Xi:$i] : (! [Xn:$i] : ((XH @ Xx3 @ Xi @ Xn) = (ccmpt @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (ccfv @ (ccin @ (ccv @ Xx3) @ (ccfv @ (ccv @ Xn) @ XF)) @ ccovol))))))) => (! [Xi:$i] : ((Xph @ Xi) => (cwcel @ (ccuni @ (ccrn @ XF)) @ (ccdm @ ccvol))))))))))).
