...
thf(arepsdf2_thm,conjecture,(! [XS:($i > $o)] : (! [XN:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [XW:($i > $o)] : (! [Xi:$i] : (((cwcel @ XS @ (XV @ Xi)) & (cwcel @ XN @ ccn0)) => ((XW = (cco @ XS @ XN @ ccreps)) <=> ((cwcel @ XW @ (ccword @ (XV @ Xi))) & ((ccfv @ XW @ cchash) = XN) & (cwral @ (^ [Xi:$i] : ((ccfv @ (ccv @ Xi) @ XW) = XS)) @ (^ [Xi:$i] : (cco @ ccc0 @ XN @ ccfzo)))))))))))).
