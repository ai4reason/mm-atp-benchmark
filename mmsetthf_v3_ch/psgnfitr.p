...
thf(apsgnfitr_thm,conjecture,(! [XB2:($i > ($i > $o))] : (! [XQ:($i > $o)] : (! [XT:($i > $o)] : (! [XG:($i > $o)] : (! [XN:($i > ($i > $o))] : ((! [Xw:$i] : (XG = (ccfv @ (XN @ Xw) @ ccsymg))) => ((! [Xw:$i] : ((XB2 @ Xw) = (ccfv @ XG @ ccbs))) => ((! [Xw:$i] : (XT = (ccrn @ (ccfv @ (XN @ Xw) @ ccpmtr)))) => (! [Xw:$i] : ((cwcel @ (XN @ Xw) @ ccfn) => ((cwcel @ XQ @ (XB2 @ Xw)) <=> (cwrex @ (^ [Xw:$i] : (XQ = (cco @ XG @ (ccv @ Xw) @ ccgsu))) @ (^ [Xw:$i] : (ccword @ XT))))))))))))))).
