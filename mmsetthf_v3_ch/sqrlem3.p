...
thf(asqrlem3_thm,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : (! [XS:($i > ($i > $o))] : ((! [Xx3:$i] : ((XS @ Xx3) = (ccrab @ (^ [Xx3:$i] : (cwbr @ (cco @ (ccv @ Xx3) @ cc2 @ ccexp) @ XA2 @ ccle)) @ (^ [Xx3:$i] : ccrp)))) => ((! [Xx3:$i] : ((XB2 @ Xx3) = (ccsup @ (XS @ Xx3) @ ccr @ cclt))) => (! [Xx3:$i] : (((cwcel @ XA2 @ ccrp) & (cwbr @ XA2 @ cc1 @ ccle)) => ((cwss @ (XS @ Xx3) @ ccr) & ((XS @ Xx3) != cc0) & (cwrex @ (^ [Xz:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xy1) @ (ccv @ Xz) @ ccle)) @ (^ [Xy1:$i] : (XS @ Xx3)))) @ (^ [Xz:$i] : ccr))))))))))).
