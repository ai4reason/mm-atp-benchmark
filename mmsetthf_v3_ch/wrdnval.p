...
thf(awrdnval_thm,conjecture,(! [XN:($i > $o)] : (! [XV:($i > $o)] : (! [XX:($i > $o)] : (((cwcel @ XV @ XX) & (cwcel @ XN @ ccn0)) => ((ccrab @ (^ [Xw:$i] : ((ccfv @ (ccv @ Xw) @ cchash) = XN)) @ (^ [Xw:$i] : (ccword @ XV))) = (cco @ XV @ (cco @ ccc0 @ XN @ ccfzo) @ ccmap))))))).
