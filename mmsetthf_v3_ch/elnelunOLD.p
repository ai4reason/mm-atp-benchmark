...
thf(aelnelunOLD_thm,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : (! [XE:($i > ($i > $o))] : (! [XN:($i > ($i > $o))] : ((! [Xs1:$i] : ((XE @ Xs1) = (ccrab @ (^ [Xs1:$i] : (cwcel @ (XB2 @ Xs1) @ (ccv @ Xs1))) @ (^ [Xs1:$i] : XA2)))) => ((! [Xs1:$i] : ((XN @ Xs1) = (ccrab @ (^ [Xs1:$i] : (~ (wnel @ (XB2 @ Xs1) @ (ccv @ Xs1)))) @ (^ [Xs1:$i] : XA2)))) => (! [Xs1:$i] : ((ccun @ (XE @ Xs1) @ (XN @ Xs1)) = XA2))))))))).
