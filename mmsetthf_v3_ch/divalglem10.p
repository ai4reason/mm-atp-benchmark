...
thf(adivalglem10_thm,conjecture,(! [XD:($i > $o)] : (! [XS:($i > ($i > ($i > $o)))] : (! [XN:($i > $o)] : ((cwcel @ XN @ ccz) => ((cwcel @ XD @ ccz) => ((XD != ccc0) => ((! [Xr:$i] : (! [Xq:$i] : ((XS @ Xr @ Xq) = (ccrab @ (^ [Xr:$i] : (cwbr @ XD @ (cco @ XN @ (ccv @ Xr) @ ccmin) @ ccdvds)) @ (^ [Xr:$i] : ccn0))))) => (cwreu @ (^ [Xr:$i] : (cwrex @ (^ [Xq:$i] : ((cwbr @ ccc0 @ (ccv @ Xr) @ ccle) & (cwbr @ (ccv @ Xr) @ (ccfv @ XD @ ccabs) @ cclt) & (XN = (cco @ (cco @ (ccv @ Xq) @ XD @ ccmul) @ (ccv @ Xr) @ ccaddc)))) @ (^ [Xq:$i] : ccz))) @ (^ [Xr:$i] : ccz)))))))))).
