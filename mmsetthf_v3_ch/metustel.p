...
thf(ametustel_thm,conjecture,(! [XB2:($i > $o)] : (! [XD:($i > $o)] : (! [XF:($i > ($i > $o))] : (! [XX:($i > $o)] : ((! [Xa:$i] : ((XF @ Xa) = (ccrn @ (ccmpt @ (^ [Xa:$i] : ccrp) @ (^ [Xa:$i] : (ccima @ (cccnv @ XD) @ (cco @ ccc0 @ (ccv @ Xa) @ ccico))))))) => (! [Xa:$i] : ((cwcel @ XD @ (ccfv @ XX @ ccpsmet)) => ((cwcel @ XB2 @ (XF @ Xa)) <=> (cwrex @ (^ [Xa:$i] : (XB2 = (ccima @ (cccnv @ XD) @ (cco @ ccc0 @ (ccv @ Xa) @ ccico)))) @ (^ [Xa:$i] : ccrp))))))))))).
