...
thf(akqreglem2_thm,conjecture,(! [XF:($i > ($i > ($i > $o)))] : (! [XJ:($i > $o)] : (! [XX:($i > $o)] : ((! [Xx3:$i] : (! [Xy1:$i] : ((XF @ Xx3 @ Xy1) = (ccmpt @ (^ [Xx3:$i] : XX) @ (^ [Xx3:$i] : (ccrab @ (^ [Xy1:$i] : (cwcel @ (ccv @ Xx3) @ (ccv @ Xy1))) @ (^ [Xy1:$i] : XJ))))))) => (((cwcel @ XJ @ (ccfv @ XX @ cctopon)) & (cwcel @ (ccfv @ XJ @ cckq) @ ccreg)) => (cwcel @ XJ @ ccreg))))))).
