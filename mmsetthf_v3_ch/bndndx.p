...
thf(abndndx_thm,conjecture,(! [XA2:($i > ($i > $o))] : ((cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xk:$i] : ((cwcel @ (XA2 @ Xk) @ ccr) & (cwbr @ (XA2 @ Xk) @ (ccv @ Xx3) @ ccle))) @ (^ [Xk:$i] : ccn))) @ (^ [Xx3:$i] : ccr)) => (cwrex @ (^ [Xk:$i] : (cwbr @ (XA2 @ Xk) @ (ccv @ Xk) @ ccle)) @ (^ [Xk:$i] : ccn))))).
