...
thf(alpfval_thm,conjecture,(! [XJ:($i > $o)] : (! [XX:($i > $o)] : ((XX = (ccuni @ XJ)) => ((cwcel @ XJ @ cctop) => ((ccfv @ XJ @ cclp) = (ccmpt @ (^ [Xx3:$i] : (ccpw @ XX)) @ (^ [Xx3:$i] : (^ [Xy1:$i] : (cwcel @ (ccv @ Xy1) @ (ccfv @ (ccdif @ (ccv @ Xx3) @ (ccsn @ (ccv @ Xy1))) @ (ccfv @ XJ @ cccl)))))))))))).
