...
thf(arpnnen1OLD_thm,conjecture,(! [XT:($i > ($i > ($i > $o)))] : (! [XF:($i > $o)] : ((! [Xx3:$i] : (! [Xk:$i] : ((XT @ Xx3 @ Xk) = (ccrab @ (^ [Xn:$i] : (cwbr @ (cco @ (ccv @ Xn) @ (ccv @ Xk) @ ccdiv) @ (ccv @ Xx3) @ cclt)) @ (^ [Xn:$i] : ccz))))) => ((XF = (ccmpt @ (^ [Xx3:$i] : ccr) @ (^ [Xx3:$i] : (ccmpt @ (^ [Xk:$i] : ccn) @ (^ [Xk:$i] : (cco @ (ccsup @ (XT @ Xx3 @ Xk) @ ccr @ cclt) @ (ccv @ Xk) @ ccdiv)))))) => (cwbr @ ccr @ (cco @ ccq @ ccn @ ccmap) @ ccdom)))))).
