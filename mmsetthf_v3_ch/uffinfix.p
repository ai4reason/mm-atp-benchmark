...
thf(auffinfix_thm,conjecture,(! [XS:($i > ($i > ($i > $o)))] : (! [XF:($i > $o)] : (! [XX:($i > $o)] : (! [Xx3:$i] : (! [Xy1:$i] : (((cwcel @ XF @ (ccfv @ XX @ ccufil)) & (cwcel @ (XS @ Xx3 @ Xy1) @ XF) & (cwcel @ (XS @ Xx3 @ Xy1) @ ccfn)) => (cwrex @ (^ [Xx3:$i] : (XF = (ccrab @ (^ [Xy1:$i] : (cwcel @ (ccv @ Xx3) @ (ccv @ Xy1))) @ (^ [Xy1:$i] : (ccpw @ XX))))) @ (^ [Xx3:$i] : XX))))))))).
