...
thf(aplyco0_thm,conjecture,(! [XA2:($i > $o)] : (! [XN:($i > $o)] : (((cwcel @ XN @ ccn0) & (cwf @ ccn0 @ ccc @ XA2)) => (((ccima @ XA2 @ (ccfv @ (cco @ XN @ cc1 @ ccaddc) @ ccuz)) = (ccsn @ ccc0)) <=> (cwral @ (^ [Xk:$i] : (((ccfv @ (ccv @ Xk) @ XA2) != ccc0) => (cwbr @ (ccv @ Xk) @ XN @ ccle))) @ (^ [Xk:$i] : ccn0))))))).
