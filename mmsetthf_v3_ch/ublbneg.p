...
thf(aublbneg_thm,conjecture,(! [XA2:($i > $o)] : ((cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccle)) @ (^ [Xy1:$i] : XA2))) @ (^ [Xx3:$i] : ccr)) => (cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xx3) @ (ccv @ Xy1) @ ccle)) @ (^ [Xy1:$i] : (ccrab @ (^ [Xz:$i] : (cwcel @ (ccneg @ (ccv @ Xz)) @ XA2)) @ (^ [Xz:$i] : ccr))))) @ (^ [Xx3:$i] : ccr))))).
