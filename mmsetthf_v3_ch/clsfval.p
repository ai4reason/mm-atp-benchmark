...
thf(aclsfval_thm,conjecture,(! [XJ:($i > $o)] : (! [XX:($i > ($i > $o))] : ((! [Xy1:$i] : ((XX @ Xy1) = (ccuni @ XJ))) => (! [Xy1:$i] : ((cwcel @ XJ @ cctop) => ((ccfv @ XJ @ cccl) = (ccmpt @ (^ [Xx3:$i] : (ccpw @ (XX @ Xy1))) @ (^ [Xx3:$i] : (ccint @ (ccrab @ (^ [Xy1:$i] : (cwss @ (ccv @ Xx3) @ (ccv @ Xy1))) @ (^ [Xy1:$i] : (ccfv @ XJ @ cccld))))))))))))).
