thf(cwceq_tp,type,(cwceq : (($i > $o) > (($i > $o) > $o)))).
thf(ccfv_tp,type,(ccfv : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(ccbs_tp,type,(ccbs : ($i > $o))).
thf(cctopn_tp,type,(cctopn : ($i > $o))).
thf(ccsigagen_tp,type,(ccsigagen : ($i > $o))).
thf(cc0g_tp,type,(cc0g : ($i > $o))).
thf(ccvsca_tp,type,(ccvsca : ($i > $o))).
thf(ccsca_tp,type,(ccsca : ($i > $o))).
thf(ccrrh_tp,type,(ccrrh : ($i > $o))).
thf(cwi_tp,type,(cwi : ($o > ($o > $o)))).
thf(cwcel_tp,type,(cwcel : (($i > $o) > (($i > $o) > $o)))).
thf(ccuni_tp,type,(ccuni : (($i > $o) > ($i > $o)))).
thf(ccrn_tp,type,(ccrn : (($i > $o) > ($i > $o)))).
thf(ccmeas_tp,type,(ccmeas : ($i > $o))).
thf(ccdm_tp,type,(ccdm : (($i > $o) > ($i > $o)))).
thf(cco_tp,type,(cco : (($i > $o) > (($i > $o) > (($i > $o) > ($i > $o)))))).
thf(ccsitg_tp,type,(ccsitg : ($i > $o))).
thf(ccbn_tp,type,(ccbn : ($i > $o))).
thf(ccrefld_tp,type,(ccrefld : ($i > $o))).
thf(ccrrext_tp,type,(ccrrext : ($i > $o))).
thf(cwss_tp,type,(cwss : (($i > $o) > (($i > $o) > $o)))).
thf(ccr_tp,type,(ccr : ($i > $o))).
thf(cwf_tp,type,(cwf : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(ccico_tp,type,(ccico : ($i > $o))).
thf(ccpnf_tp,type,(ccpnf : ($i > $o))).
thf(ccc0_tp,type,(ccc0 : ($i > $o))).
thf(ccima_tp,type,(ccima : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(ccv_tp,type,(ccv : ($i > ($i > $o)))).
thf(cw3a_tp,type,(cw3a : ($o > ($o > ($o > $o))))).
thf(cccmn_tp,type,(cccmn : ($i > $o))).
thf(cclmod_tp,type,(cclmod : ($i > $o))).
thf(cctps_tp,type,(cctps : ($i > $o))).
thf(ccmt_tp,type,(ccmt : ($i > $o))).
thf(cccms_tp,type,(cccms : ($i > $o))).
thf(ccxp_tp,type,(ccxp : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(ccds_tp,type,(ccds : ($i > $o))).
thf(ccres_tp,type,(ccres : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(asitgclbn_thm,axiom,(! [Xph:$o] : (! [XB2:($i > $o)] : (! [XS:($i > $o)] : (! [Xc_x:($i > $o)] : (! [XF:($i > $o)] : (! [XH:($i > $o)] : (! [XJ:($i > $o)] : (! [XM:($i > $o)] : (! [XV:($i > $o)] : (! [XW:($i > $o)] : (! [Xc_0:($i > $o)] : ((cwceq @ XB2 @ (ccfv @ XW @ ccbs)) => ((cwceq @ XJ @ (ccfv @ XW @ cctopn)) => ((cwceq @ XS @ (ccfv @ XJ @ ccsigagen)) => ((cwceq @ Xc_0 @ (ccfv @ XW @ cc0g)) => ((cwceq @ Xc_x @ (ccfv @ XW @ ccvsca)) => ((cwceq @ XH @ (ccfv @ (ccfv @ XW @ ccsca) @ ccrrh)) => ((cwi @ Xph @ (cwcel @ XW @ XV)) => ((cwi @ Xph @ (cwcel @ XM @ (ccuni @ (ccrn @ ccmeas)))) => ((cwi @ Xph @ (cwcel @ XF @ (ccdm @ (cco @ XW @ XM @ ccsitg)))) => ((cwi @ Xph @ (cwcel @ XW @ ccbn)) => ((cwi @ Xph @ (cwcel @ (ccfv @ XW @ ccsca) @ ccrrext)) => (cwi @ Xph @ (cwcel @ (ccfv @ XF @ (cco @ XW @ XM @ ccsitg)) @ XB2))))))))))))))))))))))))).
thf(asyl6eqel_thm,axiom,(! [Xph:$o] : (! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : ((cwi @ Xph @ (cwceq @ XA2 @ XB2)) => ((cwcel @ XB2 @ XC) => (cwi @ Xph @ (cwcel @ XA2 @ XC))))))))).
thf(ccofld_tp,type,(ccofld : ($i > $o))).
thf(ccsubrg_tp,type,(ccsubrg : ($i > $o))).
thf(cccnfld_tp,type,(cccnfld : ($i > $o))).
thf(ccmetu_tp,type,(ccmetu : ($i > $o))).
thf(ccuss_tp,type,(ccuss : ($i > $o))).
thf(cccusp_tp,type,(cccusp : ($i > $o))).
thf(ccchr_tp,type,(ccchr : ($i > $o))).
thf(ccnlm_tp,type,(ccnlm : ($i > $o))).
thf(cczlm_tp,type,(cczlm : ($i > $o))).
thf(ccdr_tp,type,(ccdr : ($i > $o))).
thf(ccnrg_tp,type,(ccnrg : ($i > $o))).
thf(cwa_tp,type,(cwa : ($o > ($o > $o)))).
thf(arerrext_thm,axiom,(cwcel @ ccrefld @ ccrrext)).
thf(csitgclre_conj,conjecture,(! [Xph:$o] : (! [XB2:($i > $o)] : (! [XS:($i > $o)] : (! [Xc_x:($i > $o)] : (! [XF:($i > $o)] : (! [XH:($i > $o)] : (! [XJ:($i > $o)] : (! [XM:($i > $o)] : (! [XV:($i > $o)] : (! [XW:($i > $o)] : (! [Xc_0:($i > $o)] : ((cwceq @ XB2 @ (ccfv @ XW @ ccbs)) => ((cwceq @ XJ @ (ccfv @ XW @ cctopn)) => ((cwceq @ XS @ (ccfv @ XJ @ ccsigagen)) => ((cwceq @ Xc_0 @ (ccfv @ XW @ cc0g)) => ((cwceq @ Xc_x @ (ccfv @ XW @ ccvsca)) => ((cwceq @ XH @ (ccfv @ (ccfv @ XW @ ccsca) @ ccrrh)) => ((cwi @ Xph @ (cwcel @ XW @ XV)) => ((cwi @ Xph @ (cwcel @ XM @ (ccuni @ (ccrn @ ccmeas)))) => ((cwi @ Xph @ (cwcel @ XF @ (ccdm @ (cco @ XW @ XM @ ccsitg)))) => ((cwi @ Xph @ (cwcel @ XW @ ccbn)) => ((cwi @ Xph @ (cwceq @ (ccfv @ XW @ ccsca) @ ccrefld)) => (cwi @ Xph @ (cwcel @ (ccfv @ XF @ (cco @ XW @ XM @ ccsitg)) @ XB2))))))))))))))))))))))))).
