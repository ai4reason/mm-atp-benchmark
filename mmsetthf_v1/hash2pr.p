...
thf(ahashvnfin_thm,axiom,(! [XS:($i > $o)] : (! [XN:($i > $o)] : (! [XV:($i > $o)] : (cwi @ (cwa @ (cwcel @ XS @ XV) @ (cwcel @ XN @ ccn0)) @ (cwi @ (cwceq @ (ccfv @ XS @ cchash) @ XN) @ (cwcel @ XS @ ccfn))))))).
...
thf(chash2pr_conj,conjecture,(! [XV:($i > $o)] : (! [XW:($i > ($i > ($i > $o)))] : (! [Xa:$i] : (! [Xb:$i] : (cwi @ (cwa @ (cwcel @ XV @ (XW @ Xa @ Xb)) @ (cwceq @ (ccfv @ XV @ cchash) @ cc2)) @ (cwex @ (^ [Xa:$i] : (cwex @ (^ [Xb:$i] : (cwceq @ XV @ (ccpr @ (ccv @ Xa) @ (ccv @ Xb))))))))))))).
