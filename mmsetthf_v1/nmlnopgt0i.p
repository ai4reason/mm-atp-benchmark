thf(cwcel_tp,type,(cwcel : (($i > $o) > (($i > $o) > $o)))).
thf(cclo_tp,type,(cclo : ($i > $o))).
thf(cwb_tp,type,(cwb : ($o > ($o > $o)))).
thf(cwne_tp,type,(cwne : (($i > $o) > (($i > $o) > $o)))).
thf(cch0o_tp,type,(cch0o : ($i > $o))).
thf(cwbr_tp,type,(cwbr : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(ccc0_tp,type,(ccc0 : ($i > $o))).
thf(ccfv_tp,type,(ccfv : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(ccnop_tp,type,(ccnop : ($i > $o))).
thf(cclt_tp,type,(cclt : ($i > $o))).
thf(cwf_tp,type,(cwf : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(cchil_tp,type,(cchil : ($i > $o))).
thf(abitr3i_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((cwb @ Xps @ Xph) => ((cwb @ Xps @ Xch) => (cwb @ Xph @ Xch))))))).
thf(cwn_tp,type,(cwn : ($o > $o))).
thf(cwceq_tp,type,(cwceq : (($i > $o) > (($i > $o) > $o)))).
thf(anecon3bii_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : (! [XD:($i > $o)] : ((cwb @ (cwceq @ XA2 @ XB2) @ (cwceq @ XC @ XD)) => (cwb @ (cwne @ XA2 @ XB2) @ (cwne @ XC @ XD)))))))).
thf(cclno_tp,type,(cclno : ($i > $o))).
thf(cc0o_tp,type,(cc0o : ($i > $o))).
thf(ccnmoo_tp,type,(ccnmoo : ($i > $o))).
thf(cco_tp,type,(cco : (($i > $o) > (($i > $o) > (($i > $o) > ($i > $o)))))).
thf(ccno_tp,type,(ccno : ($i > $o))).
thf(ccsm_tp,type,(ccsm : ($i > $o))).
thf(ccva_tp,type,(ccva : ($i > $o))).
thf(ccop_tp,type,(ccop : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(anmlnop0iHIL_thm,axiom,(! [XT:($i > $o)] : ((cwcel @ XT @ cclo) => (cwb @ (cwceq @ (ccfv @ XT @ ccnop) @ ccc0) @ (cwceq @ XT @ cch0o))))).
thf(cwi_tp,type,(cwi : ($o > ($o > $o)))).
thf(aax_mp_ax,axiom,(! [Xph:$o] : (! [Xps:$o] : (Xph => ((cwi @ Xph @ Xps) => Xps))))).
thf(alnopfi_thm,axiom,(! [XT:($i > $o)] : ((cwcel @ XT @ cclo) => (cwf @ cchil @ cchil @ XT)))).
thf(ccle_tp,type,(ccle : ($i > $o))).
thf(ccxr_tp,type,(ccxr : ($i > $o))).
thf(anmopgt0_thm,axiom,(! [XT:($i > $o)] : (cwi @ (cwf @ cchil @ cchil @ XT) @ (cwb @ (cwne @ (ccfv @ XT @ ccnop) @ ccc0) @ (cwbr @ ccc0 @ (ccfv @ XT @ ccnop) @ cclt))))).
thf(cnmlnopgt0i_conj,conjecture,(! [XT:($i > $o)] : ((cwcel @ XT @ cclo) => (cwb @ (cwne @ XT @ cch0o) @ (cwbr @ ccc0 @ (ccfv @ XT @ ccnop) @ cclt))))).
