...
thf(adf_tx_ax,axiom,(cwceq @ cctx @ (ccmpt2 @ (^ [Xr:$i] : (^ [Xs1:$i] : ccvv)) @ (^ [Xr:$i] : (^ [Xs1:$i] : ccvv)) @ (^ [Xr:$i] : (^ [Xs1:$i] : (ccfv @ (ccrn @ (ccmpt2 @ (^ [Xx3:$i] : (^ [Xy1:$i] : (ccv @ Xr))) @ (^ [Xx3:$i] : (^ [Xy1:$i] : (ccv @ Xs1))) @ (^ [Xx3:$i] : (^ [Xy1:$i] : (ccxp @ (ccv @ Xx3) @ (ccv @ Xy1)))))) @ cctg)))))).
thf(cwbr_tp,type,(cwbr : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(ccio_tp,type,(ccio : (($i > $o) > ($i > $o)))).
thf(afvex_thm,axiom,(! [XA2:($i > $o)] : (! [XF:($i > $o)] : (cwcel @ (ccfv @ XA2 @ XF) @ ccvv)))).
thf(ctxval_conj,conjecture,(! [XB2:($i > ($i > ($i > $o)))] : (! [XR:($i > $o)] : (! [XS:($i > $o)] : (! [XV:($i > ($i > ($i > $o)))] : (! [XW:($i > ($i > ($i > $o)))] : ((! [Xx3:$i] : (! [Xy1:$i] : (cwceq @ (XB2 @ Xx3 @ Xy1) @ (ccrn @ (ccmpt2 @ (^ [Xx3:$i] : (^ [Xy1:$i] : XR)) @ (^ [Xx3:$i] : (^ [Xy1:$i] : XS)) @ (^ [Xx3:$i] : (^ [Xy1:$i] : (ccxp @ (ccv @ Xx3) @ (ccv @ Xy1))))))))) => (! [Xx3:$i] : (! [Xy1:$i] : (cwi @ (cwa @ (cwcel @ XR @ (XV @ Xx3 @ Xy1)) @ (cwcel @ XS @ (XW @ Xx3 @ Xy1))) @ (cwceq @ (cco @ XR @ XS @ cctx) @ (ccfv @ (XB2 @ Xx3 @ Xy1) @ cctg)))))))))))).
