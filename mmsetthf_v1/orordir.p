thf(cwb_tp,type,(cwb : ($o > ($o > $o)))).
thf(cwo_tp,type,(cwo : ($o > ($o > $o)))).
thf(abitr3i_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((cwb @ Xps @ Xph) => ((cwb @ Xps @ Xch) => (cwb @ Xph @ Xch))))))).
thf(aorbi2i_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((cwb @ Xph @ Xps) => (cwb @ (cwo @ Xch @ Xph) @ (cwo @ Xch @ Xps))))))).
thf(aoridm_thm,axiom,(! [Xph:$o] : (cwb @ (cwo @ Xph @ Xph) @ Xph))).
thf(aor4_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (! [Xth:$o] : (cwb @ (cwo @ (cwo @ Xph @ Xps) @ (cwo @ Xch @ Xth)) @ (cwo @ (cwo @ Xph @ Xch) @ (cwo @ Xps @ Xth)))))))).
thf(corordir_conj,conjecture,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (cwb @ (cwo @ (cwo @ Xph @ Xps) @ Xch) @ (cwo @ (cwo @ Xph @ Xch) @ (cwo @ Xps @ Xch))))))).
