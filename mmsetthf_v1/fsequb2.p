...
thf(cfsequb2_conj,conjecture,(! [XF:($i > $o)] : (! [XM:($i > $o)] : (! [XN:($i > $o)] : (cwi @ (cwf @ (cco @ XM @ XN @ ccfz) @ ccr @ XF) @ (cwrex @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccle)) @ (^ [Xy1:$i] : (ccrn @ XF)))) @ (^ [Xx3:$i] : ccr))))))).
