...
thf(aiscld2_thm,axiom,(! [XS:($i > $o)] : (! [XJ:($i > $o)] : (! [XX:($i > $o)] : ((cwceq @ XX @ (ccuni @ XJ)) => (cwi @ (cwa @ (cwcel @ XJ @ cctop) @ (cwss @ XS @ XX)) @ (cwb @ (cwcel @ XS @ (ccfv @ XJ @ cccld)) @ (cwcel @ (ccdif @ XX @ XS) @ XJ)))))))).
thf(auniretop_thm,axiom,(cwceq @ ccr @ (ccuni @ (ccfv @ (ccrn @ ccioo) @ cctg)))).
thf(ciocmnfcld_conj,conjecture,(! [XA2:($i > $o)] : (cwi @ (cwcel @ XA2 @ ccr) @ (cwcel @ (cco @ ccmnf @ XA2 @ ccioc) @ (ccfv @ (ccfv @ (ccrn @ ccioo) @ cctg) @ cccld))))).
