...
thf(adf_oadd_ax,axiom,(cwceq @ ccoa @ (ccmpt2 @ (^ [Xx3:$i] : (^ [Xy1:$i] : ccon0)) @ (^ [Xx3:$i] : (^ [Xy1:$i] : ccon0)) @ (^ [Xx3:$i] : (^ [Xy1:$i] : (ccfv @ (ccv @ Xy1) @ (ccrdg @ (ccmpt @ (^ [Xz:$i] : ccvv) @ (^ [Xz:$i] : (ccsuc @ (ccv @ Xz)))) @ (ccv @ Xx3)))))))).
thf(cwbr_tp,type,(cwbr : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(ccio_tp,type,(ccio : (($i > $o) > ($i > $o)))).
thf(afvex_thm,axiom,(! [XA2:($i > $o)] : (! [XF:($i > $o)] : (cwcel @ (ccfv @ XA2 @ XF) @ ccvv)))).
thf(cfnoa_conj,conjecture,(cwfn @ ccoa @ (ccxp @ ccon0 @ ccon0))).
