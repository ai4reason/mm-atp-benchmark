...
thf(adf_addr_ax,axiom,(cwceq @ ccplusr @ (ccmpt2 @ (^ [Xx3:$i] : (^ [Xy1:$i] : ccvv)) @ (^ [Xx3:$i] : (^ [Xy1:$i] : ccvv)) @ (^ [Xx3:$i] : (^ [Xy1:$i] : (ccmpt @ (^ [Xv:$i] : ccr) @ (^ [Xv:$i] : (cco @ (ccfv @ (ccv @ Xv) @ (ccv @ Xx3)) @ (ccfv @ (ccv @ Xv) @ (ccv @ Xy1)) @ ccaddc)))))))).
thf(amptex_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : ((cwcel @ XA2 @ ccvv) => (cwcel @ (ccmpt @ (^ [Xx3:$i] : XA2) @ (^ [Xx3:$i] : (XB2 @ Xx3))) @ ccvv))))).
thf(ccc_tp,type,(ccc : ($i > $o))).
thf(areex_thm,axiom,(cwcel @ ccr @ ccvv)).
thf(caddrval_conj,conjecture,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > ($i > $o))] : (! [XD:($i > ($i > $o))] : (! [Xv:$i] : (cwi @ (cwa @ (cwcel @ XA2 @ (XC @ Xv)) @ (cwcel @ XB2 @ (XD @ Xv))) @ (cwceq @ (cco @ XA2 @ XB2 @ ccplusr) @ (ccmpt @ (^ [Xv:$i] : ccr) @ (^ [Xv:$i] : (cco @ (ccfv @ (ccv @ Xv) @ XA2) @ (ccfv @ (ccv @ Xv) @ XB2) @ ccaddc))))))))))).
