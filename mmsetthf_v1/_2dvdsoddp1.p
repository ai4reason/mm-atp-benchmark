thf(cwi_tp,type,(cwi : ($o > ($o > $o)))).
thf(cwcel_tp,type,(cwcel : (($i > $o) > (($i > $o) > $o)))).
thf(ccodd_tp,type,(ccodd : ($i > $o))).
thf(cwbr_tp,type,(cwbr : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(cc2_tp,type,(cc2 : ($i > $o))).
thf(cco_tp,type,(cco : (($i > $o) > (($i > $o) > (($i > $o) > ($i > $o)))))).
thf(cc1_tp,type,(cc1 : ($i > $o))).
thf(ccaddc_tp,type,(ccaddc : ($i > $o))).
thf(ccdvds_tp,type,(ccdvds : ($i > $o))).
thf(cwn_tp,type,(cwn : ($o > $o))).
thf(ccz_tp,type,(ccz : ($i > $o))).
thf(cwb_tp,type,(cwb : ($o > ($o > $o)))).
thf(ampbid_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((cwi @ Xph @ Xps) => ((cwi @ Xph @ (cwb @ Xps @ Xch)) => (cwi @ Xph @ Xch))))))).
thf(a_2ndvdsodd_thm,axiom,(! [XZ:($i > $o)] : (cwi @ (cwcel @ XZ @ ccodd) @ (cwn @ (cwbr @ cc2 @ XZ @ ccdvds))))).
thf(asyl_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((cwi @ Xph @ Xps) => ((cwi @ Xps @ Xch) => (cwi @ Xph @ Xch))))))).
thf(ccdiv_tp,type,(ccdiv : ($i > $o))).
thf(aoddz_thm,axiom,(! [XZ:($i > $o)] : (cwi @ (cwcel @ XZ @ ccodd) @ (cwcel @ XZ @ ccz)))).
thf(ccmin_tp,type,(ccmin : ($i > $o))).
thf(aoddp1even_thm,axiom,(! [XN:($i > $o)] : (cwi @ (cwcel @ XN @ ccz) @ (cwb @ (cwn @ (cwbr @ cc2 @ XN @ ccdvds)) @ (cwbr @ cc2 @ (cco @ XN @ cc1 @ ccaddc) @ ccdvds))))).
thf(c_2dvdsoddp1_conj,conjecture,(! [XZ:($i > $o)] : (cwi @ (cwcel @ XZ @ ccodd) @ (cwbr @ cc2 @ (cco @ XZ @ cc1 @ ccaddc) @ ccdvds)))).
