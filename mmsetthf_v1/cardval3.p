...
thf(adf_card_ax,axiom,(cwceq @ cccrd @ (ccmpt @ (^ [Xx3:$i] : ccvv) @ (^ [Xx3:$i] : (ccint @ (ccrab @ (^ [Xy1:$i] : (cwbr @ (ccv @ Xy1) @ (ccv @ Xx3) @ ccen)) @ (^ [Xy1:$i] : ccon0))))))).
thf(ccardval3_conj,conjecture,(! [XA2:($i > $o)] : (cwi @ (cwcel @ XA2 @ (ccdm @ cccrd)) @ (cwceq @ (ccfv @ XA2 @ cccrd) @ (ccint @ (ccrab @ (^ [Xx3:$i] : (cwbr @ (ccv @ Xx3) @ XA2 @ ccen)) @ (^ [Xx3:$i] : ccon0))))))).
