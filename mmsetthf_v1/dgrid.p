...
thf(adgr1term_thm,axiom,(! [XA2:($i > $o)] : (! [XF:($i > ($i > $o))] : (! [XN:($i > $o)] : ((! [Xz:$i] : (cwceq @ (XF @ Xz) @ (ccmpt @ (^ [Xz:$i] : ccc) @ (^ [Xz:$i] : (cco @ XA2 @ (cco @ (ccv @ Xz) @ XN @ ccexp) @ ccmul))))) => (! [Xz:$i] : (cwi @ (cw3a @ (cwcel @ XA2 @ ccc) @ (cwne @ XA2 @ ccc0) @ (cwcel @ XN @ ccn0)) @ (cwceq @ (ccfv @ (XF @ Xz) @ ccdgr) @ XN)))))))).
...
thf(cdgrid_conj,conjecture,(cwceq @ (ccfv @ ccidp @ ccdgr) @ cc1)).
