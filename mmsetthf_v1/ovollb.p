...
thf(covollb_conj,conjecture,(! [XA2:($i > $o)] : (! [XS:($i > $o)] : (! [XF:($i > $o)] : ((cwceq @ XS @ (ccseq @ ccaddc @ (cccom @ (cccom @ ccabs @ ccmin) @ XF) @ cc1)) => (cwi @ (cwa @ (cwf @ ccn @ (ccin @ ccle @ (ccxp @ ccr @ ccr)) @ XF) @ (cwss @ XA2 @ (ccuni @ (ccrn @ (cccom @ ccioo @ XF))))) @ (cwbr @ (ccfv @ XA2 @ ccovol) @ (ccsup @ (ccrn @ XS) @ ccxr @ cclt) @ ccle))))))).
