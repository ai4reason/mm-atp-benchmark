thf(cwi_tp,type,(cwi : ($o > ($o > $o)))).
thf(cwcel_tp,type,(cwcel : (($i > $o) > (($i > $o) > $o)))).
thf(ccnrg_tp,type,(ccnrg : ($i > $o))).
thf(cctrg_tp,type,(cctrg : ($i > $o))).
thf(cctgp_tp,type,(cctgp : ($i > $o))).
thf(ccrg_tp,type,(ccrg : ($i > $o))).
thf(ccfv_tp,type,(ccfv : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(ccmgp_tp,type,(ccmgp : ($i > $o))).
thf(cctmd_tp,type,(cctmd : ($i > $o))).
thf(ccmnd_tp,type,(ccmnd : ($i > $o))).
thf(cctps_tp,type,(cctps : ($i > $o))).
thf(ccplusf_tp,type,(ccplusf : ($i > $o))).
thf(cco_tp,type,(cco : (($i > $o) > (($i > $o) > (($i > $o) > ($i > $o)))))).
thf(cctopn_tp,type,(cctopn : ($i > $o))).
thf(cctx_tp,type,(cctx : ($i > $o))).
thf(cccn_tp,type,(cccn : ($i > $o))).
thf(ccbs_tp,type,(ccbs : ($i > $o))).
thf(cctopon_tp,type,(cctopon : ($i > $o))).
thf(ccrglmod_tp,type,(ccrglmod : ($i > $o))).
thf(ccnlm_tp,type,(ccnlm : ($i > $o))).
thf(ccid_tp,type,(ccid : ($i > $o))).
thf(cwceq_tp,type,(cwceq : (($i > $o) > (($i > $o) > $o)))).
thf(cwtru_tp,type,(cwtru : $o)).
thf(cc1_tp,type,(cc1 : ($i > $o))).
thf(ccts_tp,type,(ccts : ($i > $o))).
thf(cc9_tp,type,(cc9 : ($i > $o))).
thf(cw3a_tp,type,(cw3a : ($o > ($o > ($o > $o))))).
thf(cwb_tp,type,(cwb : ($o > ($o > $o)))).
thf(asyl3anbrc_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : (! [Xth:$o] : (! [Xta:$o] : ((cwi @ Xph @ Xps) => ((cwi @ Xph @ Xch) => ((cwi @ Xph @ Xth) => ((cwb @ Xta @ (cw3a @ Xps @ Xch @ Xth)) => (cwi @ Xph @ Xta))))))))))).
thf(ccabl_tp,type,(ccabl : ($i > $o))).
thf(ccngp_tp,type,(ccngp : ($i > $o))).
thf(anrgtgp_thm,axiom,(! [XR:($i > $o)] : (cwi @ (cwcel @ XR @ ccnrg) @ (cwcel @ XR @ cctgp)))).
thf(ccabv_tp,type,(ccabv : ($i > $o))).
thf(ccnm_tp,type,(ccnm : ($i > $o))).
thf(anrgring_thm,axiom,(! [XR:($i > $o)] : (cwi @ (cwcel @ XR @ ccnrg) @ (cwcel @ XR @ ccrg)))).
thf(asyl_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((cwi @ Xph @ Xps) => ((cwi @ Xps @ Xch) => (cwi @ Xph @ Xch))))))).
thf(ccmulr_tp,type,(ccmulr : ($i > $o))).
thf(ccplusg_tp,type,(ccplusg : ($i > $o))).
thf(ccv_tp,type,(ccv : ($i > ($i > $o)))).
thf(cwa_tp,type,(cwa : ($o > ($o > $o)))).
thf(cwral_tp,type,(cwral : (($i > $o) > (($i > ($i > $o)) > $o)))).
thf(ccgrp_tp,type,(ccgrp : ($i > $o))).
thf(aringmgp_thm,axiom,(! [XR:($i > $o)] : (! [XG:($i > $o)] : ((cwceq @ XG @ (ccfv @ XR @ ccmgp)) => (cwi @ (cwcel @ XR @ ccrg) @ (cwcel @ XG @ ccmnd)))))).
thf(aeqid_thm,axiom,(! [XA2:($i > $o)] : (cwceq @ XA2 @ XA2))).
thf(asylibr_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((cwi @ Xph @ Xps) => ((cwb @ Xch @ Xps) => (cwi @ Xph @ Xch))))))).
thf(asylib_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (! [Xch:$o] : ((cwi @ Xph @ Xps) => ((cwb @ Xps @ Xch) => (cwi @ Xph @ Xch))))))).
thf(atgptps_thm,axiom,(! [XG:($i > $o)] : (cwi @ (cwcel @ XG @ cctgp) @ (cwcel @ XG @ cctps)))).
thf(cc0_tp,type,(cc0 : ($i > $o))).
thf(cwn_tp,type,(cwn : ($o > $o))).
thf(ccvv_tp,type,(ccvv : ($i > $o))).
thf(cctop_tp,type,(cctop : ($i > $o))).
thf(ccab_tp,type,(ccab : (($i > $o) > ($i > $o)))).
thf(aistps_thm,axiom,(! [XA2:($i > $o)] : (! [XJ:($i > $o)] : (! [XK:($i > $o)] : ((cwceq @ XA2 @ (ccfv @ XK @ ccbs)) => ((cwceq @ XJ @ (ccfv @ XK @ cctopn)) => (cwb @ (cwcel @ XK @ cctps) @ (cwcel @ XJ @ (ccfv @ XA2 @ cctopon))))))))).
thf(amgpbas_thm,axiom,(! [XB2:($i > $o)] : (! [XR:($i > $o)] : (! [XM:($i > $o)] : ((cwceq @ XM @ (ccfv @ XR @ ccmgp)) => ((cwceq @ XB2 @ (ccfv @ XR @ ccbs)) => (cwceq @ XB2 @ (ccfv @ XM @ ccbs)))))))).
thf(ccrest_tp,type,(ccrest : ($i > $o))).
thf(amgptopn_thm,axiom,(! [XR:($i > $o)] : (! [XJ:($i > $o)] : (! [XM:($i > $o)] : ((cwceq @ XM @ (ccfv @ XR @ ccmgp)) => ((cwceq @ XJ @ (ccfv @ XR @ cctopn)) => (cwceq @ XJ @ (ccfv @ XM @ cctopn)))))))).
thf(ccsubrg_tp,type,(ccsubrg : ($i > $o))).
thf(arlmnlm_thm,axiom,(! [XR:($i > $o)] : (cwi @ (cwcel @ XR @ ccnrg) @ (cwcel @ (ccfv @ XR @ ccrglmod) @ ccnlm)))).
thf(ccxme_tp,type,(ccxme : ($i > $o))).
thf(ccmt_tp,type,(ccmt : ($i > $o))).
thf(ccxmt_tp,type,(ccxmt : ($i > $o))).
thf(ccaddc_tp,type,(ccaddc : ($i > $o))).
thf(ccdiv_tp,type,(ccdiv : ($i > $o))).
thf(cc2_tp,type,(cc2 : ($i > $o))).
thf(ccvsca_tp,type,(ccvsca : ($i > $o))).
thf(cclmod_tp,type,(cclmod : ($i > $o))).
thf(ccrp_tp,type,(ccrp : ($i > $o))).
thf(cclt_tp,type,(cclt : ($i > $o))).
thf(cwbr_tp,type,(cwbr : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(cwrex_tp,type,(cwrex : (($i > $o) > (($i > ($i > $o)) > $o)))).
thf(cwf_tp,type,(cwf : (($i > $o) > (($i > $o) > (($i > $o) > $o))))).
thf(ccmopn_tp,type,(ccmopn : ($i > $o))).
thf(ccxp_tp,type,(ccxp : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(ccds_tp,type,(ccds : ($i > $o))).
thf(ccres_tp,type,(ccres : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(ccscaf_tp,type,(ccscaf : ($i > $o))).
thf(ccsca_tp,type,(ccsca : ($i > $o))).
thf(anlmvscn_thm,axiom,(! [Xc_x:($i > $o)] : (! [XF:($i > $o)] : (! [XJ:($i > $o)] : (! [XK:($i > $o)] : (! [XW:($i > $o)] : ((cwceq @ XF @ (ccfv @ XW @ ccsca)) => ((cwceq @ Xc_x @ (ccfv @ XW @ ccscaf)) => ((cwceq @ XJ @ (ccfv @ XW @ cctopn)) => ((cwceq @ XK @ (ccfv @ XF @ cctopn)) => (cwi @ (cwcel @ XW @ ccnlm) @ (cwcel @ Xc_x @ (cco @ (cco @ XK @ XJ @ cctx) @ XJ @ cccn))))))))))))).
thf(cwss_tp,type,(cwss : (($i > $o) > (($i > $o) > $o)))).
thf(ccsra_tp,type,(ccsra : ($i > $o))).
thf(ccress_tp,type,(ccress : ($i > $o))).
thf(arlmsca2_thm,axiom,(! [XR:($i > $o)] : (cwceq @ (ccfv @ XR @ ccid) @ (ccfv @ (ccfv @ XR @ ccrglmod) @ ccsca)))).
thf(ccmpt2_tp,type,(ccmpt2 : (($i > ($i > ($i > $o))) > (($i > ($i > ($i > $o))) > (($i > ($i > ($i > $o))) > ($i > $o)))))).
thf(arlmscaf_thm,axiom,(! [XR:($i > $o)] : (cwceq @ (ccfv @ (ccfv @ XR @ ccmgp) @ ccplusf) @ (ccfv @ (ccfv @ XR @ ccrglmod) @ ccscaf)))).
thf(arlmtopn_thm,axiom,(! [XR:($i > $o)] : (cwceq @ (ccfv @ XR @ cctopn) @ (ccfv @ (ccfv @ XR @ ccrglmod) @ cctopn)))).
thf(atrud_thm,axiom,(! [Xph:$o] : ((cwi @ cwtru @ Xph) => Xph))).
thf(atopnpropd_thm,axiom,(! [Xph:$o] : (! [XK:($i > $o)] : (! [XL:($i > $o)] : ((cwi @ Xph @ (cwceq @ (ccfv @ XK @ ccbs) @ (ccfv @ XL @ ccbs))) => ((cwi @ Xph @ (cwceq @ (ccfv @ XK @ ccts) @ (ccfv @ XL @ ccts))) => (cwi @ Xph @ (cwceq @ (ccfv @ XK @ cctopn) @ (ccfv @ XL @ cctopn))))))))).
thf(aa1i_thm,axiom,(! [Xph:$o] : (! [Xps:$o] : (Xph => (cwi @ Xps @ Xph))))).
thf(ccslot_tp,type,(ccslot : (($i > $o) > ($i > $o)))).
thf(astrfvi_thm,axiom,(! [XS:($i > $o)] : (! [XE:($i > $o)] : (! [XN:($i > $o)] : (! [XX:($i > $o)] : ((cwceq @ XE @ (ccslot @ XN)) => ((cwceq @ XX @ (ccfv @ XS @ XE)) => (cwceq @ XX @ (ccfv @ (ccfv @ XS @ ccid) @ XE))))))))).
thf(adf_base_ax,axiom,(cwceq @ ccbs @ (ccslot @ cc1))).
thf(adf_tset_ax,axiom,(cwceq @ ccts @ (ccslot @ cc9))).
thf(cwsbc_tp,type,(cwsbc : (($i > $o) > (($i > $o) > $o)))).
thf(ccin_tp,type,(ccin : (($i > $o) > (($i > $o) > ($i > $o))))).
thf(aistmd_thm,axiom,(! [XF:($i > $o)] : (! [XG:($i > $o)] : (! [XJ:($i > $o)] : ((cwceq @ XF @ (ccfv @ XG @ ccplusf)) => ((cwceq @ XJ @ (ccfv @ XG @ cctopn)) => (cwb @ (cwcel @ XG @ cctmd) @ (cw3a @ (cwcel @ XG @ ccmnd) @ (cwcel @ XG @ cctps) @ (cwcel @ XF @ (cco @ (cco @ XJ @ XJ @ cctx) @ XJ @ cccn)))))))))).
thf(aistrg_thm,axiom,(! [XR:($i > $o)] : (! [XM:($i > $o)] : ((cwceq @ XM @ (ccfv @ XR @ ccmgp)) => (cwb @ (cwcel @ XR @ cctrg) @ (cw3a @ (cwcel @ XR @ cctgp) @ (cwcel @ XR @ ccrg) @ (cwcel @ XM @ cctmd))))))).
thf(cnrgtrg_conj,conjecture,(! [XR:($i > $o)] : (cwi @ (cwcel @ XR @ ccnrg) @ (cwcel @ XR @ cctrg)))).
