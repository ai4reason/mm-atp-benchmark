...
thf(aelz2_thm,axiom,(! [XN:($i > $o)] : (cwb @ (cwcel @ XN @ ccz) @ (cwrex @ (^ [Xx3:$i] : (cwrex @ (^ [Xy1:$i] : (cwceq @ XN @ (cco @ (ccv @ Xx3) @ (ccv @ Xy1) @ ccmin))) @ (^ [Xy1:$i] : ccn))) @ (^ [Xx3:$i] : ccn))))).
...
thf(czaddcl_conj,conjecture,(! [XM:($i > $o)] : (! [XN:($i > $o)] : (cwi @ (cwa @ (cwcel @ XM @ ccz) @ (cwcel @ XN @ ccz)) @ (cwcel @ (cco @ XM @ XN @ ccaddc) @ ccz))))).
