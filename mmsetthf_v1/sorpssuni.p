...
thf(asorpssi_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : (cwi @ (cwa @ (cwor @ XA2 @ ccrpss) @ (cwa @ (cwcel @ XB2 @ XA2) @ (cwcel @ XC @ XA2))) @ (cwo @ (cwss @ XB2 @ XC) @ (cwss @ XC @ XB2))))))).
...
thf(csorpssuni_conj,conjecture,(! [XY:($i > $o)] : (cwi @ (cwor @ XY @ ccrpss) @ (cwb @ (cwrex @ (^ [Xu:$i] : (cwral @ (^ [Xv:$i] : (cwn @ (cwpss @ (ccv @ Xu) @ (ccv @ Xv)))) @ (^ [Xv:$i] : XY))) @ (^ [Xu:$i] : XY)) @ (cwcel @ (ccuni @ XY) @ XY))))).
