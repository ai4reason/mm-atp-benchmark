...
thf(adf_reps_ax,axiom,(cwceq @ ccreps @ (ccmpt2 @ (^ [Xs1:$i] : (^ [Xn:$i] : ccvv)) @ (^ [Xs1:$i] : (^ [Xn:$i] : ccn0)) @ (^ [Xs1:$i] : (^ [Xn:$i] : (ccmpt @ (^ [Xx3:$i] : (cco @ ccc0 @ (ccv @ Xn) @ ccfzo)) @ (^ [Xx3:$i] : (ccv @ Xs1)))))))).
thf(creps_conj,conjecture,(! [XS:($i > $o)] : (! [XN:($i > $o)] : (! [XV:($i > ($i > $o))] : (! [Xx3:$i] : (cwi @ (cwa @ (cwcel @ XS @ (XV @ Xx3)) @ (cwcel @ XN @ ccn0)) @ (cwceq @ (cco @ XS @ XN @ ccreps) @ (ccmpt @ (^ [Xx3:$i] : (cco @ ccc0 @ XN @ ccfzo)) @ (^ [Xx3:$i] : XS))))))))).
