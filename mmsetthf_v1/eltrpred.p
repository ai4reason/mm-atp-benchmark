...
thf(celtrpred_conj,conjecture,(! [XA2:($i > $o)] : (! [XR:($i > $o)] : (! [XX:($i > $o)] : (! [XY:($i > $o)] : (cwb @ (cwcel @ XY @ (cctrpred @ XA2 @ XR @ XX)) @ (cwrex @ (^ [Xi:$i] : (cwcel @ XY @ (ccfv @ (ccv @ Xi) @ (ccres @ (ccrdg @ (ccmpt @ (^ [Xa:$i] : ccvv) @ (^ [Xa:$i] : (cciun @ (^ [Xy1:$i] : (ccv @ Xa)) @ (^ [Xy1:$i] : (ccpred @ XA2 @ XR @ (ccv @ Xy1)))))) @ (ccpred @ XA2 @ XR @ XX)) @ ccom)))) @ (^ [Xi:$i] : ccom)))))))).
