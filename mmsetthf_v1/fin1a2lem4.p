...
thf(aomcan_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > $o)] : (! [XC:($i > $o)] : (cwi @ (cwa @ (cw3a @ (cwcel @ XA2 @ ccon0) @ (cwcel @ XB2 @ ccon0) @ (cwcel @ XC @ ccon0)) @ (cwcel @ cc0 @ XA2)) @ (cwb @ (cwceq @ (cco @ XA2 @ XB2 @ ccomu) @ (cco @ XA2 @ XC @ ccomu)) @ (cwceq @ XB2 @ XC))))))).
...
thf(cfin1a2lem4_conj,conjecture,(! [XE:($i > ($i > $o))] : ((! [Xx3:$i] : (cwceq @ (XE @ Xx3) @ (ccmpt @ (^ [Xx3:$i] : ccom) @ (^ [Xx3:$i] : (cco @ cc2o @ (ccv @ Xx3) @ ccomu))))) => (! [Xx3:$i] : (cwf1 @ ccom @ ccom @ (XE @ Xx3)))))).
