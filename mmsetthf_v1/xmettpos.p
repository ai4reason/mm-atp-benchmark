...
thf(atpossym_thm,axiom,(! [XA2:($i > $o)] : (! [XF:($i > $o)] : (cwi @ (cwfn @ XF @ (ccxp @ XA2 @ XA2)) @ (cwb @ (cwceq @ (cctpos @ XF) @ XF) @ (cwral @ (^ [Xx3:$i] : (cwral @ (^ [Xy1:$i] : (cwceq @ (cco @ (ccv @ Xx3) @ (ccv @ Xy1) @ XF) @ (cco @ (ccv @ Xy1) @ (ccv @ Xx3) @ XF))) @ (^ [Xy1:$i] : XA2))) @ (^ [Xx3:$i] : XA2))))))).
thf(cxmettpos_conj,conjecture,(! [XD:($i > $o)] : (! [XX:($i > $o)] : (cwi @ (cwcel @ XD @ (ccfv @ XX @ ccxmt)) @ (cwceq @ (cctpos @ XD) @ XD))))).
