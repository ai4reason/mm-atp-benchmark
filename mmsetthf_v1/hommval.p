...
thf(adf_homul_ax,axiom,(cwceq @ cchot @ (ccmpt2 @ (^ [Xf1:$i] : (^ [Xg1:$i] : ccc)) @ (^ [Xf1:$i] : (^ [Xg1:$i] : (cco @ cchil @ cchil @ ccmap))) @ (^ [Xf1:$i] : (^ [Xg1:$i] : (ccmpt @ (^ [Xx3:$i] : cchil) @ (^ [Xx3:$i] : (cco @ (ccv @ Xf1) @ (ccfv @ (ccv @ Xx3) @ (ccv @ Xg1)) @ ccsm)))))))).
thf(amptex_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : ((cwcel @ XA2 @ ccvv) => (cwcel @ (ccmpt @ (^ [Xx3:$i] : XA2) @ (^ [Xx3:$i] : (XB2 @ Xx3))) @ ccvv))))).
thf(chommval_conj,conjecture,(! [XA2:($i > $o)] : (! [XT:($i > $o)] : (cwi @ (cwa @ (cwcel @ XA2 @ ccc) @ (cwf @ cchil @ cchil @ XT)) @ (cwceq @ (cco @ XA2 @ XT @ cchot) @ (ccmpt @ (^ [Xx3:$i] : cchil) @ (^ [Xx3:$i] : (cco @ XA2 @ (ccfv @ (ccv @ Xx3) @ XT) @ ccsm)))))))).
