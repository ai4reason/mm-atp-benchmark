...
thf(aabrexct_thm,axiom,(! [XA2:($i > $o)] : (! [XB2:($i > ($i > $o))] : (cwi @ (cwbr @ XA2 @ ccom @ ccdom) @ (cwbr @ (ccab @ (^ [Xy1:$i] : (cwrex @ (^ [Xx3:$i] : (cwceq @ (ccv @ Xy1) @ (XB2 @ Xx3))) @ (^ [Xx3:$i] : XA2)))) @ ccom @ ccdom))))).
...
thf(csigaclcu2_conj,conjecture,(! [XA2:($i > ($i > $o))] : (! [XS:($i > $o)] : (cwi @ (cwa @ (cwcel @ XS @ (ccuni @ (ccrn @ ccsiga))) @ (cwral @ (^ [Xk:$i] : (cwcel @ (XA2 @ Xk) @ XS)) @ (^ [Xk:$i] : ccn))) @ (cwcel @ (cciun @ (^ [Xk:$i] : ccn) @ (^ [Xk:$i] : (XA2 @ Xk))) @ XS))))).
