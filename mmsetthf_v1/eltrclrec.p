...
thf(celtrclrec_conj,conjecture,(! [XC:($i > $o)] : (! [XR:($i > $o)] : (! [XV:($i > ($i > ($i > $o)))] : (! [XX:($i > ($i > $o))] : ((cwceq @ XC @ (ccmpt @ (^ [Xr:$i] : ccvv) @ (^ [Xr:$i] : (cciun @ (^ [Xn:$i] : ccn) @ (^ [Xn:$i] : (cco @ (ccv @ Xr) @ (ccv @ Xn) @ ccrelexp)))))) => (! [Xn:$i] : (! [Xr:$i] : (cwi @ (cwcel @ XR @ (XV @ Xn @ Xr)) @ (cwb @ (cwcel @ (XX @ Xr) @ (ccfv @ XR @ XC)) @ (cwrex @ (^ [Xn:$i] : (cwcel @ (XX @ Xr) @ (cco @ XR @ (ccv @ Xn) @ ccrelexp))) @ (^ [Xn:$i] : ccn)))))))))))).
