...
thf(cbrrtrclrec_conj,conjecture,(! [XC:($i > $o)] : (! [XR:($i > $o)] : (! [XV:($i > ($i > ($i > $o)))] : (! [XX:($i > ($i > $o))] : (! [XY:($i > ($i > $o))] : ((cwceq @ XC @ (ccmpt @ (^ [Xr:$i] : ccvv) @ (^ [Xr:$i] : (cciun @ (^ [Xn:$i] : ccn0) @ (^ [Xn:$i] : (cco @ (ccv @ Xr) @ (ccv @ Xn) @ ccrelexp)))))) => (! [Xn:$i] : (! [Xr:$i] : (cwi @ (cwcel @ XR @ (XV @ Xn @ Xr)) @ (cwb @ (cwbr @ (XX @ Xr) @ (XY @ Xr) @ (ccfv @ XR @ XC)) @ (cwrex @ (^ [Xn:$i] : (cwbr @ (XX @ Xr) @ (XY @ Xr) @ (cco @ XR @ (ccv @ Xn) @ ccrelexp))) @ (^ [Xn:$i] : ccn0))))))))))))).
