...
thf(adf_arw_ax,axiom,(cwceq @ ccarw @ (ccmpt @ (^ [Xc:$i] : cccat) @ (^ [Xc:$i] : (ccuni @ (ccrn @ (ccfv @ (ccv @ Xc) @ cchoma))))))).
...
thf(carwrcl_conj,conjecture,(! [XA2:($i > $o)] : (! [XC:($i > $o)] : (! [XF:($i > $o)] : ((cwceq @ XA2 @ (ccfv @ XC @ ccarw)) => (cwi @ (cwcel @ XF @ XA2) @ (cwcel @ XC @ cccat))))))).
